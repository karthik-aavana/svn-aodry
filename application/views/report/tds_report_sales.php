                        ...
                        <table id="list_datatable" class="table table-bordered table-striped table-hover table-responsive">
                            <thead>
                                <tr>
                                    ...
                                    <th>
                                        <a data-backdrop="static" data-keyboard="false" data-toggle="modal" data-target="#tds_receivable_modal">TDS receivable u/s<span class="fa fa-toggle-down"></span></a>
                                    </th>
                                    <th>
                                    ...
                                    <th>
                                        <a data-backdrop="static" data-keyboard="false" data-toggle="modal" data-target="#tds_amt_modal">TDS Amount<span class="fa fa-toggle-down"></span></a>
                                    </th>
                                </tr>
                            </thead>
                            <tbody></tbody>
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            <select class="select2" multiple="multiple"  id="filter_tan"  name="filter_tan">
                                ...
                                <?php
                                foreach ($customers as $key => $value) {
                                    if ($value->customer_tan_number != '') {
                                        ?>
                                        <option value="<?= $value->customer_tan_number ?>"><?= $value->customer_tan_number ?></option>
                                        <?php
                                    }
                                }
                                ?>                               
...
<div class="modal fade" id="tds_receivable_modal" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            <select class="select2" multiple="multiple"  id="filter_tds_section"  name="filter_tds_section">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            <select class="select2" multiple="multiple"  id="filter_tds_percentage"  name="filter_tds_percentage">
                                <option value="">Please Select Rate of Tax</option>
                                <?php
                                foreach ($tds_percentage as $key => $value) {
...
<div class="modal fade" id="taxable_value_modal" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default filter_search" data-dismiss="modal">Apply</button>
                <button type="button" class="btn btn-warning" id="reset-tds-taxable" >Reset</button>
                <!-- <button type="button" class="btn btn-default filter_search_rest" >Reset All</button> -->
            </div>
        </div>      
    </div>
...
<div class="modal fade" id="tds_amt_modal" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            <input type="number" name="from_tds_amount" class="form-control" id="from_tds_amount" placeholder="From" autocomplete="off" min="1">
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div class="form-group">
                            <input type="number" name="to_tds_amount" class="form-control" id="to_tds_amount" placeholder="To" autocomplete="off" min="1">
                        </div>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-default filter_search" data-dismiss="modal">Apply</button>
                <button type="button" class="btn btn-warning" id="reset-tds-amount" >Reset</button>
                <!-- <button type="button" class="btn btn-default filter_search_rest" >Reset All</button> -->
            </div>
        </div>      
    </div>
</div>
<div class="modal fade" id="tds_report_select_columns" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel-3" aria-hidden="true" data-backdrop="static">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="col-md-6">
                                ...
                                <input type="checkbox" name="tan" id="tan" value="2" checked>
                                <label for="tan">TAN</label><br>
   ...
   var tds_datatable;
    $(document).ready(function () {
        ...
        $("#reset-section").click(function () {
            $("#tds_receivable_modal .select2-selection__rendered").empty();
            $('#filter_tds_section option:selected').prop("selected", false);
        })
        $("#reset-percentage").click(function () {
            $("#rate_tax_modal .select2-selection__rendered").empty();
            $('#filter_tds_percentage option:selected').prop("selected", false);
        })
        $("#reset-tds-taxable").click(function () {
            $('#from_taxable_amount').val('');
            $('#to_taxable_amount').val('');
        })
        $("#reset-tds-amount").click(function () {
            $('#from_tds_amount').val('');
            $('#to_tds_amount').val('');
        })
        ...
        function appendFilter() { //button filter event click
            ...
            var filter_customer = "";
            var filter_tan = "";
            var filter_tds_section = "";
            var filter_tds_percentage = "";
            var filter_taxable_amount = "";
            var filter_tds_amount = "";
            ...
            $("#filter_tds_section option:selected").each(function(){
                var $this = $(this);
                if ($this.length) {
                    filter_tds_section += $this.text() + ", ";
                }
            });
            if (filter_tds_section != '') {
                label += "<label id='lbl_tds_section' class='label-style'><b> TDS Receivable U/S : </b> " + filter_tds_section.slice(0, -2) + '<i class="fa fa-times"></i></label>'+" ";
            }
            $("#filter_tds_percentage option:selected").each(function(){
                var $this = $(this);
                if ($this.length) {
                    filter_tds_percentage += $this.text() + ", ";
                }
            });
            if (filter_tds_percentage != '') {
                label += "<label id='lbl_tds_percentage' class='label-style'><b> Rate Of Tax : </b> " + filter_tds_percentage.slice(0, -2) + '<i class="fa fa-times"></i></label>'+" ";
            }
            ...
            var from_tds_amount = $('#from_tds_amount').val();
            var to_tds_amount = $('#to_tds_amount').val();
            if (from_tds_amount != "" || to_tds_amount != "") {
                filter_tds_amount = "<label id='lbl_tds_amount' class='label-style'><b> TDS Amount : </b> FROM " + from_tds_amount + " TO " + to_tds_amount + '<i class="fa fa-times"></i></label>';
            }
            if (filter_tds_amount != '') {
                label += filter_tds_amount + " ";
            }
            if (label != "") {
        ...
        $(document).on('click',".fa-times",function(){
            ...
            if(label1 == 'lbl_tds_section'){
                $("#tds_receivable_modal .select2-selection__rendered").empty();
                $('#filter_tds_section option:selected').prop("selected", false);
                $("#list_datatable").dataTable().fnDestroy()
                generateOrderTable();
            }
            if(label1 == 'lbl_tds_percentage'){
                $("#rate_tax_modal .select2-selection__rendered").empty();
                $('#filter_tds_percentage option:selected').prop("selected", false);
                $("#list_datatable").dataTable().fnDestroy()
                generateOrderTable();
        ...
        tds_datatable = $("#list_datatable").DataTable({
           ...
           "iDisplayLength": 50,
            ...
            buttons: [
                ...
                {extend: 'pdfHtml5',
                    text: 'PDF',
                    filename: 'TDS Receivable Reports (Sales)',
                ...
                },
                {
                    text: 'Select columns',
                    action: function (e, dt, node, config) {
                        $('#tds_report_select_columns').modal('show');
                    }
    ...
    $(document).ready(function(){
        $('#tds_report_select_columns #select_column').click(function(){
            var checked_arr = [];
            var unchecked_arr = [];

            $.each($('#tds_report_select_columns input[type="checkbox"]:checked'),function(key,value){
                checked_arr.push(this.value);
            });

            $.each($('#tds_report_select_columns input[type="checkbox"]:not(:checked)'),function(key,value){
                unchecked_arr.push(this.value);
            });
            
            tds_datatable.columns(checked_arr).visible(true);

            tds_datatable.columns(unchecked_arr).visible(false);
           
            $('#tds_report_select_columns ').modal('hide');
        });
    });
