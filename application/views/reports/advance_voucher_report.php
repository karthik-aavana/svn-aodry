...
<div class="content-wrapper">
    <div class="fixed-breadcrumb">
        <ol class="breadcrumb abs-ol">
            ...
            <li class="active">Advance Voucher Report</li>
        </ol>
    </div>
    <!-- Main content -->
            ...
            <div class="col-md-12">
                <div class="box">
                    <div class="box-header with-border">
                        <h3 class="box-title">Advance Voucher Report</h3>
                            ...
                            <table id="list_datatable" class="custom_datatable table table-bordered table-striped table-hover table-responsive" >
                                <thead>
                                    <tr>
                                        ...
                                        <th><a href="#" data-backdrop="static" data-keyboard="false" class="open_distinct_modal"  data-toggle="modal" data-target="#distinct-modal" data-title="Please Enter Advance Voucher Amount" data-type="received_amount">Advance Voucher amount<span class="fa fa-toggle-down mr-20"></span></a></th>

<!-- <th style="min-width: 50px;text-align: center;"><a href="#" data-backdrop="static" data-keyboard="false" class="open_distinct_modal"  data-toggle="modal" data-target="#distinct-modal" data-title="Please Select Receivable Date" data-type="receivable_date">Tax amount<span class="fa fa-toggle-down mr-20"></span></a></th>                                     <th><a href="#" data-backdrop="static" data-keyboard="false" class="open_distinct_modal"  data-toggle="modal" data-target="#distinct-modal" data-title="Please Select CGST" data-type="taxable_value">CGST<span class="fa fa-toggle-down mr-20"></span></a></th>
<th><a href="#" data-backdrop="static" data-keyboard="false" class="open_distinct_modal"  data-toggle="modal" data-target="#distinct-modal" data-title="Please Select SGST" data-type="sgst">SGST<span class="fa fa-toggle-down mr-20"></span></a></th>
                        ...
                        <div class="col-md-12 hide">
                            <div class="form-group">
                                <select id="modal-select_type"  name="modal-select_type">
                                    ...
                                    <option value="received_amount">Receipt Amount</option>
...
foreach ($invoices as $key => $value) {
    ?>
                                        <option value="<?= $value->advance_voucher_id ?>"><?= $value->voucher_number ?></option>
                                        <?php
                                    }
                                    ?>
                        ...
                        <div class="col-md-12" id="billing_country_div">
                            <div class="form-group">
                                <select class="select2" multiple="multiple"  id="filter_billing_country"  name="filter_billing_countryt">
...
<?php
foreach ($advance_currency as $key => $value) {
    ?>
...
?> 
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-warning" id="reset-invoice">Reset Number</button>
                    <button type="button" class="btn btn-warning" id="reset-in-date">Reset Date</button>
                    <button type="button" class="btn btn-warning" id="reset-in-country">Reset </button>
                    ...
                    <button type="button" class="btn btn-warning" id="reset-place">Reset</button>
                    <button type="button" class="btn btn-warning" id="reset-in-currency">Reset</button>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="col-md-6">
                                ...
                                <input type="checkbox" name="billing_country" id="billing_country" value="3" checked>
                                <label for="billing_country">Billing Country </label><br>

                                <input type="checkbox" name="place_of_supply" id="place_of_supply" value="4" checked>
                                <label for="place_of_supply">Place of Supply</label><br>

                                <input type="checkbox" name="billing_currency" id="billing_currency" value="5" checked>
                                <label for="billing_currency">Billing Currency</label><br>

                            </div>
                            <div class="col-md-6">
                                <input type="checkbox" name="payment_mode" id="payment_mode" value="6" checked>
                                <label for="payment_mode">Payment Mode</label><br>

                                <input type="checkbox" name="sales_taxable_value" id="sales_taxable_value" value="7" checked>
                                <label for="sales_taxable_value">Taxable Amount</label><br>

                                <input type="checkbox" name="sales_cgst_amount" id="sales_cgst_amount" value="8" checked>
                                <label for="sales_cgst_amount">CGST</label><br>

                                <input type="checkbox" name="sales_sgst_amount" id="sales_sgst_amount" value="9"  checked>
                                <label for="sales_sgst_amount">SGST/UTGST</label><br>

                                <input type="checkbox" name="sales_igst_amount" id="sales_igst_amount" value="10" checked>
                                <label for="sales_igst_amount">IGST</label><br>

                                <input type="checkbox" name="grand_total" id="grand_total" value="11" checked>
                                <label for="grand_total">Advance Voucher amount</label><br>
                            </div>
                        </div>
                    </div>
                </form>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#reset-in-country').click(function () {
        ...
        $('#reset-in-currency').click(function () {
        ...
        $("#modal-select_type").on("change", function (event)
        {
            ...
            if (type == "billing_country")
            {
                ...
                $("#reset-in-country").show();
            } else
            {
                $("#billing_country_div").hide();
                $("#reset-in-country").hide();
            }

            if (type == "billing_currency")
            {
                $("#billing_currency_div").show();
                $("#reset-in-currency").show();
            } else
            {
                $("#billing_currency_div").hide();
                $("#reset-in-currency").hide();
            }
        ...
        function resetall() {
            ...
            $('#filter_filter_payment_mode option:selected').prop("selected", false)
        ...
        function generateOrderTable() {
            list_datatable = $('#list_datatable').DataTable({
               ...
               "iDisplayLength": 50,
                ...
                "ajax": {
                    "url": base_url + "report/advance_voucher_report",
                    "dataType": "json",
                    "type": "POST",
                    "data": {
                        ...
                        'from_taxable_amount': $('#from_taxable_amount').val(),
                        'to_taxable_amount': $('#to_taxable_amount').val(),
                        'from_sgst_amount': $('#from_sgst_amount').val(),
                        'to_sgst_amount': $('#to_sgst_amount').val(),
                        'from_cgst_amount': $('#from_cgst_amount').val(),
                        'to_cgst_amount': $('#to_cgst_amount').val(),
                        'from_igst_amount': $('#from_igst_amount').val(),
                        'to_igst_amount': $('#to_igst_amount').val(),
                    ...
                    "dataSrc": function (result) {
                        ...
                        var tfoot = parseFloat(result.total_list_record[0].tot_voucher_igst_amount).toFixed(2);
                        $('#total_list_record5').html(tfoot);
                        tfoot = parseFloat(result.total_list_record[0].tot_receipt_amount).toFixed(2);
              ...
              dom: 'Bfrltip',
                buttons: [
                    ...
                    {extend: 'pdfHtml5',
                        text: 'PDF',
                        filename: 'Advance Voucher Report',
        ...
        function appendFilter() { //button filter event click
            ...
            if (from_reciept_amount != "" || to_reciept_amount != "") {
                filter_invoice_amount = "<label id='lbl_invoice_Amount' class='label-style'><b> Advance Voucher Amount : </b> FROM " + from_reciept_amount + " TO " + to_reciept_amount + '<i class="fa fa-times"></i></label>';
            }
...
$(document).ready(function(){
    $('#advance_voucher_report_select_columns #select_column').click(function(){
        var checked_arr = [];
        var unchecked_arr = [];

        $.each($('#advance_voucher_report_select_columns input[type="checkbox"]:checked'),function(key,value){
            checked_arr.push(this.value);
        });

        $.each($('#advance_voucher_report_select_columns input[type="checkbox"]:not(:checked)'),function(key,value){
        ...
        $('#advance_voucher_report_select_columns').modal('hide');
    });
});
</script>
