...
<html>
    <head>
        ...
        <title> <?= (@$title ? $title : 'outlet Invoice'); ?> </title>
    ...
    <body>
        ...
        <span style="float: right; margin-top: -25px"><h3><u><?= (@$title ? $title : 'Outlet Invoice'); ?></u><br><small style="font-size: 8px;line-height: 2"><?=($invoice_type != '' ? '('.$invoice_type.')' : ''); ?></small></h3>
        ...
        <table class="table header_table mt-20" >
            <tr>
                <td valign="top">
                    ...
                    <?php
                    $e_way_bill_date = '';
                    $date = $data[0]->outlet_e_way_bill_date;
                    $e_way_bill_date = $date_for_e_bill = date('d-m-Y', strtotime($date));
                    ?>
                    <?php
                    if (isset($data[0]->outlet_e_way_bill_date)) {
                        if ($data[0]->outlet_e_way_bill_date != '' && $data[0]->outlet_e_way_bill_date != '0000-00-00') {
                            echo '<br>E Way Bill Date : ' . $e_way_bill_date;
                        }
                    }
                    ?>
                     <?php
                    if (isset($data[0]->outlet_e_way_bill_number)) {
                        if ($data[0]->outlet_e_way_bill_number != "") {
                            echo '<br>E Way Bill Number : ' . $data[0]->outlet_e_way_bill_number;
                        }
                    }
                    ?>
                </td>
                <td width="33%" valign="top" class="bg-table">
                    ...
                    Invoice Number:                  
                    <span class="bold"><?php
                        if (isset($data[0]->outlet_invoice_number)) {
                        ...
                        <?php if ($tds_exist > 0) { // && strtolower($value->tds_module_type) == 'tcs'   ?>
                            <td style="text-align: right;">
                                ...
                                if ($value->outlet_item_tds_amount < 1) {
                                    ...
                                    ?><?= precise_amount($value->outlet_item_tds_amount); ?><br>(<?= round($value->outlet_item_tds_percentage, 2); ?>%)
                                <?php } ?>
                            </td>
                        <?php } ?>
                        <?php
                ...
                <td style="width:30%">
                    <table class="total_sum">
                        <tr>                 
                            ...
                            <td align="right"><?= $this->numbertowords->formatInr(($data[0]->outlet_sub_total * $convert_rate)); ?></td>
                        </tr>
                        <?php if ($discount_exist > 0) { ?>
                            <tr>
                                <td align="right">Discount (-)</td>
                                <td align="right"><?= $this->numbertowords->formatInr(($data[0]->outlet_discount_amount * $convert_rate)); ?></td>
                            </tr>
                        <?php } ?>
                        <?php if ($tds_exist > 0) { ?>              
                            <?php if ($data[0]->outlet_tcs_amount > 0) { ?>
                                <tr>
                                    ...
                                    <td align="right"><?= $this->numbertowords->formatInr(($data[0]->outlet_tcs_amount * $convert_rate)); ?></td>
                                </tr>
                            <?php } ?>
                        <?php } ?>
                        <?php if ($igst_exist > 0) { ?>
                            <tr>
                                ...
                                <td align="right"><?= $this->numbertowords->formatInr(($data[0]->outlet_igst_amount * $convert_rate)); ?></td>
                            </tr>
                        <?php } elseif ($cgst_exist > 0 || $sgst_exist > 0) { ?>
                            <tr>
                                <td align="right"><?= ($is_utgst == '1' ? 'UTGST' : 'SGST'); ?></td>
                                <td align="right"><?= $this->numbertowords->formatInr(($data[0]->outlet_sgst_amount * $convert_rate)); ?></td>
                            </tr>
                            <tr>
                                <td align="right">CGST</td>
                                <td align="right"><?= $this->numbertowords->formatInr(($data[0]->outlet_cgst_amount * $convert_rate)); ?></td>
                            </tr>
                        <?php } ?>
                        <?php if ($cess_exist > 0) { ?>
                            <tr>
                                <td align="right">CESS</td>
                                <td align="right"><?= $this->numbertowords->formatInr(($data[0]->outlet_tax_cess_amount * $convert_rate)); ?></td>
                            </tr>   
                        <?php } ?>
                        <?php if ($data[0]->total_freight_charge > 0) { ?>
                            <tr>
                        ...
                        <tr>
                            ...
                            <td align="right"><b><?php echo $this->numbertowords->formatInr(($data[0]->outlet_grand_total * $convert_rate)); ?>/-</td>
                        </tr>
                    </table> 
                </td>
            </tr>
        ...
        <table class="mt-50">
            <tr>
                <td class="amount_text">
                    Amount (in Words): <b><?php echo $data[0]->currency_name . " " . $this->numbertowords->convert_number(precise_amount(($data[0]->outlet_grand_total * $convert_rate)),$data[0]->unit,$data[0]->decimal_unit) . " Only"; ?> </b>
                </td>
            </tr>
        </table>
        <?php
        if (isset($data[0]->outlet_type_of_supply) && $data[0]->outlet_type_of_supply != "regular") {
            ?>        
            <div id="notices">
                <div>
                    NOTICE:
                ...
                <div class="notice">
                    <?php
                    if (isset($data[0]->outlet_type_of_supply) && $data[0]->outlet_type_of_supply == "export_with_payment") {
                        ?>
                        <p><small> SUPPLY MEANT FOR EXPORT ON PAYMENT OF INTEGRATED TAX</small></p>
                    <?php } else if (isset($data[0]->outlet_type_of_supply) && $data[0]->outlet_type_of_supply == "export_without_payment") { ?>
