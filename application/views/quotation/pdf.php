...
<html>
    <head>
        ...
        <title> <?= (@$title ? $title : 'Quotation'); ?> </title>
    ...
    <body>
        ...
        <span style="float: right; margin-top: -25px"><h3><u><?= (@$title ? $title : 'Quotation'); ?></u><br><!-- <small style="font-size: 8px;line-height: 2">(<?php echo $invoice_type; ?>)</small> --></h3>
        ...
        <table class="table header_table mt-20" >
            <tr>
                ...
                <td width="33%" valign="top" class="bg-table">
                    ...
                    ?>                  
                    <span class="bold"><?php
                        if (isset($data[0]->quotation_date)) {
                            echo $date_for;
                        }
                        ?></span>
                    <br>
                ...
                foreach ($items as $value) {
                    ...
                    <tr>
                        ...
                        <td><?php
                            ...
                            ?></td>
                        <td style="text-align: right;"><?php echo precise_amount(($value->quotation_item_unit_price * $convert_rate)); ?></td>
                        <td style="text-align: right;"><?php echo precise_amount(($value->quotation_item_sub_total * $convert_rate)); ?></td>
                        <?php
                        if ($discount_exist > 0) {
                            ?>
                            <td style="text-align: right;"><?php echo precise_amount(($value->quotation_item_discount_amount * $convert_rate)); ?></td>
                        <?php } ?>
                        <?php
                        if ($discount_exist > 0 && ($tax_exist > 0 || $igst_exist > 0 || $cgst_exist > 0 || $sgst_exist > 0 )) {
                            ?>
                            <td style="text-align: right;"><?php echo precise_amount(($value->quotation_item_taxable_value * $convert_rate)); ?></td>
                        <?php } ?>
                            ...
                            <td style="text-align: right;">
                                <?php if ($value->item_type == 'product' || $value->item_type == 'product_inventory') { ?>
                                    ...
                                    if ($value->quotation_item_tds_amount < 1) {
                                        ...
                                        ?><?= precise_amount($value->quotation_item_tds_amount * $convert_rate); ?><br>(<?= round($value->quotation_item_tds_percentage, 2); ?>%)<?php } ?>
                                    <?php
                                } else {
                                    echo '-';
                                }
                        ...
                        if ($tax_exist > 0) {
                            ...
                            <td style="text-align: right;">
                                ...
                                if ($value->quotation_item_tax_amount < 1) {
                                    ...
                                    ?>
                                    <?php echo precise_amount(($value->quotation_item_tax_amount * $convert_rate)); ?><br>(<?= ($value->quotation_item_tax_percentage > 0 ? round(abs($value->quotation_item_tax_percentage), 2) . '%' : '-'); ?>)<?php } ?></td>
                        <?php } elseif ($igst_exist > 0) { ?>
                            <td style="text-align: right;">
                                <?php
                                if ($value->quotation_item_igst_amount < 1) {
                                    ...
                                    ?>
                                    <?= ($value->quotation_item_igst_amount > 0 ? precise_amount($value->quotation_item_igst_amount * $convert_rate) : '-'); ?>
                        ...
                        <?php } elseif ($cgst_exist > 0 || $sgst_exist > 0) { ?>
                            <td style="text-align: right;">
                                ...
                                if ($value->quotation_item_cgst_amount < 1) {
                                    ...
                                    ?>
                                    <?php echo precise_amount(($value->quotation_item_cgst_amount * $convert_rate)); ?><br>(<?php echo round($value->quotation_item_cgst_percentage, 2); ?>%)<?php } ?></td>
                            <td style="text-align: right;">
                                <?php
                                if ($value->quotation_item_sgst_amount < 1) {
                                    echo '-';
                                } else {
                                    ?><?php echo precise_amount(($value->quotation_item_sgst_amount * $convert_rate)); ?><br>(<?php echo round($value->quotation_item_sgst_percentage, 2); ?>%)<?php } ?></td>
                        <?php } ?>
                        <?php if ($cess_exist > 0) { ?>
                            <td style="text-align: right;">
                                <?php
                                if ($value->quotation_item_tax_cess_amount < 1) {
                                    ...
                                    ?><?= precise_amount(($value->quotation_item_tax_cess_amount * $convert_rate)); ?><br>(<?= round($value->quotation_item_tax_cess_percentage, 2); ?>%)<?php } ?></td>
                        <?php } ?>
                        <td style="text-align: right;"><?php echo precise_amount(($value->quotation_item_grand_total * $convert_rate)); ?></td>
                    </tr>
                    <?php
                    $i++;
                    $quantity = bcadd($quantity, $value->quotation_item_quantity);
                    $price = bcadd($price, $value->quotation_item_unit_price, 2);
                    $grand_total = bcadd($grand_total, $value->quotation_item_grand_total, 2);
                    if ($igst_exist > 0) $tot_igst += $value->quotation_item_igst_amount;
                    if ($sgst_exist > 0) $tot_sgst += $value->quotation_item_sgst_amount;
                    if ($cgst_exist > 0) $tot_cgst += $value->quotation_item_cgst_amount;
                }
                ?>
                <tr>
                    <th colspan="2"></th>
                    ...
                    <th  style="text-align: right;"><?= precise_amount(($data[0]->quotation_sub_total * $convert_rate)) ?></th>
                    <?php
                    if ($discount_exist > 0) {
                        ?>
                        <th  style="text-align: right;"><?= precise_amount(($data[0]->quotation_discount_amount * $convert_rate)) ?></th>
                    <?php } ?>
                    <?php
                    if ($discount_exist > 0 && ($tax_exist > 0 || $igst_exist > 0 || $cgst_exist > 0 || $sgst_exist > 0 )) {
                        ?>
                        <th  style="text-align: right;"><?= precise_amount(($data[0]->quotation_taxable_value * $convert_rate)) ?></th>
                    <?php } ?>
                    <?php
                    if ($tds_exist > 0) {
                        ?>
                        <!-- <th colspan="2" ><?= precise_amount((($data[0]->quotation_tds_amount + $data[0]->quotation_tcs_amount) * $convert_rate)) ?></th> -->
                        <th style="text-align: right;"><?= precise_amount((($data[0]->quotation_tcs_amount) * $convert_rate)) ?></th>
                    <?php } ?>
                    <?php
                    if ($tax_exist > 0) {
                        ?>
                        <th style="text-align: right;"><?= precise_amount(($data[0]->quotation_tax_amount * $convert_rate)) ?></th>
                    <?php } elseif ($igst_exist > 0) { ?>
                    ...
                    <?php } ?>
                    <?php if ($cess_exist > 0) { ?>
                        <th style="text-align: right;"><?= precise_amount(($data[0]->quotation_tax_cess_amount * $convert_rate)) ?></th>
                    <?php } ?>
                    <th  style="text-align: right;"><?= precise_amount($grand_total* $convert_rate, 2); ?></th>
                </tr>
            </tbody>
        </table>
        <table class="mt-20">
                    ...
                    <?php } ?>
                       ...
                       <!--  <?php if ($data[0]->total_other_amount != 0) { ?>
                            ...
                            <tbody>
                                <tr>
                                    <?php if ($data[0]->total_freight_charge > 0) { ?>
                                        <th><?php echo precise_amount(($data[0]->total_freight_charge * $convert_rate)); ?> /-
                                        <table class="tax_calculation">
                                            <tr><th>CGST:</th><td>7812.45</td></tr>
                                            <tr><th>SGST:</th><td>7812.45</td></tr>
                                            <tr><th>IGST:</th><td>7812.45</td></tr>
                                        </table>                                        
                                        </th>
                                        <?php
                                    }
                                    if ($data[0]->total_insurance_charge > 0) {
                                        ?>
                                        <th><?php echo precise_amount(($data[0]->total_insurance_charge * $convert_rate)); ?> /-
                                    ...
                                    if ($data[0]->total_packing_charge > 0) {
                                        ?>
                                        <th><?php echo precise_amount(($data[0]->total_packing_charge * $convert_rate)); ?> /-
                                        <table class="tax_calculation">
                                              <tr><th>CGST:</th><td>7812.45</td></tr>
                                            <tr><th>SGST:</th><td>7812.45</td></tr>
                                            <tr><th>IGST:</th><td>7812.45</td></tr>
                                    ...
                                    if ($data[0]->total_incidental_charge > 0) {
                                        ?>
                                        <th><?php echo precise_amount(($data[0]->total_incidental_charge * $convert_rate)); ?> /-
                                        <table class="tax_calculation">
                                             <tr><th>CGST:</th><td>7812.45</td></tr>
                                            <tr><th>SGST:</th><td>7812.45</td></tr>
                                            <tr><th>IGST:</th><td>7812.45</td></tr>
                                    ...
                                    if ($data[0]->total_inclusion_other_charge > 0) {
                                        ?>
                                        <th><?php echo precise_amount(($data[0]->total_inclusion_other_charge * $convert_rate)); ?> /-
                                        <table class="tax_calculation">
                                              <tr><th>CGST:</th><td>7812.45</td></tr>
                                            <tr><th>SGST:</th><td>7812.45</td></tr>
                                            <tr><th>IGST:</th><td>7812.45</td></tr>
                                    ...
                                    if ($data[0]->total_exclusion_other_charge > 0) {
                                        ?>
                                        <th><?php echo precise_amount(($data[0]->total_exclusion_other_charge * $convert_rate)); ?> /-
                                        <table class="tax_calculation">
                                            <tr><th>CGST:</th><td>7812.45</td></tr>
                                            <tr><th>SGST:</th><td>7812.45</td></tr>
                                            <tr><th>IGST:</th><td>7812.45</td></tr>
                ...
                <td style="width:30%">
                    <table class="total_sum">
                        <tr>                 
                            ...
                            <td align="right"><?= $this->numbertowords->formatInr(($data[0]->quotation_sub_total * $convert_rate)); ?></td>
                        </tr>
                        <?php if ($discount_exist > 0) { ?>
                            <tr>
                                <td align="right">Discount (-)</td>
                                <td align="right"><?= $this->numbertowords->formatInr(($data[0]->quotation_discount_amount * $convert_rate)); ?></td>
                            </tr>
                        <?php } ?>
                        <?php if ($tds_exist > 0) { ?>              
                            <?php if ($data[0]->quotation_tcs_amount > 0) { ?>
                                <tr>
                                    <td align="right">TCS</td>
                                    <td align="right"><?= $this->numbertowords->formatInr(($data[0]->quotation_tcs_amount * $convert_rate)); ?></td>
                                </tr>
                            <?php } ?>
                        <?php } ?>
                        <?php if ($igst_exist > 0) { ?>
                            <tr>
                                ...
                                <td align="right"><?= $this->numbertowords->formatInr(($data[0]->quotation_igst_amount * $convert_rate)); ?></td>
                            </tr>
                        <?php } elseif ($cgst_exist > 0 || $sgst_exist > 0) { ?>
                            <tr>
                                <td align="right"><?= ($is_utgst == '1' ? 'UTGST' : 'SGST'); ?></td>
                                <td align="right"><?= $this->numbertowords->formatInr(($data[0]->quotation_sgst_amount * $convert_rate)); ?></td>
                            </tr>
                            <tr>
                                <td align="right">CGST</td>
                                <td align="right"><?= $this->numbertowords->formatInr(($data[0]->quotation_cgst_amount * $convert_rate)); ?></td>
                            </tr>
                        <?php } ?>
                        <?php if ($cess_exist > 0) { ?>
                            <tr>
                                <td align="right">CESS</td>
                                <td align="right"><?= $this->numbertowords->formatInr(($data[0]->quotation_tax_cess_amount * $convert_rate)); ?></td>
                            </tr>   
                        <?php } ?>
                        <?php if ($data[0]->total_freight_charge > 0) { ?>
                            <tr>
                        ...
                        <tr>
                            ...
                            <td align="right"><b><?php echo $this->numbertowords->formatInr(($data[0]->quotation_grand_total * $convert_rate)); ?>/-</td>
                        </tr>
                    </table> 
                </td>
            </tr>
        ...
        <table class="table mt-50">
            <tr>
                <td class="amount_text">
                    Amount (in Words): <b><?php echo $data[0]->currency_name . " " . $this->numbertowords->convert_number(precise_amount(($data[0]->quotation_grand_total * $convert_rate)),$data[0]->unit,$data[0]->decimal_unit) . " Only"; ?> </b>
                </td>
            </tr>
        </table>
        <?php
        if (isset($data[0]->quotation_type_of_supply) && $data[0]->quotation_type_of_supply != "regular") {
            ?>        
            <div id="notices mt-50">
                ...
                <div class="notice">
                    <?php
                    if (isset($data[0]->quotation_type_of_supply) && $data[0]->quotation_type_of_supply == "export_with_payment") {
                        ?>
                        <p><small> SUPPLY MEANT FOR EXPORT ON PAYMENT OF INTEGRATED TAX</small></p>
                    <?php } else if (isset($data[0]->quotation_type_of_supply) && $data[0]->quotation_type_of_supply == "export_without_payment") { ?>
