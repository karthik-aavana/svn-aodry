...
<html>
  <head>
    ...
    <title>
            <?= (@$title ? $title : 'Sales debit note Invoice'); ?>
    </title>
  ...
  <body>
        ...
        <span style="float: right; margin-top: -25px"><h3><u><?= (@$title ? $title : 'Sales Debit Note Invoice'); ?></u><br><small style="font-size: 8px;line-height: 2">(<?php echo $invoice_type; ?>)</small></h3>
           <br><p class="right"></p></span>
        <!-- <span style="float: right; margin-top: -25px"><h3><u><?= (@$title ? $title : 'Sales Debit Note Bill'); ?></u></h3></span> -->
      ...
      <tr>
          ...
          <?php
                  ...
                  echo '<br>E-mail : ' . $data[0]->customer_email;
                    ...
                    <?php
                    $e_way_bill_date = '';
                    $date = $data[0]->sales_debit_note_e_way_bill_date;
                    $date_for_e_bill = date('d-m-Y', strtotime($date));
                    ?>
                    <?php
                    if (isset($data[0]->sales_debit_note_e_way_bill_date)) {
                        if ($data[0]->sales_debit_note_e_way_bill_date != '' && $data[0]->sales_debit_note_e_way_bill_date != '0000-00-00') {
                            echo '<br>E Way Bill Date : ' . $date_for_e_bill;
                        }
                    }
                    ?>
                    <?php
                    if (isset($data[0]->sales_debit_note_e_way_bill_number)) {
                        if ($data[0]->sales_debit_note_e_way_bill_number != "") {
                            echo '<br>E Way Bill Number : ' . $data[0]->sales_debit_note_e_way_bill_number;
                        }
                    }
                    ?>
                    <?php
          ...
          foreach ($items as $value) {
            ...
            <tr>
              ...
              <td ><?php
                  ...
                  ?></td>
                        <td style="text-align: right;"><?php echo precise_amount($value->sales_debit_note_item_unit_price, 2); ?></td>
                        <td style="text-align: right;"><?php echo precise_amount($value->sales_debit_note_item_sub_total, 2); ?></td>
              <?php
                        if ($dtcount > 0) {
                  ?>
                            <td style="text-align: right;"><?php echo precise_amount($value->sales_debit_note_item_discount_amount, 2); ?></td>
              <?php } ?>
              <?php
                        if ($dtcount > 0 && ($tax_exist > 0 || $igst_exist > 0 || $cgst_exist > 0 || $sgst_exist > 0 )) {
                  ?>
                            <td style="text-align: right;"><?php echo precise_amount($value->sales_debit_note_item_taxable_value, 2); ?></td>
              <?php } ?>
              <?php
                        if ($tds_exist > 0) {
                  ?>
                            <td style="text-align: right;">
                              ...
                              <?php if($value->sales_debit_note_item_tds_amount < 1){ echo '-'; }else{ ?>
                              <?php echo precise_amount($value->sales_debit_note_item_tds_amount, 2); ?><br>(<?php echo round($value->sales_debit_note_item_tds_percentage,2); ?>%)
                              <?php } ?>
                              <?php }else{ 
                                  echo '-';
                              } ?>
              ...
              <?php
                  ...
                  ?>
                            <td style="text-align: right;">
                              <?php if($value->sales_debit_note_item_tax_amount < 1){ echo '-'; }else{ ?>
                              <?php echo precise_amount($value->sales_debit_note_item_tax_amount, 2); ?><br>(<?php echo round($value->sales_debit_note_item_tax_percentage,2); ?>%)
                              <?php } ?>
                            </td>
                  <?php
                        } elseif ($igst_exist > 0) {
                  ?>
                            <td style="text-align: right;">
                              <?php if($value->sales_debit_note_item_igst_amount < 1){ echo '-'; }else{ ?>
                              <?php echo precise_amount($value->sales_debit_note_item_igst_amount, 2); ?><br>(<?php echo round($value->sales_debit_note_item_igst_percentage,2); ?>%)
                              <?php } ?>
                            </td>
                  <?php
                        } elseif ($cgst_exist > 0 || $sgst_exist > 0) {
                  ?>
                            <td style="text-align: right;">
                              <?php if($value->sales_debit_note_item_cgst_amount < 1){ echo '-'; }else{ ?>
                              <?php echo precise_amount($value->sales_debit_note_item_cgst_amount, 2); ?><br>(<?php echo round($value->sales_debit_note_item_cgst_percentage,2); ?>%)
                              <?php } ?>
                            </td>                  
                            <td style="text-align: right;">
                              <?php if($value->sales_debit_note_item_sgst_amount < 1){ echo '-'; }else{ ?>
                              <?php echo precise_amount($value->sales_debit_note_item_sgst_amount, 2); ?><br>(<?php echo round($value->sales_debit_note_item_sgst_percentage); ?>%)
                              <?php } ?>
                            </td>
              <?php } ?>
                <?php if ($cess_exist > 0) { ?>
                        <td style="text-align: right;">
                          <?php if($value->sales_debit_note_item_tax_cess_amount < 1){ echo '-'; }else{ ?>
                          <?= precise_amount(($value->sales_debit_note_item_tax_cess_amount * $convert_rate)); ?><br>(<?= round($value->sales_debit_note_item_tax_cess_percentage,2); ?>%)
                          <?php } ?>
                        </td>
                    <?php } ?>
                        <td style="text-align: right;"><?php echo precise_amount($value->sales_debit_note_item_grand_total, 2); ?></td>
            </tr>
            <?php
            $i++;
            /*$quantity += $quantity;*/
              ...
              if ($igst_exist > 0) $tot_igst += $value->sales_debit_note_item_igst_amount;
              if ($sgst_exist > 0) $tot_sgst += $value->sales_debit_note_item_sgst_amount;
              if ($cgst_exist > 0) $tot_cgst += $value->sales_debit_note_item_cgst_amount;
        }
        ?>
        <tr>
          ...
          <th  ><!-- <?= round($quantity,2) ?> --></th>
                    ...
                    <th  style="text-align: right;"><?= precise_amount($data[0]->sales_debit_note_sub_total, 2) ?></th>
          <?php
                    if ($dtcount > 0) {
              ?>
                        <th style="text-align: right;"><?= precise_amount($data[0]->sales_debit_note_discount_amount, 2) ?></th>
          <?php } ?>
          <?php
                    if ($dtcount > 0 && ($tax_exist > 0 || $igst_exist > 0 || $cgst_exist > 0 || $sgst_exist > 0 )) {
              ?>
                        <th style="text-align: right;"><?= precise_amount($data[0]->sales_debit_note_taxable_value, 2) ?></th>
          <?php } ?>
          <?php
                    if ($tds_exist > 0) {
              ?>
                        <th style="text-align: right;"><?= precise_amount(($data[0]->sales_debit_note_tcs_amount), 2); ?></th>
          <?php }
          ?>
          <?php
                    if ($tax_exist > 0) {
              ?>
                        <th style="text-align: right;"><?= precise_amount($data[0]->sales_debit_note_tax_amount, 2) ?></th>
              <?php
                    } elseif ($igst_exist > 0) {
              ?>
          ...
          ?>
          <?php if ($cess_exist > 0) { ?>
                        <th style="text-align: right;"><?= precise_amount(($data[0]->sales_debit_note_tax_cess_amount * $convert_rate)) ?></th>
                    <?php } ?>
                    
                    <th  ><?= precise_amount($grand_total, 2); ?></th>
        </tr>
      </tbody>
    </table>
    <table class="mt-20">
            ...
            <tr>                   
                ...
                <td align="right"><?= $this->numbertowords->formatInr(($data[0]->sales_debit_note_sub_total * $convert_rate)); ?></td>
            </tr>
            <?php if ($discount_exist > 0) { ?>
            <tr>
                <td align="right">Discount (-)</td>
                <td align="right"><?= $this->numbertowords->formatInr(($data[0]->sales_debit_note_discount_amount * $convert_rate)); ?></td>
            </tr>
            <?php } ?>
            <?php if ($tds_exist > 0) { ?>
                <!-- <?php if ($data[0]->sales_debit_note_tds_amount > 0) { ?> 
                    <tr>
                        <td align="right">TDS</td>
                        <td align="right"><?= precise_amount(($data[0]->sales_debit_note_tds_amount * $convert_rate)); ?></td>
                    </tr>
                <?php } ?> -->
                <?php if ($data[0]->sales_debit_note_tcs_amount > 0) { ?>
                    <tr>
                        <td align="right">TCS</td>
                        <td align="right"><?= $this->numbertowords->formatInr(($data[0]->sales_debit_note_tcs_amount * $convert_rate)); ?></td>
                    </tr>
                <?php } ?>
            <?php } ?>
            <?php if ($igst_exist > 0) { ?>
            <tr>
                ...
                <td align="right"><?= $this->numbertowords->formatInr(($data[0]->sales_debit_note_igst_amount * $convert_rate)); ?></td>
            </tr>
            <?php } elseif ($cgst_exist > 0 || $sgst_exist > 0) { ?>
                <tr>
                    <td align="right"><?= ($is_utgst == '1' ? 'UTGST' : 'SGST'); ?></td>
                    <td align="right"><?= $this->numbertowords->formatInr(($data[0]->sales_debit_note_sgst_amount * $convert_rate)); ?></td>
                </tr>
                <tr>
                    <td align="right">CGST</td>
                    <td align="right"><?= $this->numbertowords->formatInr(($data[0]->sales_debit_note_cgst_amount * $convert_rate)); ?></td>
                </tr>
            <?php } ?>
            <?php if ($cess_exist > 0) { ?>
            <tr>
                <td align="right">CESS</td>
                <td align="right"><?= $this->numbertowords->formatInr(($data[0]->sales_debit_note_tax_cess_amount * $convert_rate)); ?></td>
            </tr> 
            <?php } ?>
            <?php if ($data[0]->total_freight_charge > 0) { ?>
                <tr>
            ...
            <tr>
                ...
                <td align="right"><b><?php echo $this->numbertowords->formatInr(($data[0]->sales_debit_note_grand_total * $convert_rate)); ?>/-</td>
            </tr>           
        </table>
        </td>
        </tr>
        ...
        <div class="mt-50">
            <p class="amount_text">
                Amount (in Words): <b><?php echo $data[0]->currency_name . " " . $this->numbertowords->convert_number($data[0]->sales_debit_note_grand_total,$data[0]->unit,$data[0]->decimal_unit) . " Only"; ?> </b>
            </p>
        </div>   
                <?php
        if (isset($data[0]->sales_debit_note_type_of_supply) && $data[0]->sales_debit_note_type_of_supply != "regular") {
            ...
            <div id="notices mt-50">
                ...
                <div class="notice">
                    <?php if (isset($data[0]->sales_debit_note_type_of_supply) && $data[0]->sales_debit_note_type_of_supply == "export_with_payment") { ?>
                        <p>SUPPLY MEANT FOR EXPORT ON PAYMENT OF INTEGRATED TAX</p>
                  <?php
                    } else if (isset($data[0]->sales_debit_note_type_of_supply) && $data[0]->sales_debit_note_type_of_supply == "export_without_payment") {
                  ?>
