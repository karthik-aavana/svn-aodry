    ...
    <div class="row">
      ...
      <div class="col-md-12">
        <div class="box">
          ...
          <!-- /.box-header -->
          <form role="form" id="form" method="post" action="<?php echo base_url('payment_voucher/edit_payment'); ?>">
            <div class="box-body">
              <div class="row">
                <div class="col-md-12">
                  <div class="well">
                    <div class="row">
                      ...
                      <div class="col-sm-3">
                        <div class="form-group">
                          ...
                          <input type="text" class="form-control number_only" id="total_payment_amount" name="total_payment_amount" value="<?php echo precise_amount($data[0]->receipt_amount); ?>">
                    ...
                    <?php if(!empty($data)){
                      ...
                      foreach ($data as $key => $value) {  
                        ...
                        if($value->reference_id != 'excess_amount' && $value->reference_id != '0'){
                        ?>
                        <div class="row row_count main_row <?=($value->is_edit == '0' ? 'editable_disable' : '');?>" row="<?=$i;?>" id="ToClone">
                              ...
                              <select class="reference_number form-control select2" id="reference_number_<?=$i;?>" name="reference_number">
                                ...
                                <?php if(!empty($purchase_data)){
                                  ...
                                  foreach ($purchase_data as $key => $purchase) { 
                                      $selected = '';
                                      $invoice_total = 0;
                                      if($purchase->purchase_id == $value->reference_id){
                                        $purchase->purchase_grand_total = $purchase->supplier_payable_amount;
                                        $purchase->purchase_paid_amount -= $value->invoice_payment_amount;
                                        $selected = 'selected';
                                      } 
                                      $data_items['purchase_'.$purchase->purchase_id] = $purchase;
                                      ?>
                                      <option value="<?=$purchase->purchase_id;?>" <?=$selected;?>><?=$purchase->purchase_invoice_number;?></option>
                                  <?php } ?>
                                <?php } ?>
                              </select>
                          ...
                          <div class="col-sm-2 gain_loss_amount_div">
                            <div class="form-group">
                              ...
                              <span id='gain_visible' class="toggle_lbl">Loss(<?=($value->exchange_gain_loss_type == 'plus' ? '+' : '-')?>)</span>         
                          ...
                          <input type="hidden" class="form-control" id="current_paid_amount_<?=$i;?>" name="current_paid_amount"  value="<?=($value->invoice_payment_amount != '' ? precise_amount($value->invoice_payment_amount) : '0.00');?>" >
                          ...
                          <input type="hidden" class="form-control" id="old_payment_amount_<?=$i;?>" name="old_payment_amount"  value="<?=($value->invoice_payment_amount != '' ? precise_amount($value->invoice_payment_amount) : '0.00');?>" >
                          <input type="hidden" name="payment_amount_old" value="<?=$data[0]->receipt_amount;?>">
                          <input type="hidden" name="converted_payment_amount_old" value="<?=$data[0]->receipt_amount;?>">
                          <input type="hidden" class="form-control" id="last_payment_amount_<?=$i;?>" name="last_payment_amount"  value="<?=($value->invoice_payment_amount != '' ? precise_amount($value->invoice_payment_amount) : '0.00');?>">
                          <!-- hidden -->
                        </div>
                      <?php } else { ?>
                          <div class="row row_count main_row" row="<?=$i;?>" >
                          <div class="col-sm-2 reference_number_div" style="width: 20%">
                            <div class="form-group">
                              <label for="supplier">Reference<br/>Number<span class="validation-color">*</span></label>
                              <a type="buttion" href="#" class="<?=($i == 1 ? 'add_row_button' : 'remove_row_button');?> pull-right">(<?=($i == 1 ? '+' : '-');?>)</a>
                          ...
                          <div class="col-sm-2 payment_amount_div">
                            <div class="form-group">
                              ...
                              <input type="text" class="form-control" id="payment_amount_<?=$i;?>" name="payment_amount" value="<?=precise_amount($value->invoice_payment_amount);?>">
                              <span class="validation-color" id="err_payment_amount_<?=$i;?>"></span>
                            </div>
                          </div>
                        </div>
