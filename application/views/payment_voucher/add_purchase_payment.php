      ...
      <div class="col-md-12">
        <div class="box">
          <div class="box-header with-border">
            ...
            <a class="btn btn-default pull-right back_button" id="sale_cancel" onclick1="cancel('purchase')">Back</a>
          </div>
          <select id="common_select" style="display: none;">
            ...
            <?php if(!empty($data)){
              ...
              foreach ($data as $key => $purchase) { 
                  $purchase->purchase_grand_total = $purchase->supplier_payable_amount;
                  $data_items['purchase_'.$purchase->purchase_id] = $purchase; ?>
                  <option value="<?=$purchase->purchase_id;?>"><?=$purchase->purchase_invoice_number;?></option>
              <?php } 
            } ?>
          </select>
          <!-- /.box-header -->
          <form role="form" id="form" method="post" action="<?php echo base_url('payment_voucher/add_payment'); ?>">
            <div class="box-body">
              <div class="row">
                <div class="col-md-12">
                  <div class="well">
                      ...
                      <div class="col-sm-3">
                        <div class="form-group">
                          ...
                          <select class="form-control select2" id="supplier" name="supplier" style="width: 100%;">
                              ...
                              {
                                  if ($row->supplier_id == $data[0]->purchase_party_id)
                                  {
                    ...
                    <?php
                    $reference_id_arr     = explode("," , $data[0]->purchase_id);
                    $reference_number_arr = explode("," , $data[0]->purchase_invoice_number);
                    $payment_amount_arr   = explode("," , "0");
                    $paid_amount_arr      = explode("," , precise_amount($data[0]->purchase_paid_amount));
                    /*$balance_amount_arr   = explode("," , precise_amount(($data[0]->purchase_grand_total + $data[0]->credit_note_amount - $data[0]->debit_note_amount - $data[0]->purchase_paid_amount)));*/
                    $balance_amount_arr   = explode("," , precise_amount(($data[0]->purchase_grand_total - $data[0]->purchase_paid_amount)));
                    $invoice_total_arr    = explode("," , precise_amount(($data[0]->purchase_grand_total + $data[0]->credit_note_amount - $data[0]->debit_note_amount)));
                    ...
                    <div class="row row_count main_row" row="1" id="ToClone">
                      <div class="col-sm-2 reference_number_div" style="width: 20%">
                        <div class="form-group">
                          ...
                          <span>Invoice Amount: <strong class="total_invoice_amount"><?= precise_amount($data[0]->purchase_grand_total) ?></strong>/-</span>
              ...
              <input type="hidden" class="form-control" id="reference_type" name="reference_type" value="purchase" >
              <input type="hidden" class="form-control" id="redirect" name="redirect" value="purchase" >
              
              <input type="hidden" name="invoice_data" id="invoice_data">
              <input type="hidden" name="excess_purchase_id" id="excess_purchase_id">
              <!-- Hidden Field -->
              <div class="box-footer">
                <button type="submit" id="payment_submit" class="btn btn-info">Pay</button>
                <span class="btn btn-default" id="payment_cancel" onclick="cancel('purchase')">Cancel</span>
              </div>
              <?php
              $notes_sub_module = 0;
