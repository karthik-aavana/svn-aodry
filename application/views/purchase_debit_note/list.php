                ...
                <div class="box">
                    <div id="plus_btn">
                        <div class="box-header with-border">
                            <h3 class="box-title">Purchase debit Note</h3>
                            <?php
                            if (in_array($purchase_debit_note_module_id, $active_add)) {
                                ?>
                                <a class="btn btn-sm btn-info pull-right" href="<?php echo base_url('purchase_debit_note/add'); ?>">Add Purchase debit Note </a>
                            <?php } ?>
                        </div>
                    </div>                    
                    <div id="filter">
                        ...
                        <table id="list_datatable" class="custom_datatable table table-bordered table-striped table-hover table-responsive" >
                         <thead>
                                <tr>
                                    ...
                                    <th>Debit Note Reference Number</th>
                                    <th>Supplier Name</th>
                                    <!-- <th>Supplier debit Note No</th> -->
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="form-group">
                                ...
                                <input type="hidden" name="type" id="type" value="purchase_debit_note">
...
// $this->load->view('purchase_debit_note/pay_now_modal');
// $this->load->view('purchase_debit_note/pdf_type_modal');
...
<script>
    $(document).ready(function () {
        $('#list_datatable').DataTable({
            ...
            "responsive": true,
            "ajax": {
                "url": base_url + "purchase_debit_note",
                "dataType": "json",
                "type": "POST",
        ...
        <?php 
        $purchase_dn_success = $this->session->flashdata('purchase_dn_success');
        $purchase_dn_error = $this->session->flashdata('purchase_dn_error');
        ?>
        var alert_success = '<?= $purchase_dn_success; ?>';
        var alert_failure = '<?= $purchase_dn_error; ?>';
        if(alert_success != ''){
