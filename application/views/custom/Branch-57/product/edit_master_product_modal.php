    ...
    <div class="modal-dialog modal-md" style="width: 35%">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button id="edit_master_product" class="btn btn-info" >Add</button>
...
var name_regex1 = /^[a-zA-Z\[\]/@()#$%&\-.+,\d\-_\s\']+$/;
     $("#edit_master_product").click(function (event){
        ...
        $.ajax({url: base_url + 'product/add_product_master',
                ...
                success: function (result){
                    if(result == 'duplicate'){
                        ...
                        $('#product_name_edit').html('');
                        $('#product_name_edit').append('<option value="">Select</option>');
                        $('#master_product_name').val('');
                        for (i = 0; i < data.length; i++){
                            if(result.id == data[i].id){
                                $('#product_name_edit').append('<option value="' + data[i].master_product_name + '" selected>' + data[i].master_product_name + '</option>')
                            }else{
                             $('#product_name_edit').append('<option value="' + data[i].master_product_name + '">' + data[i].master_product_name + '</option>')   
                            }
