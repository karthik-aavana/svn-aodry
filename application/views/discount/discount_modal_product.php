    ...
    $(document).ready(function () {
        ...
        $("#modal_discount_submit").click(function (event) {
            ...
            $.ajax({
                   ...
                   $("#add_discount_product #loader").show();
               },
                    success: function (result) {
                        ...
                        if(result.msg == "duplicate"){                            
                            ...
                            $('#product_discount,#markdown_discount').html('');
                            $('#product_discount,#markdown_discount').append('<option value="0">Select Discount</option>');
                            for (i = 0; i < data.length; i++){
                                $('#product_discount,#markdown_discount').append('<option value="' + data[i].discount_id+ '">' + parseFloat(data[i].discount_value) + '%</option>')
                            }
                            var tax_value = parseFloat(result.discount_value).toFixed(2);
                            $('#product_discount,#markdown_discount').val(result['id']).attr("selected", "selected");
            ...
            anime.timeline({ loop: !0 }).add({ targets: ".ml8 .circle-white", scale: [0, 3], opacity: [1, 0], easing: "easeInOutExpo", rotateZ: 360, duration: 5e3 }).anime({ targets: ".ml8 .circle-dark-dashed", rotateZ: 360, duration: 8e3, easing: "linear", loop: !0 });
        });
    });
