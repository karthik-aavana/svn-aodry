...
class Journal_ledger extends MY_Controller
{
   ...
   function __construct()
    ...
    function index(){
        ...
        $this->load->view('general_voucher/journal_voucher_list', $data);
    }
    ...
    function GetAllVoucher(){
        ...
        $sql = "SELECT journal_voucher_id FROM `tbl_journal_voucher` ";
        if($voucher_id != 0 && $voucher_id != ''){
            $sql .= " WHERE journal_voucher_id='{$voucher_id}' ";
        }else{
        ...
        if(!empty($sub_ary)){
            ...
            $extraWhere = " AND journal_voucher_id IN (SELECT journal_voucher_id FROM accounts_journal_voucher WHERE {$where})";
        }
        $sql .= $extraWhere;
        ...
        $sql .= " ORDER BY journal_voucher_id DESC LIMIT $end,$limit"; 
        ...
        if($qry->num_rows() > 0){
            ...
            foreach ($vids as $key => $value) {
                array_push($vids_ary, $value['journal_voucher_id']);               
            }
        }
