...
class Purchase_debit_note extends MY_Controller {
    ...
    public function index() {
        ...
        $data['purchase_debit_note_module_id'] = $purchase_debit_note_module_id;
        $modules = $this->modules;
        $privilege = "view_privilege";
        $data['privilege'] = $privilege;
        $section_modules = $this->get_section_modules($purchase_debit_note_module_id, $modules, $privilege);
        ...
        if (!empty($this->input->post())) {
            $columns = array(
                ...
                1 => 'supplier',
                2 => 'grand_total',
                3 => 'purchase_supplier_debit_note_number',
                4 => 'invoice_status',
                5 => 'added_user',
                6 => 'action');
            ...
            if (!empty($posts)) {
                ...
                foreach ($posts as $post) {
                    ...
                    /* $nestedData['supplier']    = $post->supplier_name . ' (<a href="' . base_url('purchase_debit_note/view/') . $purchase_debit_note_id . '">' . $post->purchase_debit_note_invoice_number . '</a>) '; */
                    $nestedData['supplier'] = $post->supplier_name . ($post->purchase_supplier_debit_note_number != '' ? ' ( ' . $post->purchase_supplier_debit_note_number . ' ) ' : '');
                    $nestedData['invoice_number'] = ' <a href="' . base_url('purchase_debit_note/view/') . $purchase_debit_note_id . '">' . $post->purchase_debit_note_invoice_number . '</a>';
                    ...
                    $nestedData['grand_total'] = $currency_symbol . ' ' . $this->precise_amount($post->purchase_debit_note_grand_total, $access_common_settings[0]->amount_precision) . ' (INV)';

                    $nestedData['debit_note_number'] = $post->purchase_supplier_debit_note_number;
                    ...
                    $this->db->where('reference_id',$post->purchase_debit_note_id);
                    $this->db->where('delete_status',0);
                    $this->db->where('reference_type','purchase_debit_note');
                    $get_pv_qry = $this->db->get();
                    $ref_id = $get_pv_qry->result();
                    $purchase_dn_voucher_id = '';
                    if(!empty($ref_id)){
                        $purchase_dn_voucher_id = $ref_id[0]->purchase_voucher_id;
                    }
                    /*if($purchase_dn_voucher_id != ''){
                        $purchase_dn_voucher_id = $this->encryption_url->encode($purchase_dn_voucher_id);
                        $nestedData['purchase_dn_voucher_view'] = ' <a href="' .base_url('purchase_voucher/view_details/') . $purchase_dn_voucher_id.'" target="_blank">' . '<i class="fa fa-file" aria-hidden="true" title="Voucher View"></i>' . '</a>'. '  ' .' <form  action="' .base_url('purchase_debit_ledger').'" method="POST" target="_blank"><input type="hidden" name="reference_id" value="'.$purchase_dn_voucher_id.'"><button type="submit">' . '<i class="fa fa-file" aria-hidden="true" title="Ledger View"></i></button></form>';
                    }*/
                    ...
                    {
                        $cols .= '<span><a class="btn btn-app" data-toggle="tooltip" data-placement="bottom" href="' . base_url('purchase_debit_note/view/') . $purchase_debit_note_id . '" title="View Purchase Debit Note"><i class="fa fa-eye"></i></a></span>';
                        $cols .= '<span><a class="btn btn-app" data-toggle="tooltip" data-placement="bottom" href="' . base_url('purchase_debit_note/pdf/') . $purchase_debit_note_id . '" title="Download PDF" target="_blank"><i class="fa fa-file-pdf-o"></i></a></span>';
                    }
                    if (in_array($purchase_debit_note_module_id , $data['active_edit']))
                    {
                        $cols .= '<span><a class="btn btn-app" data-toggle="tooltip" data-placement="bottom" href="' . base_url('purchase_debit_note/edit/') . $purchase_debit_note_id . '" title="Edit Purchase Debit Note"><i class="fa fa-pencil"></i></a></span>';
                    }
                    ...
                    /* if (in_array($purchase_debit_note_module_id, $data['active_view']))
                      ...
                      {
                      $cols .= '<span><a class="btn btn-app" data-toggle="tooltip" data-placement="bottom" href="' . base_url('purchase_debit_note/email/') . $purchase_debit_note_id . '" title="Email purchase debit note"><i class="fa fa-envelope-o"></i></a></span>';
                      }
                      } */
                    /* if ($post->currency_id != $this->session->userdata('SESS_DEFAULT_CURRENCY'))
                      {
                      $cols .= '<span data-backdrop="static" data-keyboard="false" class="convert_currency" data-toggle="modal" data-target="#convert_currency_modal" ><a class="btn btn-app" data-toggle="tooltip" data-placement="bottom" data-id="' . $purchase_debit_note_id . '" data-path="purchase_debit_note/convert_currency" data-currency_code="' . $currency_code . '" data-grand_total="' . $post->purchase_debit_note_grand_total . '" href="#" title="Convert Currency" ><i class="fa fa-exchange"></i></a></span>';
                      } */
                      if($purchase_dn_voucher_id != ''){
                        $purchase_dn_voucher_id = $this->encryption_url->encode($purchase_dn_voucher_id);
                       
                       /* $nestedData['purchase_dn_voucher_view'] = ' <a href="' .base_url('purchase_voucher/view_details/') . $purchase_dn_voucher_id.'" target="_blank">' . '<i class="fa fa-file" aria-hidden="true" title="Voucher View"></i>' . '</a>'. '  ' .' <form  action="' .base_url('purchase_debit_ledger').'" method="POST" target="_blank"><input type="hidden" name="reference_id" value="'.$purchase_dn_voucher_id.'"><button type="submit">' . '<i class="fa fa-file" aria-hidden="true" title="Ledger View"></i></button></form>';*/
                        if (in_array($purchase_debit_note_voucher_id , $data['active_view']))
                        {
                            $cols .= '<span><a href="' .base_url('purchase_voucher/view_details/') . $purchase_dn_voucher_id.'" target="_blank" class="btn btn-app" data-toggle="tooltip" data-placement="bottom" title="View Voucher"><i class="fa fa-eye"></i></a></span>';

                            $cols .= '<span><form  action="' .base_url('purchase_debit_ledger').'" method="POST" target="_blank"><input type="hidden" name="reference_id" value="'.$purchase_dn_voucher_id.'"><a href="javascript:void(0)" data-toggle="tooltip" data-placement="bottom" class="btn btn-app" title="View Ledger"><button type="submit" class="sales_action">' . '<i class="fa fa-eye" aria-hidden="true"></i></button></a></form></span>';
                        }
                    }
                    if (in_array($purchase_debit_note_module_id, $data['active_delete'])) {
                        $cols .= '<span data-backdrop="static" data-keyboard="false" class="delete_button" data-toggle="modal" data-target="#delete_modal" data-id="' . $purchase_debit_note_id . '" data-path="purchase_debit_note/delete" data-delete_message="If you delete this record then its assiociated records also will be delete!! Do you want to continue?"><a class="btn btn-app" data-toggle="tooltip" data-placement="bottom" href="#" title="Delete Purchase Debit Note" ><i class="fa fa-trash-o"></i></a></span>';
                    }
                    $cols .= '</div></div>';
            ...
            $this->load->view('purchase_debit_note/list', $data);
        }
    }
    ...
    public function add() {
        ...
        /* Modules Present */
        $data['purchase_debit_note_module_id'] = $purchase_debit_note_module_id;
        $data['module_id'] = $purchase_debit_note_module_id;
        ...
        $table_name = $this->config->item('purchase_debit_note_table');
        ...
        $this->load->view('purchase_debit_note/add', $data);
    }
    ...
    public function get_purchase_debit_note_suggestions($term, $inventory_advanced, $item_access) {
        $purchase_debit_note_module_id = $this->config->item('purchase_debit_note_module');
        $modules = $this->modules;
        $privilege = "add_privilege";
        $data['privilege'] = $privilege;
    ...
    public function add_purchase_debit_note() {
        /* echo "<per>";
          print_r($this->input->post());
          exit(); */
        $data = $this->get_default_country_state();
        $purchase_debit_note_module_id = $this->config->item('purchase_debit_note_module');
        $module_id = $purchase_debit_note_module_id;
        $modules = $this->modules;
        $privilege = "add_privilege";
        $section_modules = $this->get_section_modules($purchase_debit_note_module_id, $modules, $privilege);
        /* presents all the needed */
        ...
        $purchase_debit_note_data = array(
            "purchase_debit_note_date" => date('Y-m-d', strtotime($this->input->post('invoice_date'))),
            "purchase_id" => $this->input->post('purchase_invoice_number'),
            "purchase_debit_note_invoice_number" => $invoice_number,
            "purchase_debit_note_sub_total" => (float) $this->input->post('total_sub_total') ? (float) $this->input->post('total_sub_total') : 0,
            "purchase_supplier_debit_note_date" => date('Y-m-d', strtotime($this->input->post('purchase_supplier_debit_note_date'))),
            "purchase_supplier_debit_note_number" => $this->input->post('supplier_debit_note_no'),
            "purchase_debit_note_grand_total" => $this->input->post('total_grand_total') ? (float) $this->input->post('total_grand_total') : 0,
            "purchase_debit_note_discount_amount" => $this->input->post('total_discount_amount') ? (float) $this->input->post('total_discount_amount') : 0,
            "purchase_debit_note_tax_amount" => $this->input->post('total_tax_amount') ? (float) $this->input->post('total_tax_amount') : 0,
            "purchase_debit_note_taxable_value" => $this->input->post('total_taxable_amount') ? (float) $this->input->post('total_taxable_amount') : 0,
            "purchase_debit_note_gst_payable" => $this->input->post('gst_payable'),
            "purchase_debit_note_tds_amount" => $this->input->post('total_tds_amount') ? (float) $this->input->post('total_tds_amount') : 0,
            "purchase_debit_note_tcs_amount" => $this->input->post('total_tcs_amount') ? (float) $this->input->post('total_tcs_amount') : 0,
            "purchase_debit_note_igst_amount" => 0,
            "purchase_debit_note_cgst_amount" => 0,
            "purchase_debit_note_sgst_amount" => 0,
            "from_account" => 'purchase',
            "to_account" => 'supplier',
            "financial_year_id" => $this->session->userdata('SESS_FINANCIAL_YEAR_ID'),
            "purchase_debit_note_party_id" => $this->input->post('supplier'),
            "purchase_debit_note_party_type" => "supplier",
            "purchase_debit_note_nature_of_supply" => $this->input->post('nature_of_supply'),
            "purchase_debit_note_supplier_invoice_number" => $this->input->post('supplier_ref'),
            "purchase_debit_note_supplier_date" => $this->input->post('supplier_date'),
            "purchase_debit_note_delivery_challan_number" => $this->input->post('delivery_challan_number'),
            "purchase_debit_note_delivery_date" => $this->input->post('delivery_date'),
            "purchase_debit_note_e_way_bill_number" => $this->input->post('e_way_bill'),
            "purchase_debit_note_type_of_supply" => $this->input->post('type_of_supply'),
            "purchase_debit_note_billing_country_id" => $this->input->post('billing_country'),
            "purchase_debit_note_billing_state_id" => $this->input->post('billing_state'),
        ...
        $purchase_debit_note_data['freight_charge_tax_id'] = $this->input->post('freight_charge_tax_id') ? (float) $this->input->post('freight_charge_tax_id') : 0;
        $purchase_debit_note_data['insurance_charge_tax_id'] = $this->input->post('insurance_charge_tax_id') ? (float) $this->input->post('insurance_charge_tax_id') : 0;
        $purchase_debit_note_data['packing_charge_tax_id'] = $this->input->post('packing_charge_tax_id') ? (float) $this->input->post('packing_charge_tax_id') : 0;
        $purchase_debit_note_data['incidental_charge_tax_id'] = $this->input->post('incidental_charge_tax_id') ? (float) $this->input->post('incidental_charge_tax_id') : 0;
        $purchase_debit_note_data['inclusion_other_charge_tax_id'] = $this->input->post('inclusion_other_charge_tax_id') ? (float) $this->input->post('inclusion_other_charge_tax_id') : 0;
        $purchase_debit_note_data['exclusion_other_charge_tax_id'] = $this->input->post('exclusion_other_charge_tax_id') ? (float) $this->input->post('exclusion_other_charge_tax_id') : 0;

        $round_off_value = $purchase_debit_note_data['purchase_debit_note_grand_total'];
        ...
        $purchase_debit_note_data['round_off_amount'] = bcsub($purchase_debit_note_data['purchase_debit_note_grand_total'], $round_off_value, $section_modules['access_common_settings'][0]->amount_precision);

        $purchase_debit_note_data['purchase_debit_note_grand_total'] = $round_off_value;

        $purchase_debit_note_tax_amount = $purchase_debit_note_data['purchase_debit_note_tax_amount'] + (float) ($this->input->post('total_other_taxable_amount'));
        ...
        if ($section_modules['access_settings'][0]->tax_type == "gst") {
            ...
            if ($purchase_debit_note_data['purchase_debit_note_type_of_supply'] == 'intra_state') {
                $purchase_debit_note_data['purchase_debit_note_igst_amount'] = 0;
                $purchase_debit_note_data['purchase_debit_note_cgst_amount'] = ($purchase_debit_note_tax_amount * $cgst_amount_percentage) / 100;
                $purchase_debit_note_data['purchase_debit_note_sgst_amount'] = ($purchase_debit_note_tax_amount * $sgst_amount_percentage) / 100;
                $purchase_debit_note_data['purchase_debit_note_tax_cess_amount'] = $total_cess_amnt;
            } elseif ($purchase_debit_note_data['purchase_debit_note_type_of_supply'] == 'inter_state') {
                $purchase_debit_note_data['purchase_debit_note_igst_amount'] = $purchase_debit_note_tax_amount;
                $purchase_debit_note_data['purchase_debit_note_cgst_amount'] = 0;
                $purchase_debit_note_data['purchase_debit_note_sgst_amount'] = 0;
                $purchase_debit_note_data['purchase_debit_note_tax_cess_amount'] = $total_cess_amnt;
            } elseif ($purchase_debit_note_data['purchase_debit_note_type_of_supply'] == 'import') {
                $purchase_debit_note_data['purchase_debit_note_igst_amount'] = $purchase_debit_note_tax_amount;
                $purchase_debit_note_data['purchase_debit_note_cgst_amount'] = 0;
                $purchase_debit_note_data['purchase_debit_note_sgst_amount'] = 0;
                $purchase_debit_note_data['purchase_debit_note_tax_cess_amount'] = $total_cess_amnt;
        ...
        if ($this->session->userdata('SESS_DEFAULT_CURRENCY') == $this->input->post('currency_id')) {
            $purchase_debit_note_data['converted_grand_total'] = $purchase_debit_note_data['purchase_debit_note_grand_total'];
        } else {
            $purchase_debit_note_data['converted_grand_total'] = 0;
        }

        $data_main = array_map('trim', $purchase_debit_note_data);
        $purchase_debit_note_table = $this->config->item('purchase_debit_note_table');
        /* echo "<pre>";
          print_r($purchase_debit_note_data);exit();
         */
        if ($purchase_debit_note_id = $this->general_model->insertData($purchase_debit_note_table, $data_main)) {

            /* if ($this->session->userdata('SESS_DEFAULT_CURRENCY') == $data_main['currency_id']) { */

            $purchase_id = $data_main['purchase_id'];
            $old_amount = $this->general_model->getRecords("debit_note_amount,converted_debit_note_amount,supplier_payable_amount", "purchase", array('purchase_id' => $purchase_id));

            $new_amount = bcadd($old_amount[0]->debit_note_amount, $data_main["purchase_debit_note_grand_total"], $section_modules['access_common_settings'][0]->amount_precision);
            $cust_new_amount = bcsub($old_amount[0]->supplier_payable_amount, ($data_main["purchase_debit_note_grand_total"] - $data_main["purchase_debit_note_tds_amount"]), $section_modules['access_common_settings'][0]->amount_precision);
            ...
            $this->general_model->updateData("purchase", array(
                ...
                'purchase_id' => $purchase_id));
            /* } */
            $successMsg = 'Purchase Debit Note Added Successfully';
            $this->session->set_flashdata('purchase_dn_success',$successMsg);
            $log_data = array(
                'user_id' => $this->session->userdata('SESS_USER_ID'),
                'table_id' => $purchase_debit_note_id,
                'table_name' => $purchase_debit_note_table,
                'financial_year_id' => $this->session->userdata('SESS_FINANCIAL_YEAR_ID'),
                'branch_id' => $this->session->userdata('SESS_BRANCH_ID'),
                'message' => 'Purchase debit Note Inserted');
            $data_main['purchase_debit_note_id'] = $purchase_debit_note_id;
            $log_table = $this->config->item('log_table');
            $this->general_model->insertData($log_table, $log_data);
            $purchase_debit_note_item_data = $this->input->post('table_data');
            $js_data = json_decode($purchase_debit_note_item_data);
            $js_data               = array_reverse($js_data);
            $item_table = $this->config->item('purchase_debit_note_item_table');
            ...
            if (!empty($js_data)) {
                ...
                foreach ($js_data as $key => $value) {
                    ...
                    if ($value == null || ($value->item_grand_total <= 0 && $value->item_quantity <= 0)) {
                        ...
                        $item_data = array(
                            "item_id" => $value->item_id,
                            "item_type" => $value->item_type,
                            "purchase_debit_note_item_quantity" => $value->item_quantity ? (float) $value->item_quantity : 0,
                            "purchase_debit_note_item_unit_price" => (@$value->item_price ? (float) $value->item_price : 0),
                            "purchase_debit_note_item_sub_total" => (@$value->item_sub_total ? (float) $value->item_sub_total : 0),
                            "purchase_debit_note_item_taxable_value" => (@$value->item_taxable_value ? (float) $value->item_taxable_value : 0),
                            "purchase_debit_note_item_discount_amount" => (@$value->item_discount_amount ? (float) $value->item_discount_amount : 0),
                            "purchase_debit_note_item_discount_id" => (@$value->item_discount_id ? (float) $value->item_discount_id : 0),
                            "purchase_debit_note_item_tds_id" => (@$value->item_tds_id ? (float) $value->item_tds_id : 0),
                            "purchase_debit_note_item_tds_percentage" => (@$value->item_tds_percentage ? (float) $value->item_tds_percentage : 0),
                            "purchase_debit_note_item_tds_amount" => (@$value->item_tds_amount ? (float) $value->item_tds_amount : 0),
                            "purchase_debit_note_item_grand_total" => (@$value->item_grand_total ? (float) $value->item_grand_total : 0),
                            "purchase_debit_note_item_tax_id" => (@$value->item_tax_id ? (float) $value->item_tax_id : 0),
                            "purchase_debit_note_item_tax_cess_id" => (@$value->item_tax_cess_id ? (float) $value->item_tax_cess_id : 0),
                            "purchase_debit_note_item_igst_percentage" => 0,
                            "purchase_debit_note_item_igst_amount" => 0,
                            "purchase_debit_note_item_cgst_percentage" => 0,
                            "purchase_debit_note_item_cgst_amount" => 0,
                            "purchase_debit_note_item_sgst_percentage" => 0,
                            "purchase_debit_note_item_sgst_amount" => 0,
                            "purchase_debit_note_item_tax_percentage" => (@$value->item_tax_percentage ? (float) $value->item_tax_percentage : 0),
                            "purchase_debit_note_item_tax_amount" => (@$value->item_tax_amount ? (float) $value->item_tax_amount : 0),
                            "purchase_debit_note_item_tax_cess_percentage" => 0,
                            "purchase_debit_note_item_tax_cess_amount" => 0,
                            "purchase_debit_note_item_description" => (@$value->item_description ? $value->item_description : ""),
                            "debit_note_quantity" => 0,
                            "purchase_debit_note_id" => $purchase_debit_note_id);

                        $purchase_debit_note_item_tax_amount = $item_data['purchase_debit_note_item_tax_amount'];
                        $purchase_debit_note_item_tax_percentage = $item_data['purchase_debit_note_item_tax_percentage'];
                        ...
                        if ($section_modules['access_settings'][0]->tax_type == "gst") {
                            ...
                            if ($data['branch'][0]->branch_country_id == $purchase_debit_note_data['purchase_debit_note_billing_country_id']) {

                                if ($data['branch'][0]->branch_state_id == $purchase_debit_note_data['purchase_debit_note_billing_state_id']) {
                                    $item_data['purchase_debit_note_item_igst_amount'] = 0;
                                    $item_data['purchase_debit_note_item_cgst_amount'] = ($purchase_debit_note_item_tax_amount * $cgst_amount_percentage) / 100;
                                    $item_data['purchase_debit_note_item_sgst_amount'] = ($purchase_debit_note_item_tax_amount * $sgst_amount_percentage) / 100;
                                    $item_data['purchase_debit_note_item_tax_cess_amount'] = $item_tax_cess_amount;
                                    $item_data['purchase_debit_note_item_tax_cess_percentage'] = $item_tax_cess_percentage;
                                    $item_data['purchase_debit_note_item_igst_percentage'] = 0;
                                    $item_data['purchase_debit_note_item_cgst_percentage'] = ($purchase_debit_note_item_tax_percentage * $cgst_amount_percentage) / 100;
                                    $item_data['purchase_debit_note_item_sgst_percentage'] = ($purchase_debit_note_item_tax_percentage * $sgst_amount_percentage) / 100;
                                } else {
                                    $item_data['purchase_debit_note_item_igst_amount'] = $purchase_debit_note_item_tax_amount;
                                    $item_data['purchase_debit_note_item_cgst_amount'] = 0;
                                    $item_data['purchase_debit_note_item_sgst_amount'] = 0;
                                    $item_data['purchase_debit_note_item_tax_cess_amount'] = $item_tax_cess_amount;
                                    $item_data['purchase_debit_note_item_tax_cess_percentage'] = $item_tax_cess_percentage;
                                    $item_data['purchase_debit_note_item_igst_percentage'] = $purchase_debit_note_item_tax_percentage;
                                    $item_data['purchase_debit_note_item_cgst_percentage'] = 0;
                                    $item_data['purchase_debit_note_item_sgst_percentage'] = 0;
                                }
                            } else {

                                if ($purchase_debit_note_data['purchase_debit_note_type_of_supply'] == "export_with_payment") {
                                    $item_data['purchase_debit_note_item_igst_amount'] = $purchase_debit_note_item_tax_amount;
                                    $item_data['purchase_debit_note_item_cgst_amount'] = 0;
                                    $item_data['purchase_debit_note_item_sgst_amount'] = 0;
                        ...
                        if (($data_item['item_type'] == "product" || $data_item['item_type'] == "product_inventory") && $value->item_quantity > 0) {
                            ...
                            $history = array(
                                "item_id" => $value->item_id,
                                "item_type" => 'product',
                                "reference_id" => $purchase_debit_note_id,
                ...
                if (in_array($data['accounts_module_id'], $section_modules['active_add'])) {
                    ...
                    if (in_array($data['accounts_sub_module_id'], $section_modules['access_sub_modules'])) {
                        $action = "add";
                        $this->purchase_debit_note_voucher_entry($data_main, $js_data1, $action, $data['branch']);
                    }
                }
            }
        } else {
            $errorMsg = 'Purchase Debit Note Add Unsuccessful';
            $this->session->set_flashdata('purchase_dn_error',$errorMsg);
            redirect('purchase_debit_note', 'refresh');
        }
    ...
    public function purchase_debit_note_vouchers($section_modules, $data_main, $js_data, $branch) {
        ...
        if ((($data_main['purchase_debit_note_tax_amount'] > 0 && ($data_main['purchase_debit_note_igst_amount'] > 0 || $data_main['purchase_debit_note_cgst_amount'] > 0 || $data_main['purchase_debit_note_sgst_amount'] > 0 || $data_main['purchase_debit_note_tax_cess_amount'] > 0)) || $data_main['purchase_debit_note_tax_cess_amount'] > 0)) {
            $present = "gst";
            if ($data_main['purchase_debit_note_type_of_supply'] != 'import') {
                if ($data_main['purchase_debit_note_type_of_supply'] == 'intra_state') {
                    $present = "no_igst";
                } else {
                    $present = "igst";
                }
            }
            if ($data_main['purchase_debit_note_gst_payable'] != "yes" && $present != "gst") {

                foreach ($js_data as $key => $value) {
                    if ($present != "igst") {
                        if ($value['purchase_debit_note_item_cgst_percentage'] > 0 || $value['purchase_debit_note_item_sgst_percentage'] > 0) {

                            $cgst_ary = array(
                                            'ledger_name' => 'Input CGST@'.$value['purchase_debit_note_item_cgst_percentage'].'%',
                                            'second_grp' => 'CGST',
                                            'primary_grp' => 'Duties and taxes',
                                            'main_grp' => 'Current Assets',
                                            'default_ledger_id' => 0,
                                            'default_value' => $value['purchase_debit_note_item_cgst_percentage'],
                                            'amount' => 0
                                        );
                            if(!empty($cgst_x)){
                                $cgst_ledger = $cgst_x->ledger_name;
                                $cgst_ledger = str_ireplace('{{X}}',$value['purchase_debit_note_item_cgst_percentage'] , $cgst_ledger);
                            ...
                            /*$cgst_tax_ledger = $this->ledger_model->addGroupLedger(array(
                                'ledger_name' => 'CGST@' . $value['purchase_debit_note_item_cgst_percentage'] . '%',
                                'subgrp_1' => 'CGST',
                            ...
                            $gst_lbl = 'SGST';
                            $is_utgst = $this->general_model->checkIsUtgst($data_main['purchase_debit_note_billing_state_id']);
                            if ($is_utgst == '1')
                                $gst_lbl = 'UTGST';
                            $sgst_ary = array(
                                            'ledger_name' => 'Input '.$gst_lbl.'@'.$value['purchase_debit_note_item_sgst_percentage'].'%',
                                            'second_grp' => $gst_lbl,
                                            'primary_grp' => 'Duties and taxes',
                                            'main_grp' => 'Current Assets',
                                            'default_ledger_id' => 0,
                                            'default_value' => $value['purchase_debit_note_item_sgst_percentage'],
                                            'amount' => 0
                                        );
                            if(!empty($sgst_x)){
                                if($is_utgst == '1') {
                                    $sgst_ledger = $utgst_x->ledger_name;
                                    $sgst_ledger = str_ireplace('{{X}}',$value['purchase_debit_note_item_sgst_percentage'] , $sgst_ledger);
                            ...
                            /*$sgst_tax_ledger = $this->ledger_model->addGroupLedger(array(
                                'ledger_name' => $gst_lbl . '@' . $value['purchase_debit_note_item_sgst_percentage'] . '%',
                                'subgrp_1' => $gst_lbl,
                                'subgrp_2' => 'Duties and taxes',
                                'main_grp' => 'Current Liabilities',
                                'amount' => 0
                            ...
                            if (in_array($cgst_tax_ledger, $cgst_slab)) {
                                $cgst_slab_items[$cgst_tax_ledger] = bcadd($cgst_slab_items[$cgst_tax_ledger], $value['purchase_debit_note_item_cgst_amount'], $section_modules['access_common_settings'][0]->amount_precision);
                            } else {
                                $cgst_slab[] = $cgst_tax_ledger;
                                $cgst_slab_items[$cgst_tax_ledger] = $value['purchase_debit_note_item_cgst_amount'];
                            }

                            if (in_array($sgst_tax_ledger, $sgst_slab)) {
                                $sgst_slab_items[$sgst_tax_ledger] = bcadd($sgst_slab_items[$sgst_tax_ledger], $value['purchase_debit_note_item_sgst_amount'], $section_modules['access_common_settings'][0]->amount_precision);
                            } else {
                                $sgst_slab[] = $sgst_tax_ledger;
                                $sgst_slab_items[$sgst_tax_ledger] = $value['purchase_debit_note_item_sgst_amount'];
                            }
                        }
                    } else {
                        if ($value['purchase_debit_note_item_igst_percentage'] > 0) {
                            $igst_ary = array(
                                            'ledger_name' => 'Input IGST@'.$value['purchase_debit_note_item_igst_percentage'].'%',
                                            'second_grp' => 'IGST',
                                            'primary_grp' => 'Duties and taxes',
                                            'main_grp' => 'Current Assets',
                                            'default_ledger_id' => 0,
                                            'default_value' => $value['purchase_debit_note_item_igst_percentage'],
                                            'amount' => 0
                                        );
                            if(!empty($igst_x)){
                                $igst_ledger = $igst_x->ledger_name;
                                $igst_ledger = str_ireplace('{{X}}',$value['purchase_debit_note_item_igst_percentage'] , $igst_ledger);
                            ...
                            /*$igst_tax_ledger = $this->ledger_model->addGroupLedger(array(
                                'ledger_name' => 'IGST@' . $value['purchase_debit_note_item_igst_percentage'] . '%',
                                'subgrp_1' => 'IGST',
                                'subgrp_2' => 'Duties and taxes',
                                'main_grp' => 'Current Liabilities',
                                'amount' => 0
                            ...
                            if (in_array($igst_tax_ledger, $igst_slab)) {
                                $igst_slab_items[$igst_tax_ledger] = bcadd($igst_slab_items[$igst_tax_ledger], $value['purchase_debit_note_item_igst_amount'], $section_modules['access_common_settings'][0]->amount_precision);
                            } else {
                                $igst_slab[] = $igst_tax_ledger;
                                $igst_slab_items[$igst_tax_ledger] = $value['purchase_debit_note_item_igst_amount'];
                            }
                        }
                    }
                    if ($value['purchase_debit_note_item_tax_cess_percentage'] > 0) {
                        $default_cess_id = $purchase_ledger['CESS@X'];
                        $cess_ledger_name = $this->ledger_model->getDefaultLedgerId($default_cess_id);
                       
                        $cess_ary = array(
                                        'ledger_name' => 'Input Compensation Cess @'.$value['purchase_debit_note_item_tax_cess_percentage'].'%',
                                        'second_grp' => 'Cess',
                                        'primary_grp' => 'Duties and taxes',
                                        'main_grp' => 'Current Assets',
                                        'default_ledger_id' => 0,
                                        'default_value' => $value['purchase_debit_note_item_tax_cess_percentage'],
                                        'amount' => 0
                                    );
                        if(!empty($cess_ledger_name)){
                            $cess_ledger = $cess_ledger_name->ledger_name;
                            $cess_ledger = str_ireplace('{{X}}',$value['purchase_debit_note_item_tax_cess_percentage'] , $cess_ledger);
                        ...
                        /*$cess_tax_ledger = $this->ledger_model->addGroupLedger(array(
                            'ledger_name' => 'Compensation Cess @' . $value['purchase_debit_note_item_tax_cess_percentage'] . '%',
                            'subgrp_1' => 'Cess',
                            'subgrp_2' => 'Duties and taxes',
                            'main_grp' => 'Current Liabilities',
                            'amount' => 0
                        ...
                        if (in_array($cess_tax_ledger, $cess_slab)) {
                            $cess_slab_items[$cess_tax_ledger] = bcadd($cess_slab_items[$cess_tax_ledger], $value['purchase_debit_note_item_tax_cess_amount'], $section_modules['access_common_settings'][0]->amount_precision);
                        } else {
                            $cess_slab[] = $cess_tax_ledger;
                            $cess_slab_items[$cess_tax_ledger] = $value['purchase_debit_note_item_tax_cess_amount'];
                        }
                    }
                }
              
            } elseif ($data_main['purchase_debit_note_gst_payable'] == "yes" && $present != "gst") {
                    ...
                    if ($present != "igst") {
                        ...
                        if ($value['purchase_debit_note_item_cgst_percentage'] > 0 || $value['purchase_debit_note_item_sgst_percentage'] > 0) {
                            ...
                            $cgst_ary = array(
                                            ...
                                            'main_grp' => 'Current Assets',
                                            'default_ledger_id' => 0,
                                            'default_value' => $value['purchase_debit_note_item_cgst_amount'],
                                            'amount' => 0
                                        );
                            if(!empty($cgst_ledger_name)){
                                $cgst_ledger = $cgst_ledger_name->ledger_name;
                                $cgst_ledger = str_ireplace('{{X}}',$value['purchase_debit_note_item_cgst_amount'] , $cgst_ledger);
                                $cgst_ary['ledger_name'] = $cgst_ledger;
                                $cgst_ary['primary_grp'] = $cgst_ledger_name->sub_group_1;
                                $cgst_ary['second_grp'] = $cgst_ledger_name->sub_group_2;
                                $cgst_ary['main_grp'] = $cgst_ledger_name->main_group;
                            ...
                            $sgst_ary = array(
                                            ...
                                            'main_grp' => 'Current Assets',
                                            'default_ledger_id' => 0,
                                            'default_value' => $value['purchase_debit_note_item_sgst_amount'],
                                            'amount' => 0
                                        );
                            if(!empty($sgst_ledger_name)){
                                $sgst_ledger = $sgst_ledger_name->ledger_name;
                                $sgst_ledger = str_ireplace('{{X}}',$value['purchase_debit_note_item_sgst_amount'] , $sgst_ledger);
                                $sgst_ary['ledger_name'] = $sgst_ledger;
                                $sgst_ary['primary_grp'] = $sgst_ledger_name->sub_group_1;
                                $sgst_ary['second_grp'] = $sgst_ledger_name->sub_group_2;
                                $sgst_ary['main_grp'] = $sgst_ledger_name->main_group;
                ...
                foreach ($igst_charges_array as $key => $value) {
                    if ($data_main['purchase_debit_note_type_of_supply'] == 'intra_state') {
                        if ($cgst_charges_array[$key]['tax_percentage'] > 0 || $sgst_charges_array[$key]['tax_percentage'] > 0) {
                            if ($data_main['purchase_debit_note_gst_payable'] != "yes") {
                                $cgst_ary = array(
        ...
        /* Tax rate slab ends */
        /* TDS SLAB */
        if ($data_main['purchase_debit_note_tds_amount'] > 0 || $data_main['purchase_debit_note_tcs_amount'] > 0) {
            $tds_slab = array();
            ...
            foreach ($js_data as $key => $value) {
                ...
                if ($value['purchase_debit_note_item_tds_percentage'] > 0) {

                    $string = 'tds.section_name,td.tax_name';
                    $table = 'tax td';
                    $where = array('td.delete_status' => 0, 'td.tax_id' => $value['purchase_debit_note_item_tds_id']);
                    ...
                    if (strtoupper($module_type) == "TCS") {
                        ...
                        $tds_ary = array(
                                        'ledger_name' => $tds_subgroup.' '.$section_name.'@'.$value['purchase_debit_note_item_tds_percentage'].'%',
                                        'second_grp' => '',
                                        'primary_grp' => '',
                                        'main_grp' => 'Current Assets',
                                        'default_ledger_id' => 0,
                                        'default_value' => $value['purchase_debit_note_item_tds_percentage'],
                                        'amount' => 0
                                    );
                        if(!empty($tds_ledger_name)){
                            $tds_ledger = $tds_ledger_name->ledger_name;
                            $tds_ledger = str_ireplace('{{SECTION}}',$section_name, $tds_ledger);
                            $tds_ledger = str_ireplace('{{X}}',$value['purchase_debit_note_item_tds_percentage'] , $tds_ledger);
                    ...
                    if (in_array($tds_ledger, $tds_slab)) {
                        $tds_slab_items[$tds_ledger] = bcadd($tds_slab_items[$tds_ledger], $value['purchase_debit_note_item_tds_amount'], $section_modules['access_common_settings'][0]->amount_precision);
                    } else {
                        $tds_slab[] = $tds_ledger;
                        $tds_slab_items[$tds_ledger] = $value['purchase_debit_note_item_tds_amount'];
                    }
        ...
        $purchase_debit_note_ledger_id = $this->ledger_model->getGroupLedgerId($purchase_ary);
        /*$purchase_debit_note_ledger_id = $this->ledger_model->getDefaultLedger('Purchases');
        if ($purchase_debit_note_ledger_id == 0) {
            $purchase_debit_note_ledger_id = $this->ledger_model->addGroupLedger(array(
                'ledger_name' => 'Purchases',
                'subgrp_2' => '',
                'subgrp_1' => '',
                'main_grp' => 'Purchases group',
        ...
        $ledgers['purchase_debit_note_ledger_id'] = $purchase_debit_note_ledger_id;

        $string = 'ledger_id,supplier_name';
        $table = 'supplier';
        $where = array('supplier_id' => $data_main['purchase_debit_note_party_id']);
        ...
        $ledgers['ledger_to'] = $purchase_debit_note_ledger_id;
        ...
        if ($data_main['purchase_debit_note_gst_payable'] != "yes") {
            $grand_total = $data_main['purchase_debit_note_grand_total'];
        } else {
            $total_tax_amount = ($data_main['purchase_debit_note_tax_amount'] + $data_main['freight_charge_tax_amount'] + $data_main['insurance_charge_tax_amount'] + $data_main['packing_charge_tax_amount'] + $data_main['incidental_charge_tax_amount'] + $data_main['inclusion_other_charge_tax_amount'] - $data_main['exclusion_other_charge_tax_amount'] + $data_main['purchase_debit_note_tax_cess_amount']);
            $grand_total = bcsub($data_main['purchase_debit_note_grand_total'], $total_tax_amount, $section_modules['access_common_settings'][0]->amount_precision);
        }

        if (isset($data_main['purchase_debit_note_tds_amount']) && $data_main['purchase_debit_note_tds_amount'] > 0) {
            $grand_total = bcsub($grand_total, $data_main['purchase_debit_note_tds_amount'], $section_modules['access_common_settings'][0]->amount_precision);
        }
        ...
        $sub_total = $data_main['purchase_debit_note_sub_total'];
        ...
        $vouchers_new[] = array(
            ...
            "ledger_to" => $purchase_debit_note_ledger_id,
            "purchase_voucher_id" => '',
            "voucher_amount" => $sub_total,
            "converted_voucher_amount" => $converted_voucher_amount,
            "dr_amount" => 0,
            "cr_amount" => $sub_total,
            'ledger_id' => $purchase_debit_note_ledger_id
        );
        ...
        if ($data_main['purchase_debit_note_tds_amount'] > 0 || $data_main['purchase_debit_note_tcs_amount'] > 0) {
            foreach ($tds_slab_items as $key => $value) {
                ...
                /* if ($data_main['purchase_debit_note_tcs_amount'] > 0) {
                  $dr_amount = '';
                  $cr_amount = $value;
        ...
        /* if ((($data_main['purchase_debit_note_tax_amount'] > 0  && ($data_main['purchase_debit_note_igst_amount'] > 0 || $data_main['purchase_debit_note_cgst_amount'] > 0 || $data_main['purchase_debit_note_sgst_amount'] > 0)) || $data_main['purchase_debit_note_tax_cess_amount'] > 0 )){ */
        ...
        if ($data_main['purchase_debit_note_tax_cess_amount'] > 0) {
        ...
        /* }
          else if (($data_main['purchase_debit_note_tax_amount'] > 0 && ($data_main['purchase_debit_note_igst_amount'] == 0 && $data_main['purchase_debit_note_cgst_amount'] == 0 && $data_main['purchase_debit_note_sgst_amount'] == 0)) && $data_main['purchase_debit_note_gst_payable'] != "yes"){
        ...
        /* discount slab */
        $discount_sum = 0;
        if ($data_main['purchase_debit_note_discount_amount'] > 0) {
            ...
            foreach ($js_data as $key => $value) {
                $discount_sum = bcadd($discount_sum, $value['purchase_debit_note_item_discount_amount'], $section_modules['access_common_settings'][0]->amount_precision);
            }
    ...
    public function purchase_debit_note_voucher_entry($data_main, $js_data, $action, $branch) {
        $purchase_debit_note_voucher_module_id = $this->config->item('purchase_debit_note_module');
        $module_id = $purchase_debit_note_voucher_module_id;
        $modules = $this->get_modules();
        $privilege = "view_privilege";
        $section_modules = $this->get_section_modules($purchase_debit_note_voucher_module_id, $modules, $privilege);
        ...
        $vouchers = $this->purchase_debit_note_vouchers($section_modules, $data_main, $js_data, $branch);
        $grand_total = $data_main['purchase_debit_note_grand_total'];
        /* if ($data_main['purchase_debit_note_gst_payable'] != "yes"){
          } else {
          $total_tax_amount = ($data_main['purchase_debit_note_tax_amount'] + $data_main['freight_charge_tax_amount'] + $data_main['insurance_charge_tax_amount'] + $data_main['packing_charge_tax_amount'] + $data_main['incidental_charge_tax_amount'] + $data_main['inclusion_other_charge_tax_amount'] - $data_main['exclusion_other_charge_tax_amount']);
          $grand_total      = bcsub($data_main['purchase_debit_note_grand_total'] , $total_tax_amount,$section_modules['access_common_settings'][0]->amount_precision);
          } */
        ...
        if ($action == "add") {
            ...
            $current_date = $data_main['purchase_debit_note_date'];
            $voucher_number = $this->generate_invoice_number($access_settings, $primary_id, $table_name, $date_field_name, $current_date);

            $headers = array(
                "voucher_date" => $data_main['purchase_debit_note_date'],
                "voucher_number" => $voucher_number,
                "party_id" => $data_main['purchase_debit_note_party_id'],
                "party_type" => $data_main['purchase_debit_note_party_type'],
                "reference_id" => $data_main['purchase_debit_note_id'],
                "reference_type" => 'purchase_debit_note',
                "reference_number" => $data_main['purchase_debit_note_invoice_number'],
            ...
            $purchase_debit_note_voucher_data = $this->general_model->getRecords('purchase_voucher_id', 'purchase_voucher', array(
                'reference_id' => $data_main['purchase_debit_note_id'], 'reference_type' => 'purchase_debit_note', 'delete_status' => 0));

            if ($purchase_debit_note_voucher_data) {
                $purchase_voucher_id = $purchase_debit_note_voucher_data[0]->purchase_voucher_id;
                ...
                $old_purchase_debit_note_voucher_items = $this->general_model->getRecords($string, $table, $where, $order = "");
                $old_purchase_debit_note_ledger_ids = $this->getValues($old_purchase_debit_note_voucher_items, 'ledger_id');
                $not_deleted_ids = array();

                foreach ($vouchers as $key => $value) {
                    if (($led_key = array_search($value['ledger_id'], $old_purchase_debit_note_ledger_ids)) !== false) {
                        unset($old_purchase_debit_note_ledger_ids[$led_key]);
                        $accounts_purchase_id = $old_purchase_debit_note_voucher_items[$led_key]->accounts_purchase_id;
                ...
                if (!empty($old_purchase_debit_note_voucher_items)) {
                    $revert_ary = array();
                    foreach ($old_purchase_debit_note_voucher_items as $key => $value) {
                        if (!in_array($value->accounts_purchase_id, $not_deleted_ids)) {
                            $revert_ary[] = $value;
                            $table = 'accounts_purchase_voucher';
                            $where = array('accounts_purchase_id' => $value->accounts_purchase_id);
                            $purchase_debit_note_data = array('delete_status' => 1);
                            $this->general_model->updateData($table, $purchase_debit_note_data, $where);
                        }
                    }
                    if (!empty($revert_ary))
    ...
    function edit($id) {
        ...
        $data['data'] = $this->general_model->getRecords('*', 'purchase_debit_note', array(
        ...
        $data['shipping_address'] = $this->general_model->getRecords('*', 'shipping_address', array(
            'shipping_party_id' => $data['data'][0]->purchase_debit_note_party_id,
            'shipping_party_type' => $data['data'][0]->purchase_debit_note_party_type,
        ));

        $item_types = $this->general_model->getRecords('item_type,purchase_debit_note_item_description', 'purchase_debit_note_item', array(
            'purchase_debit_note_id' => $id));
        ...
        foreach ($item_types as $key => $value) {
            if ($value->purchase_debit_note_item_description != "") {
                $description++;
            }
        ...
        if ($data['data'][0]->purchase_debit_note_tax_amount > 0 || $data['access_settings'][0]->tax_type != "no_tax") {
            
        }

        if ($data['data'][0]->purchase_debit_note_nature_of_supply == "service" || $data['data'][0]->purchase_debit_note_nature_of_supply == "both") {
            $data['sac'] = $this->sac_call();
            $data['service_category'] = $this->service_category_call();
        }

        if ($data['data'][0]->purchase_debit_note_nature_of_supply == "product" || $data['data'][0]->purchase_debit_note_nature_of_supply == "both") {
        ...
        $purchase_debit_note_service_items = array();
        $purchase_debit_note_product_items = array();
        if (($data['data'][0]->purchase_debit_note_nature_of_supply == "service" || $data['data'][0]->purchase_debit_note_nature_of_supply == "both") && $service == 1) {

            $service_items = $this->common->purchase_debit_note_items_service_list_field($id);
            $purchase_debit_note_service_items = $this->general_model->getJoinRecords($service_items['string'], $service_items['table'], $service_items['where'], $service_items['join']);
        }

        if ($data['data'][0]->purchase_debit_note_nature_of_supply == "product" || $data['data'][0]->purchase_debit_note_nature_of_supply == "both") {
            /* if ($product == 2)
              {
              $product_items       = $this->common->purchase_debit_note_items_product_inventory_list_field($id);
              $purchase_debit_note_product_items = $this->general_model->getJoinRecords($product_items['string'] , $product_items['table'] , $product_items['where'] , $product_items['join']);
              }
              else */
            /* if ($product == 1)
              { */
            $product_items = $this->common->purchase_debit_note_items_product_list_field($id);
            $purchase_debit_note_product_items = $this->general_model->getJoinRecords($product_items['string'], $product_items['table'], $product_items['where'], $product_items['join']);
            /* } */
        }

        $data['items'] = array_merge($purchase_debit_note_product_items, $purchase_debit_note_service_items);
        ...
        if ($data['data'][0]->purchase_debit_note_tax_amount > 0 && $data['data'][0]->purchase_debit_note_igst_amount > 0 && ($data['data'][0]->purchase_debit_note_cgst_amount == 0 && $data['data'][0]->purchase_debit_note_sgst_amount == 0)) {
            /* igst tax slab */
            $igstExist = 1;
        } elseif ($data['data'][0]->purchase_debit_note_tax_amount > 0 && ($data['data'][0]->purchase_debit_note_cgst_amount > 0 || $data['data'][0]->purchase_debit_note_sgst_amount > 0) && $data['data'][0]->purchase_debit_note_igst_amount == 0) {
            /* cgst tax slab */
            $cgstExist = 1;
            $sgstExist = 1;
        } elseif ($data['data'][0]->purchase_debit_note_tax_amount > 0 && ($data['data'][0]->purchase_debit_note_igst_amount == 0 && $data['data'][0]->purchase_debit_note_cgst_amount == 0 && $data['data'][0]->purchase_debit_note_sgst_amount == 0)) {
            /* Single tax */
            $taxExist = 1;
        } elseif ($data['data'][0]->purchase_debit_note_tax_amount == 0 && ($data['data'][0]->purchase_debit_note_igst_amount == 0 && $data['data'][0]->purchase_debit_note_cgst_amount == 0 && $data['data'][0]->purchase_debit_note_sgst_amount == 0)) {
            /* No tax */
        ...
        if ($data['data'][0]->purchase_debit_note_tax_cess_amount > 0) {
            $cessExist = 1;
        }

        if ($data['data'][0]->purchase_debit_note_discount_amount > 0 || $data['access_settings'][0]->discount_visible == "yes") {
            /* Discount */
            $discountExist = 1;
            $data['discount'] = $this->discount_call();
        }
        if ($data['data'][0]->purchase_debit_note_tds_amount > 0 || $data['data'][0]->purchase_debit_note_tcs_amount > 0 || $data['access_settings'][0]->tds_visible == "yes") {
            /* Discount */
            $tdsExist = 1;
        }
        ...
        $is_utgst = $this->general_model->checkIsUtgst($data['data'][0]->purchase_debit_note_billing_state_id);
        ...
        /* echo "<pre>";
          print_r($data);
          exit(); */
        $this->load->view('purchase_debit_note/edit', $data);
    }

    public function edit_purchase_debit_note() {

        $data = $this->get_default_country_state();
        $purchase_debit_note_id = $this->input->post('purchase_debit_note_id');
        $purchase_debit_note_module_id = $this->config->item('purchase_debit_note_module');
        $module_id = $purchase_debit_note_module_id;
        $modules = $this->modules;
        $privilege = "edit_privilege";
        ...
        $purchase_debit_note_data = array(
            ...
            "purchase_debit_note_discount_amount" => $this->input->post('total_discount_amount') ? (float) $this->input->post('total_discount_amount') : 0,
            "purchase_debit_note_tax_amount" => $this->input->post('total_tax_amount') ? (float) $this->input->post('total_tax_amount') : 0,
            "purchase_debit_note_tax_cess_amount" => 0,
            "purchase_debit_note_taxable_value" => $this->input->post('total_taxable_amount') ? (float) $this->input->post('total_taxable_amount') : 0,
            "purchase_debit_note_tds_amount" => $this->input->post('total_tds_amount') ? (float) $this->input->post('total_tds_amount') : 0,
            "purchase_debit_note_tcs_amount" => $this->input->post('total_tcs_amount') ? (float) $this->input->post('total_tcs_amount') : 0,
            "purchase_debit_note_igst_amount" => 0,
        ...
        $purchase_debit_note_tax_amount = $purchase_debit_note_data['purchase_debit_note_tax_amount'];
        $purchase_debit_note_tax_amount = $purchase_debit_note_data['purchase_debit_note_tax_amount'] + (float) ($this->input->post('total_other_taxable_amount'));
        ...
        $old_purchase_debit_note_data = $this->general_model->getRecords('*', 'purchase_debit_note', array(
            'purchase_debit_note_id' => $purchase_debit_note_id));
        $purchase_id = $old_purchase_debit_note_data[0]->purchase_id;
        $old_amount = $this->general_model->getRecords("debit_note_amount, converted_debit_note_amount,supplier_payable_amount", "purchase", array(
            'purchase_id' => $purchase_id));
        $new_amount = bcsub($old_amount[0]->debit_note_amount, $old_purchase_debit_note_data[0]->purchase_debit_note_grand_total, $section_modules['access_common_settings'][0]->amount_precision);
        $cust_new_amount = bcadd($old_amount[0]->supplier_payable_amount, ($old_purchase_debit_note_data[0]->purchase_debit_note_grand_total - $old_purchase_debit_note_data[0]->purchase_debit_note_tds_amount), $section_modules['access_common_settings'][0]->amount_precision);
        $new_converted_amount = bcsub($old_amount[0]->converted_debit_note_amount, $old_purchase_debit_note_data[0]->converted_grand_total, $section_modules['access_common_settings'][0]->amount_precision);
        $this->general_model->updateData("purchase", array(
            'debit_note_amount' => $new_amount,
        ...
        $data_main = array_map('trim', $purchase_debit_note_data);
        $purchase_debit_note_table = $this->config->item('purchase_debit_note_table');
        $where = array('purchase_debit_note_id' => $purchase_debit_note_id);

        if ($this->general_model->updateData($purchase_debit_note_table, $data_main, $where)) {

            $old_amount1 = $this->general_model->getRecords("debit_note_amount,supplier_payable_amount", "purchase", array(
                'purchase_id' => $purchase_id));
            $new_amount1 = bcadd($old_amount1[0]->debit_note_amount, $purchase_debit_note_data['purchase_debit_note_grand_total'], $section_modules['access_common_settings'][0]->amount_precision);
            $cust_new_amount1 = bcsub($old_amount1[0]->supplier_payable_amount, ($purchase_debit_note_data['purchase_debit_note_grand_total'] - $purchase_debit_note_data['purchase_debit_note_tds_amount']), $section_modules['access_common_settings'][0]->amount_precision);
            $this->general_model->updateData("purchase", array(
                'debit_note_amount' => $new_amount1, 'supplier_payable_amount' => $cust_new_amount1), array(
                'purchase_id' => $purchase_id));
            $successMsg = 'Purchase Debit Note Updated Successfully';
            $this->session->set_flashdata('purchase_dn_success',$successMsg);
            $log_data = array(
                'user_id' => $this->session->userdata('SESS_USER_ID'),
                'table_id' => $purchase_debit_note_id,
                ...
                'financial_year_id' => $this->session->userdata('SESS_FINANCIAL_YEAR_ID'),
                'branch_id' => $this->session->userdata('SESS_BRANCH_ID'),
                'message' => 'Purchase debit Note Updated');
            $data_main['purchase_debit_note_id'] = $purchase_debit_note_id;
            $log_table = $this->config->item('log_table');
            $this->general_model->insertData($log_table, $log_data);
            $purchase_debit_note_item_data = $this->input->post('table_data');
            ...
            if (!empty($js_data)) {
                ...
                $string = 'purchase_debit_note_item_id,purchase_debit_note_item_quantity,item_type,item_id';
                $table = 'purchase_debit_note_item';
                $where = array(
                    'purchase_debit_note_id' => $purchase_debit_note_id,
                    'delete_status' => 0);
                $old_purchase_debit_note_items = $this->general_model->getRecords($string, $table, $where, $order = "");
                $old_item_ids = $this->getValues($old_purchase_debit_note_items, 'item_id');

                $not_deleted_ids = array();

                foreach ($old_purchase_debit_note_items as $key => $value) {
                    ...
                    if (!empty($product)) {
                        $product_qty = bcadd($product[0]->product_quantity, $value->purchase_debit_note_item_quantity, $section_modules['access_common_settings'][0]->amount_precision);
                        /* update Product Price */
                        ...
                        if (!empty($history)) {
                            $history_quantity = bcadd($history[0]->quantity, $value->purchase_debit_note_item_quantity, $section_modules['access_common_settings'][0]->amount_precision);
                ...
                foreach ($js_data as $key => $value) {
                    if ($value != null) {
                        ...
                        if (($item_key = array_search($value->item_id, $old_item_ids)) !== false) {
                            ...
                            $purchase_debit_note_item_id = $old_purchase_debit_note_items[$item_key]->purchase_debit_note_item_id;
                            array_push($not_deleted_ids, $purchase_debit_note_item_id);
                            $where = array('purchase_debit_note_item_id' => $purchase_debit_note_item_id);
                ...
                if (!empty($old_purchase_debit_note_items)) {
                    foreach ($old_purchase_debit_note_items as $key => $items) {
                        if (!in_array($items->purchase_debit_note_item_id, $not_deleted_ids)) {
                            $table = 'purchase_debit_note_item';
                            $where = array(
                                'purchase_debit_note_item_id' => $items->purchase_debit_note_item_id);
                            $purchase_debit_note_data = array(
                                'delete_status' => 1);
            ...
            $errorMsg = 'Purchase Debit Note Update Unsuccessful';
            $this->session->set_flashdata('purchase_dn_error',$errorMsg);
            redirect('purchase_debit_note', 'refresh');
        }
    }
    ...
    public function delete() {
        ...
        $purchase_debit_note_id = $this->encryption_url->decode($id);
        $purchase_debit_note_module_id = $this->config->item('purchase_debit_note_module');
        $data['module_id'] = $purchase_debit_note_module_id;
        $modules = $this->modules;
        ...
        $data['data'] = $this->general_model->getRecords('*', 'purchase_debit_note', array(
            'purchase_debit_note_id' => $purchase_debit_note_id));
        $data['items'] = $this->general_model->getRecords('*', 'purchase_debit_note_item', array(
            'purchase_debit_note_id' => $purchase_debit_note_id,
            'delete_status' => 0));
        ...
        if ($this->general_model->updateData('purchase_debit_note', array('delete_status' => 1), array('purchase_debit_note_id' => $purchase_debit_note_id))) {
            $purchase_id = $data['data'][0]->purchase_id;
            $old_amount = $this->general_model->getRecords("debit_note_amount, converted_debit_note_amount", "purchase", array(
                'purchase_id' => $purchase_id));
            $new_amount = bcsub($old_amount[0]->debit_note_amount, $data['data'][0]->purchase_debit_note_grand_total, $section_modules['access_common_settings'][0]->amount_precision);
            ...
            foreach ($data['items'] as $key => $value) {
                ...
                if ($value->item_type == "product" || $value->item_type == "product_inventory") {
                    ...
                    $product_result = $this->general_model->getJoinRecords($product_data['string'], $product_data['table'], $product_data['where'], $product_data['join']);
                    $product_quantity = ($product_result[0]->product_quantity - $value->purchase_debit_note_item_quantity);
                    /* update Product Price */
                    $pro_price = $this->getAVGItemPrice($value->item_id);
                    /* END */
                    $data1 = array(
            ...
            $successMsg = 'Purchase Debit Note Deleted Successfully';
            $this->session->set_flashdata('purchase_dn_success',$successMsg);
            $log_data = array(
                'user_id' => $this->session->userdata('SESS_USER_ID'),
                'table_id' => $purchase_debit_note_id,
    ...
    public function view($id) {
        ...
        $purchase_debit_note_data = $this->common->purchase_debit_note_list_field1($id);

        $data['data'] = $this->general_model->getJoinRecords($purchase_debit_note_data['string'], $purchase_debit_note_data['table'], $purchase_debit_note_data['where'], $purchase_debit_note_data['join']);
        ...
        if ($data['data'][0]->purchase_debit_note_tds_amount > 0 || $data['data'][0]->purchase_debit_note_tcs_amount > 0) {
            /* Discount */
            $tdsExist = 1;
        }

        if ($data['data'][0]->purchase_debit_note_discount_amount > 0) {
            /* Discount */
            $discountExist = 1;
        }
        ...
        $this->load->view('purchase_debit_note/view', $data);
    }
    ...
    public function pdf($id) {
        ...
        if ($purchase_debit_note_product_items && $purchase_debit_note_service_items) {
            $nature_of_supply = "Product/Service";
        } elseif ($purchase_debit_note_product_items) {
            $nature_of_supply = "Product";
        } elseif ($purchase_debit_note_service_items) {
        ...
        $html = $this->load->view('purchase_debit_note/pdf', $data, true);
        ...
        $dompdf->stream($data['data'][0]->purchase_debit_note_invoice_number, array(
            'Attachment' => 0));
    }
    ...
    public function email($id) {
        ...
        if ($email_sub_module == 1) {
            ...
            $purchase_debit_note_data = $this->common->purchase_debit_note_list_field1($id);
            $data['data'] = $this->general_model->getJoinRecords($purchase_debit_note_data['string'], $purchase_debit_note_data['table'], $purchase_debit_note_data['where'], $purchase_debit_note_data['join']);
            $country_data = $this->common->country_field($data['data'][0]->purchase_debit_note_billing_country_id);
            $data['data_country'] = $this->general_model->getRecords($country_data['string'], $country_data['table'], $country_data['where']);
            $state_data = $this->common->state_field($data['data'][0]->purchase_debit_note_billing_country_id, $data['data'][0]->purchase_debit_note_billing_state_id);
            ...
            foreach ($data['items'] as $value) {
                $igst = bcadd($igst, $value->purchase_debit_note_item_igst_amount, $section_modules['access_common_settings'][0]->amount_precision);
                $cgst = bcadd($cgst, $value->purchase_debit_note_item_cgst_amount, $section_modules['access_common_settings'][0]->amount_precision);
                $sgst = bcadd($sgst, $value->purchase_debit_note_item_sgst_amount, $section_modules['access_common_settings'][0]->amount_precision);

                if ($value->purchase_debit_note_item_description != "" && $value->purchase_debit_note_item_description != null) {
                    $dpcount++;
                }

                if ($value->purchase_debit_note_item_discount_amount != "" && $value->purchase_debit_note_item_discount_amount != null && $value->purchase_debit_note_item_discount_amount != 0) {
                    $dtcount++;
                }
            }

            if ($data['data'][0]->purchase_debit_note_tax_amount > 0)
                $tax_exist = 1;
            if ($data['data'][0]->purchase_debit_note_tds_amount > 0)
                $tds_exist = 1;
            ...
            //and now im creating new instance dompdf
            $file_path = "././pdf_form/";
            $file_name = str_replace($data['access_settings'][0]->invoice_seperation, "_", $data['data'][0]->purchase_debit_note_invoice_number);
            ...
            $data['email_template'] = $this->general_model->getRecords('*', 'email_template', array(
                ...
                'branch_id' => $this->session->userdata('SESS_BRANCH_ID'),
                'delete_status' => 0));
            $this->load->view('purchase_debit_note/email', $data);
        }
        else {
            $this->load->view('purchase_debit_note', $data);
        }
    }
    ...
    public function convert_currency() {
        ...
        $debit_note_data = $this->general_model->getRecords('purchase_id,currency_converted_amount', 'purchase_debit_note', array(
            'purchase_debit_note_id' => $id));
        $old_debit_note_converted_amount = $debit_note_data[0]->currency_converted_amount;

        $purchase_data = $this->general_model->getRecords('converted_debit_note_amount', 'purchase', array(
            'purchase_id' => $debit_note_data[0]->purchase_id));
        $old_purchase_con_debit_amt = $purchase_data[0]->converted_debit_note_amount;

        $converted_debit_amount = bcsub($old_purchase_con_debit_amt, $old_debit_note_converted_amount, $section_modules['access_common_settings'][0]->amount_precision);
        $total_converted_debit_amount = bcadd($converted_debit_amount, $new_converted_amount, $section_modules['access_common_settings'][0]->amount_precision);

        $data = array(
            'currency_converted_rate' => $new_converted_rate,
            'currency_converted_amount' => $this->input->post('currency_converted_amount'));
        $this->general_model->updateData('purchase_debit_note', $data, array(
            'purchase_debit_note_id' => $id));
