...
class Bank_ledger extends MY_Controller{
   ...
   function __construct(){
    ...
    function index(){
        $bank_voucher_module         = $this->config->item('bank_voucher_module');
        $data['module_id']               = $bank_voucher_module;
        $modules                         = $this->modules;
        $privilege                       = "view_privilege";
        $data['privilege']               = $privilege;
        $section_modules                 = $this->get_section_modules($bank_voucher_module, $modules, $privilege);
        ...
        $this->load->view('bank_voucher/voucher_list', $data);
    }
    ...
    function GetAllVoucher(){
        ...
        $sql = "SELECT bank_voucher_id FROM `bank_voucher` ";
        if($voucher_id != 0 && $voucher_id != ''){
            $sql .= " WHERE bank_voucher_id='{$voucher_id}' ";
        }else{
        ...
        if(!empty($sub_ary)){
            ...
            $extraWhere = " AND bank_voucher_id IN (SELECT bank_voucher_id FROM accounts_bank_voucher WHERE {$where})";
        }
        $sql .= $extraWhere;
        ...
        $sql .= " ORDER BY bank_voucher_id DESC LIMIT $end,$limit"; 
        ...
        if($qry->num_rows() > 0){
            ...
            foreach ($vids as $key => $value) {
                array_push($vids_ary, $value['bank_voucher_id']);               
            }
        }
        ...
        if(!empty($vids_ary)){
            $this->db->select('ts.voucher_number,ts.reference_id,ts.bank_voucher_id,ts.voucher_date,ts.receipt_amount,ts.reference_number,ts.delete_status,ts.description,tsa.ledger_id,tsa.voucher_amount,tsa.dr_amount,tsa.cr_amount,tsa.accounts_bank_id');
            $this->db->where_in('ts.bank_voucher_id',$vids_ary);
            $this->db->where('ledger_id != ','');
            $this->db->where('tsa.delete_status',0);
            $this->db->order_by('ts.bank_voucher_id', 'DESC');
            $this->db->join('accounts_bank_voucher tsa',
                'ts.bank_voucher_id=tsa.bank_voucher_id','left');
            $getAll = $this->db->get('bank_voucher ts');
        ...
        if(!empty($response)){
            ...
            foreach ($response as $key => $value) {
                ...
                $vid = $value['bank_voucher_id'];
                $accounts_voucher_id = ($value['accounts_bank_id'] != '' ? $value['accounts_bank_id']: 0);
