...
class Sales_debit_ledgers extends MY_Controller {
    ...
    function index() {
        $sales_debit_note_voucher_id = $this->config->item('sales_debit_note_voucher');
        $data['module_id'] = $sales_debit_note_voucher_id;
        $modules = $this->modules;
        $privilege = "view_privilege";
        $data['privilege'] = $privilege;
        $section_modules = $this->get_section_modules($sales_debit_note_voucher_id, $modules, $privilege);
        ...
        $this->load->view('sales_voucher/sales_debit_ledger', $data);
    }
    ...
    function GetAllVoucher() {
        ...
        if (null != $this->input->post('invoice_to'))
            $to_date = date('Y-m-d', strtotime($this->input->post('invoice_to')));

        if (null != $this->input->post('invoice_from'))
            $from_date = date('Y-m-d', strtotime($this->input->post('invoice_from')));

        $sql = "SELECT sales_voucher_id FROM `sales_voucher` ";
        if ($voucher_id != 0 && $voucher_id != '') {
            $sql .= " WHERE sales_voucher_id='{$voucher_id}' ";
        } else {
            $sql .= " WHERE branch_id='{$branch_id}' AND voucher_date BETWEEN '{$from_date}' AND '{$to_date}' AND reference_type='sales_debit_note' ";
        }

        $sub_ary = array();
        $extraWhere = '';
        /* if($voucher_id != '0') $sql .= " AND sales_voucher_id = '{$voucher_id}' "; */
        $reference_id = $this->encryption_url->decode($this->input->post('reference_id'));
        if(null != $reference_id){
            $sub_ary[] = "sales_voucher_id = '".$reference_id."'";
        }
        ...
        if (!empty($sub_ary)) {
            ...
            $extraWhere = " AND sales_voucher_id IN (SELECT sales_voucher_id FROM accounts_sales_voucher WHERE {$where})";
        }
        $sql .= $extraWhere;
        ...
        $sql .= " ORDER BY sales_voucher_id DESC LIMIT $end,$limit";
        $qry = $this->db->query($sql);
        $vids_ary = array();
        if ($qry->num_rows() > 0) {
            $vids = $qry->result_array();
            foreach ($vids as $key => $value) {
                array_push($vids_ary, $value['sales_voucher_id']);
            }
        }
        ...
        if (!empty($vids_ary)) {
            $this->db->select('ts.voucher_number,ts.reference_id,ts.sales_voucher_id,ts.voucher_date,ts.receipt_amount,ts.reference_number,ts.delete_status,ts.description,tsa.ledger_id,tsa.voucher_amount,tsa.dr_amount,tsa.cr_amount,tsa.accounts_sales_id');
            $this->db->where_in('ts.sales_voucher_id', $vids_ary);
            //$this->db->where_in('ts.sales_voucher_id',array('28'));
            $this->db->where('ledger_id != ', '');
            $this->db->where('tsa.delete_status', '0');
            $this->db->order_by('ts.sales_voucher_id', 'DESC');
            $this->db->join('accounts_sales_voucher tsa', 'ts.sales_voucher_id=tsa.sales_voucher_id', 'left');
            $getAll = $this->db->get('sales_voucher ts');
        ...
        if (!empty($response)) {
            ...
            foreach ($response as $key => $value) {
                $vid = $value['sales_voucher_id'];
                $total_voucher['v_' . $vid] = (@$total_voucher['v_' . $vid] ? $total_voucher['v_' . $vid] + 1 : 1);
            }
            foreach ($response as $key => $value) {
                $vid = $value['sales_voucher_id'];
                $accounts_voucher_id = ($value['accounts_sales_id'] != '' ? $value['accounts_sales_id'] : 0);
                ...
                if (@$resp['voucher_' . $vid]) {
                    ...
                    if ($value['delete_status'] == '0') {
                        /* $tr .= "<td><a href='". base_url('sales_debit_note/edit/') .$reference_id."' class='edit_led' data-id='".$vid."'><i class='glyphicon glyphicon-edit'></i></a> "; *///| <a href='javascript:void(0);' data-id='".$vid."' class='sub_led'><i class='glyphicon glyphicon-check'></i></a></td>
                    } else {
                        /* $tr .= "<td></td>"; */
                    }
                    /* }else{
