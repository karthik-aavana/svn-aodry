...
class Payment_voucher extends MY_Controller
{
    ...
    public function multi_payment_list()
    {
        $payment_voucher_module_id         = $this->config->item('payment_voucher_module');
        $data['payment_voucher_module_id'] = $payment_voucher_module_id;
        $modules                           = $this->modules;
        ...
        if (!empty($this->input->post()))
        {
            ...
            $send_data = array();
            if($totalData > 0){
                $supp_list = $this->supplier_call();
                $supp_ids = array();
                foreach ($supp_list as $key => $value) {
                    $supp_ids[$value->supplier_id] = $value->supplier_name;
                }
                if (empty($this->input->post('search')['value']))
                {
                    $list_data['limit']  = $limit;
                ...
                if (!empty($posts))
                {
                    foreach ($posts as $post)
                    {
                        $payment_id   = $this->encryption_url->encode($post->payment_id);
                        $purchase_id = $this->encryption_url->encode($post->reference_id);
                        $purchase_ids = explode(',', $post->reference_id);
                        $references = explode(',', $post->reference_number);
                        $reference_numbers = '';
                        $reference = 'purchase';
                        if($post->reference_type == 'expense') $reference = 'expense_bill';
                        if(!empty($purchase_ids) && !empty($references)){
                            foreach ($purchase_ids as $key => $purchase_id) {
                                if($purchase_id != 'excess_amount' && $purchase_id != 'opening_balance'){
                                    $purchase_id = $this->encryption_url->encode($purchase_id);
                                    $reference_numbers .= ' <a href="' . base_url().$reference.'/view/' . $purchase_id . '">' . $references[$key] . '</a>, ';
                                }else{
                                    $reference_numbers .= $purchase_id.', ';
                                }
                            }
                        }
                        $nestedData['voucher_date']              = date('d-m-Y',strtotime($post->voucher_date));
                        $nestedData['voucher_number']            = '<a href="' . base_url('payment_voucher/view_details/') . $payment_id . '">' . $post->voucher_number . '</a>';
                        $suppliers  = $post->supplier_name;
                        
                        $reference_number          = str_replace(",", ",<br/>", $post->reference_number);
                        $related_post = $this->allPayment($post->payment_id);
                        $amount =  str_replace(",", ",<br/> ", $post->imploded_receipt_amount);
                        $receipt_amount = $this->precise_amount($amount, $access_common_settings[0]->amount_precision);
                        if(!empty($related_post)){
                            foreach ($related_post as $key => $value) {
                                if($value->payment_id != $post->payment_id){
                                    $suppliers .= "<br>".$supp_ids[$value->party_id];
                                    
                                    $receipt_amount .= "<br>".$this->precise_amount($value->receipt_amount, $access_common_settings[0]->amount_precision);
                                    $purchase_ids = explode(',', $value->reference_id);
                        ...
                        if (in_array($payment_voucher_module_id, $data['active_edit']) && $post->voucher_status != "2")
                        {
                            $cols .= '<span><a data-toggle="tooltip" data-placement="bottom" href="' . base_url('payment_voucher/multi_edit/') . $payment_id . '" title="Edit Payment Voucher" class="btn btn-app"><i class="fa fa-pencil"></i></a></span>';
                        }
                        ...
                        if (in_array($payment_voucher_module_id, $data['active_delete']))
                        {
                            $cols .= '<span data-backdrop="static" data-keyboard="false" class="delete_button" data-toggle="modal" data-target="#delete_modal" data-id="' . $payment_id . '" data-path="payment_voucher/multi_delete" ><a  href="#" data-toggle="tooltip" data-placement="bottom" title="Delete Payment Voucher" class="btn btn-app"><i class="fa fa-trash-o"></i></a></span>';
                        }
    ...
    public function add_multi_supplier_payment(){
        /*echo "<pre>";
        print_r($this->input->post());
        exit();*/
        $all_reference = $this->input->post('invoice_data');
        $supplier_invoice_data = json_decode($all_reference,true);
        $payment_voucher_module_id = $this->config->item('payment_voucher_module');
        $data['module_id']         = $payment_voucher_module_id;
        $modules                   = $this->modules;
        $privilege                 = "add_privilege";
        ...
        $is_main_payment = 1;
        $parent_payment_id = 0;
        if(!empty($supplier_invoice_data)){
        foreach ($supplier_invoice_data as $key => $inv_data) {
            ...
            if(!empty($invoice_data)){
                ...
                foreach ($invoice_data as $key => $value) {
                    ...
                    array_push($reference_numbers, $value['reference_number']);
                    array_push($payment_amount, $value['payment_amount']);
                    $payment_grand_total += $value['payment_amount'];
                }
            }
           
            $payment_grand_total = $inv_data['total_payment_amount'];
            $supplier = $this->general_model->getRecords('ledger_id,supplier_name', 'supplier', array( 'supplier_id' => $inv_data['supplier_id']));
            ...
            $payment_data = array(
                ...
                "updated_date"            => "",
                "updated_user_id"         => "",
                "is_main_payment"         => $is_main_payment,
            ...
            if ($payment_id = $this->general_model->insertData($payment_voucher_table, $data_main)) {
                ...
                foreach ($invoice_data as $key7 => $value7) {
                    ...
                    $reference_type = $data_main['reference_type'];
                    if($value7['reference_number'] == 'excess_amount') $reference_type = 'excess_amount';
                    if($value7['reference_number'] == 'opening_balance') $reference_type = 'opening_balance';
                    if ($value7['reference_number'] != 'excess_amount' && $value7['reference_number'] != 'opening_balance') {
                        $reference_data[] = array('payment_id' => $payment_id,
                                            'reference_id' => $value7['reference_number'],
                                            'reference_type' => $reference_type,
                                            'Invoice_total_paid' => ($purchase_paid_amount - $value7['payment_total_paid']),
                    ...
                    if ($data_main['reference_type'] == 'purchase' && $value7['reference_number'] != 'excess_amount' && $value7['reference_number'] != 'opening_balance') {
                    ...
                    } elseif($data_main['reference_type'] == 'expense'  && $value7['reference_number'] != 'excess_amount' && $value7['reference_number'] != 'opening_balance'){
                        ...
                        /*if($excess_purchase_id){
                            ...
                            /* Total paid amount */
                            $this->db->select('SUM(paid_amount) as paid_amount');
                            $this->db->where('party_id',$inv_data['supplier_id']);
                            $this->db->where('party_type','supplier');
                            $this->db->where('delete_status',0);
                            $this->db->from('received_opening_balance');
                            ...
                            $opn_paid_amnt = $opening_balance_paid + $value7['payment_amount'];
                            $pending_amount = $value7['opening_balance'] - $opn_paid_amnt;
                            $opening_ary = array('party_id' => $inv_data['supplier_id'],
                                            'party_type' => 'supplier',
                                            'receipt_id' => $payment_id,
                                            'ledger_id' => $value7['ledger_id'],
                                            'opening_balance' => $value7['opening_balance'],
                                            'paid_amount' => $value7['payment_amount'],
                                            'added_date' => date('Y-m-d'),
                            ...
                            $reference_data[] = array('payment_id' => $payment_id,
                                                ...
                                                'payment_amount' => $value7['payment_amount'],
                                                'Invoice_total_paid' => $opn_paid_amnt,
                                                ...
                                                'round_off' => 0,
                                                'round_off_icon' => '',
                                                'payment_total_paid' =>$value7['payment_amount']
                                            );
                ...
                if(!empty($reference_data))$this->db->insert_batch('payment_invoice_reference',$reference_data);
                $successMsg = 'Payment Voucher Added Successfully';
                $this->session->set_flashdata('payment_voucher_success',$successMsg);
                $log_data = array(
                    'user_id'           => $this->session->userdata('SESS_USER_ID'),
    ...
    public function multi_edit($id){
        ...
        /*$data['currency'] = $this->currency_call();*/
        $data['payment_ids'] = $this->allPayment($id);
        $data['purchase_data'] = array();
        $service = 0;
        $bank_exist = 0;
        if ($data['payment_ids'][0]->payment_mode != "" && $data['payment_ids'][0]->payment_mode != "cash" && $data['payment_ids'][0]->payment_mode != "other payment mode" && $data['payment_ids'][0]->payment_mode != "bank"){
            $bank_exist = 1;
        }
        $data['opening_balance'] = array();
        foreach ($data['payment_ids'] as $k => $payment_id) {
            $pid = $payment_id->payment_id;

            $this->db->select('r.*,rv.reference_id,rv.exchange_gain_loss,rv.exchange_gain_loss_type,rv.discount,rv.other_charges,rv.round_off,rv.round_off_icon,rv.payment_amount as invoice_payment_amount,rv.payment_total_paid,rv.Invoice_pending,rv.Invoice_total_paid,c.supplier_name,c.supplier_mobile');
            $this->db->from('payment_voucher r');
            $this->db->join('payment_invoice_reference rv','r.payment_id=rv.payment_id','left');
            $this->db->join('supplier c','r.party_id=c.supplier_id','left');
            $this->db->where('r.payment_id',$pid);
            $d = $this->db->get();
        
            $data['data'][$pid] = $d->result();
            $data['data'][0] = $data['data'][$pid][0];
        
            $reference_type = $payment_id->reference_type;
            if(!empty($data['data'][$pid])){
                
                $supplier_id = $payment_id->party_id;
                if($reference_type == 'purchase'){
                ...
                $data['purchase_data'] = array_merge($data['purchase_data'],$purchase_data);
            }

            $purchase_data_1 = array();
           
            if(!empty($purchase_data)){
                foreach ($data['data'][$pid] as $key => $value) {
                    $is_edit = 0;
                    if(array_key_exists('purchase_'.$value->reference_id, $purchase_data)){
                        ...
                        $pending_amount = $invoice_total - $paid_amount - $value->payment_total_paid;
                        $data['data'][$pid][$key]->invoice_total = $invoice_total;
                        $data['data'][$pid][$key]->invoice_paid_amount = $paid_amount;
                        $data['data'][$pid][$key]->invoice_pending = $pending_amount;
                        $data['data'][$pid][$key]->reference_number = $purchase_invoice->purchase_invoice_number;
                        
                        if($last_payment[0]->payment_id == $value->payment_id && $pending_amount > 0)$is_edit = 1;

                        if($is_edit == 0){
                            $data['data'][$pid][$key]->invoice_paid_amount = $value->Invoice_total_paid;
                            $data['data'][$pid][$key]->invoice_pending = $value->Invoice_pending;
                        }
                    }
                    if($value->reference_number == 'excess_amount' || $value->reference_id == '0') $is_edit = 1;
                    $data['data'][$pid][$key]->is_edit = $is_edit;
                }
            }
            ...
            $this->db->select('*');
            $this->db->where('receipt_id',$pid);
            $this->db->where('party_id',$supplier_id);
            $this->db->where('delete_status',0);
            ...
            if(!empty($opening_balance)){
                ...
                $data['opening_balance'][$pid] = $opening_ary;
            }
        }
        $data['invoice_data'] = $purchase_data_1;
        $data['bank_exist'] = $bank_exist;
    ...
    public function edit_multi_payment(){
        $all_reference = $this->input->post('invoice_data');
        $supp_invoice_data = json_decode($all_reference,true);
        $payment_voucher_module_id = $this->config->item('payment_voucher_module');
        $data['module_id']         = $payment_voucher_module_id;
        $modules                   = $this->modules;
        $privilege                 = "edit_privilege";
        ...
        $old_payments = $this->allPayment($parent_payment_id);
        $old_payment_ids = array();
        foreach ($old_payments as $key => $value) {
            array_push($old_payment_ids, $value->payment_id);
        }

        foreach ($supp_invoice_data as $key => $ref_data) {
            $invoice_data = $ref_data['data_item'];
            
            $payment_id  = (@$ref_data['payment_id'] ? $ref_data['payment_id'] : 0);
            if (($key = array_search($payment_id, $old_payment_ids)) !== false) {
                unset($old_payment_ids[$key]);
            }
            ...
            $payment_grand_total = $ref_data['total_payment_amount'];
            $payment_amount_old  = (@$ref_data['payment_amount_old'] ? $ref_data['payment_amount_old'] : 0);

            $supplier = $this->general_model->getRecords('ledger_id,supplier_name', 'supplier', array(
                'supplier_id' => $ref_data['supplier_id']));
            $ledger_to   = $supplier[0]->ledger_id;
            $supplier_name = $supplier[0]->supplier_name;
            $supplier_ledger_id = $supplier[0]->ledger_id;
            ...
            if($payment_id != '' && $payment_id != 0){
                $action = 'edit';
                $this->general_model->updateData($payment_voucher_table, $data_main, $where);
            }else{
                $data_main['parent_payment_id'] = $parent_payment_id;
                $data_main['is_main_payment'] = 0;
                ...
                $action = 'add';
                $payment_id = $this->general_model->insertData($payment_voucher_table, $data_main);
            }
            if ($payment_id != 0){
                $purchase_id_data       = $reference_numbers;
                $this->db->select('*');
                ...
                $this->db->where('receipt_id',$payment_id);
                $this->db->where('delete_status','0');
                ...
                foreach ($invoice_data as $key => $value) {
                    ...
                    if($value['reference_number'] != 'excess_amount'  && $value['reference_number'] != 'opening_balance'){
                        ...
                        $this->db->insert('payment_invoice_reference',$reference_data);
                            ...
                            /* Total paid amount */
                            $this->db->select('SUM(paid_amount) as paid_amount');
                            $this->db->where('party_id',$ref_data['supplier_id']);
                            $this->db->where('party_type','supplier');
                            $this->db->where('delete_status',0);
                            $this->db->from('received_opening_balance');
                            $op_paid_amnt = $this->db->get();
                            ...
                            if(!empty($opn_paid_amount))
                            $opening_balance_paid = $opn_paid_amount[0]->paid_amount;
                            $opn_paid_amnt = $opening_balance_paid + $value['payment_amount'];
                            $pending_amount = $value['opening_balance'] - $opn_paid_amnt;
                            $opening_ary = array('party_id' => $ref_data['supplier_id'],
                                            'party_type' => 'supplier',
                                            'receipt_id' => $payment_id,
                            ...
                            $reference_data = array('payment_id' => $payment_id,
                                                ...
                                                'round_off' => 0,
                                                'round_off_icon' => '',
                                                'payment_total_paid' =>$value['payment_amount']
                                            );
                            $this->db->insert('payment_invoice_reference',$reference_data);
                            $voucher_data[] = $reference_data;
                    }
                ...
                if (in_array($data['accounts_module_id'], $data['active_add'])){
                    ...
                    if (in_array($data['accounts_sub_module_id'], $data['access_sub_modules'])){
                        ...
                        $this->VoucherEntry($data_main,$voucher_data,$supplier_name,$supplier_ledger_id , $action);
                    }
                }
            }
        }

        if(!empty($old_payment_ids)){
            
            foreach ($old_payment_ids as $key => $payment_id) {
    ...
    public function multi_delete()
    {
        ...
        $data                   = array_merge($data, $section_modules);
        $access_common_settings = $section_modules['access_common_settings'];
        $all_payments = $this->allReceipts($payment_id);
        $payment_data = $this->general_model->getRecords('*', $payment_voucher_table, array(
            'payment_id' => $payment_id));
        foreach ($payment_data as $key => $value) {
...
// $converted_paid_amount = bcsub($expense_data[0]->converted_paid_amount, $old_converted_receipt_array[$key]);
    ...
    public function view_details($id) {
        ...
        if($this->config->item('Sanath') == $this->session->userdata('SESS_FIRM_ID')){
            ...
            foreach ($all_payment as $key => $value) {
                $payment_id = $value->payment_id;
                $voucher_details = $this->common->payment_voucher_details($payment_id);

                $payments[] = $this->general_model->getJoinRecords($voucher_details['string'], $voucher_details['table'], $voucher_details['where'], $voucher_details['join']);
            }
    ...
    public function allPayment($id){
        ...
        $this->db->from('payment_voucher r');
        $this->db->where('r.payment_id',$id);
        $this->db->or_where('(r.parent_payment_id='.$id.' AND delete_status=0)');
        $d = $this->db->get();
        return $d->result();
    }
}
