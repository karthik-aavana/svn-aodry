...
class Purchase_ledger extends MY_Controller
{
    ...
    function index(){
        ...
        $this->load->view('purchase_voucher/voucher_list', $data);
    }
    ...
    function purchase_credit_ledger(){
        ...
        $data=array_merge($data,$section_modules);
        $data['currency'] = $this->currency_call();
        $data['reference_type'] = 'purchase_credit_note';
        $this->load->view('purchase_voucher/voucher_list', $data);
    }
    ...
    function purchase_debit_ledger(){
        ...
        $data=array_merge($data,$section_modules);
        $data['currency'] = $this->currency_call();
        $data['reference_type'] = 'purchase_debit_note';
        $this->load->view('purchase_voucher/voucher_list', $data);
    }
    ...
    function GetAllVoucher(){
        ...
        $sql = "SELECT purchase_voucher_id FROM `purchase_voucher` ";
        if($voucher_id != 0 && $voucher_id != ''){
            $sql .= " WHERE purchase_voucher_id='{$voucher_id}' ";
        }else{
            $sql .= " WHERE branch_id='{$branch_id}' AND voucher_date BETWEEN '{$from_date}' AND '{$to_date}' AND reference_type='{$reference_type}' ";
        }

        $sub_ary = array();
        $extraWhere = '';
        /*if($voucher_id != '0') $sql .= " AND purchase_voucher_id = '{$voucher_id}' ";*/
        $reference_id = $this->encryption_url->decode($this->input->post('reference_id'));
        if(null != $reference_id){
            $sub_ary[] = "purchase_voucher_id = '".$reference_id."'";
        }
        ...
        if(!empty($sub_ary)){
            ...
            $extraWhere = " AND purchase_voucher_id IN (SELECT purchase_voucher_id FROM accounts_purchase_voucher WHERE {$where})";
        }
        $sql .= $extraWhere;
        ...
        $sql .= " ORDER BY purchase_voucher_id DESC LIMIT $end,$limit"; 
        $qry = $this->db->query($sql);
        $vids_ary = array();
        if($qry->num_rows() > 0){
            $vids = $qry->result_array();
            foreach ($vids as $key => $value) {
                array_push($vids_ary, $value['purchase_voucher_id']);
            }
        }
        ...
        if(!empty($vids_ary)){
            $this->db->select('ts.voucher_number,ts.reference_id,ts.purchase_voucher_id,ts.voucher_date,ts.receipt_amount,ts.reference_number,ts.delete_status,ts.description,tsa.ledger_id,tsa.voucher_amount,tsa.dr_amount,tsa.cr_amount,tsa.accounts_purchase_id');
            $this->db->where_in('ts.purchase_voucher_id',$vids_ary);
            //$this->db->where_in('ts.purchase_voucher_id',array('28'));
            $this->db->where('ledger_id != ','');
            $this->db->where('tsa.delete_status','0');
            $this->db->order_by('ts.purchase_voucher_id', 'DESC');
            $this->db->join('accounts_purchase_voucher tsa','ts.purchase_voucher_id=tsa.purchase_voucher_id','left');
            $getAll = $this->db->get('purchase_voucher ts');
        ...
        if(!empty($response)){
            ...
            foreach ($response as $key => $value) {
                ...
                $vid = $value['purchase_voucher_id'];
                $accounts_voucher_id = ($value['accounts_purchase_id'] != '' ? $value['accounts_purchase_id']: 0);
                ...
                if(@$resp['voucher_'.$vid]){
                    ...
                    $colspan = 3;
                    /*if($this->voucher_type == 'purchase' || $this->voucher_type == 'purchase')*/ 
                    $colspan = 5;
                ...
                //    $tr .= "<td><input data-id='{$vid}' type='text' value='' class='disable_in form-control' name='invoice_narration'></td>";
                    ...
                    $tr .= "<td>{$Cr_html}</td>";
                    /*if($this->access['m_update']){*/
                        $reference_id = $purchase_id = $this->encryption_url->encode($value['reference_id']);
                        if($value['delete_status'] == '0'){
                            /*$tr .= "<td><a href='". base_url('purchase/edit/') .$reference_id."' class='edit_led' data-id='".$vid."'><i class='glyphicon glyphicon-edit'></i></a> ";*///| <a href='javascript:void(0);' data-id='".$vid."' class='sub_led'><i class='glyphicon glyphicon-check'></i></a></td>
                        }else{
                            /*$tr .= "<td></td>";*/
                        }
                    /*}else{
