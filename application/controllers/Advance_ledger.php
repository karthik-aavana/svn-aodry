...
class Advance_ledger extends MY_Controller
{
   ...
   function __construct()
    ...
    function index(){
        ...
        $this->load->view('advance_voucher/voucher_list', $data);
    }
    ...
    function GetAllVoucher(){
        ...
        $sql = "SELECT advance_voucher_id FROM `advance_voucher` ";
        if($voucher_id != 0 && $voucher_id != ''){
            $sql .= " WHERE advance_voucher_id='{$voucher_id}' ";
        }else{
        ...
        $sub_ary[] = " delete_status = '0'";
        if(null != $this->input->post('invoice_status')){
        }

        if(!empty($sub_ary)){
            $where = implode(' AND ', $sub_ary);
            $extraWhere = " AND advance_voucher_id IN (SELECT advance_voucher_id FROM accounts_advance_voucher WHERE {$where})";
        }
        $sql .= $extraWhere;
        ...
        $sql .= " ORDER BY advance_voucher_id DESC LIMIT $end,$limit"; 
        $qry = $this->db->query($sql);
        $vids_ary = array();
        if($qry->num_rows() > 0){
            $vids = $qry->result_array();
            foreach ($vids as $key => $value) {
                array_push($vids_ary, $value['advance_voucher_id']);
            }
        }
        ...
        if(!empty($vids_ary)){
            $this->db->select('ts.voucher_number,ts.reference_id,ts.advance_voucher_id,ts.voucher_date,ts.receipt_amount,ts.reference_number,ts.delete_status,ts.description,tsa.ledger_id,tsa.voucher_amount,tsa.dr_amount,tsa.cr_amount,tsa.accounts_advance_id');
            $this->db->where_in('ts.advance_voucher_id',$vids_ary);
            $this->db->where('ledger_id != ','');
            $this->db->where('tsa.delete_status','0');
            $this->db->order_by('ts.advance_voucher_id', 'DESC');
            $this->db->join('accounts_advance_voucher tsa',
                'ts.advance_voucher_id=tsa.advance_voucher_id','left');
            $getAll = $this->db->get('advance_voucher ts');
        ...
        if(!empty($response)){
            ...
            foreach ($response as $key => $value) {
                ...
                $vid = $value['advance_voucher_id'];
                $accounts_voucher_id = ($value['accounts_advance_id'] != '' ? $value['accounts_advance_id']: 0);
