...
class Refund_ledger extends MY_Controller
{
   ...
   function __construct()
    ...
    function index(){
        ...
        $this->load->view('refund_voucher/voucher_list', $data);
    }
    ...
    function GetAllVoucher(){
        ...
        $sql = "SELECT refund_id FROM `refund_voucher` ";
        if($voucher_id != 0 && $voucher_id != ''){
            $sql .= " WHERE refund_id='{$voucher_id}' ";
        }else{
        ...
        if(!empty($sub_ary)){
            ...
            $extraWhere = " AND refund_id IN (SELECT refund_voucher_id FROM accounts_refund_voucher WHERE {$where})";
        }
        $sql .= $extraWhere;
        ...
        $sql .= " ORDER BY refund_id DESC LIMIT $end,$limit"; 
        $qry = $this->db->query($sql);
        $vids_ary = array();
        if($qry->num_rows() > 0){
            $vids = $qry->result_array();
            foreach ($vids as $key => $value) {
                array_push($vids_ary, $value['refund_id']);
            }
        }
        ...
        if(!empty($vids_ary)){
            $this->db->select('ts.voucher_number,ts.reference_id,ts.refund_id,ts.voucher_date,ts.receipt_amount,ts.reference_number,ts.delete_status,ts.description,tsa.ledger_id,tsa.voucher_amount,tsa.dr_amount,tsa.cr_amount,tsa.accounts_refund_id');
            $this->db->where_in('ts.refund_id',$vids_ary);
            $this->db->where('ledger_id != ','');
            $this->db->order_by('ts.refund_id', 'DESC');
            $this->db->join('accounts_refund_voucher tsa',
                'ts.refund_id=tsa.refund_voucher_id','left');
            $getAll = $this->db->get('refund_voucher ts');
        ...
        if(!empty($response)){
            ...
            foreach ($response as $key => $value) {
                ...
                $vid = $value['refund_id'];
                $accounts_voucher_id = ($value['accounts_refund_id'] != '' ? $value['accounts_refund_id']: 0);
