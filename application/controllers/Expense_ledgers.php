...
class Expense_ledgers extends MY_Controller
{
    ...
    function index(){
        ...
        $this->load->view('expense_voucher/voucher_list', $data);
    }
    function GetAllVoucher(){
        ...
        $sql = "SELECT expense_voucher_id FROM `expense_voucher` ";
        if($voucher_id != 0 && $voucher_id != ''){
            $sql .= " WHERE expense_voucher_id='{$voucher_id}' ";
        }else{
            $sql .= " WHERE branch_id='{$branch_id}' AND voucher_date BETWEEN '{$from_date}' AND '{$to_date}' AND reference_type='expense_bill' ";
        }
        $sub_ary = array();
        $extraWhere = '';
        /*if($voucher_id != '0') $sql .= " AND expense_voucher_id = '{$voucher_id}' ";*/
        $reference_id = $this->encryption_url->decode($this->input->post('reference_id'));
        if(null != $reference_id){
            $sub_ary[] = "expense_voucher_id = '".$reference_id."'";
        }
        ...
        if(!empty($sub_ary)){
            ...
            $extraWhere = " AND expense_voucher_id IN (SELECT expense_voucher_id FROM accounts_expense_voucher WHERE {$where})";
        }
        $sql .= $extraWhere;
        ...
        $sql .= " ORDER BY expense_voucher_id DESC LIMIT $end,$limit"; 
        $qry = $this->db->query($sql);
        $vids_ary = array();
        if($qry->num_rows() > 0){
            $vids = $qry->result_array();
            foreach ($vids as $key => $value) {
                array_push($vids_ary, $value['expense_voucher_id']);
            }
        }
        ...
        if(!empty($vids_ary)){
            $this->db->select('ts.voucher_number,ts.reference_id,ts.expense_voucher_id,ts.voucher_date,ts.receipt_amount,ts.reference_number,ts.delete_status,ts.description,tsa.ledger_id,tsa.voucher_amount,tsa.dr_amount,tsa.cr_amount,tsa.accounts_expense_id');
            $this->db->where_in('ts.expense_voucher_id',$vids_ary);
            //$this->db->where_in('ts.expense_voucher_id',array('28'));
            $this->db->where('ledger_id != ','');
            $this->db->where('tsa.delete_status','0');
            $this->db->order_by('ts.expense_voucher_id', 'DESC');
            $this->db->join('accounts_expense_voucher tsa','ts.expense_voucher_id=tsa.expense_voucher_id','left');
            $getAll = $this->db->get('expense_voucher ts');
        ...
        if(!empty($response)){
            ...
            foreach ($response as $key => $value) {
                ...
                $vid = $value['expense_voucher_id'];
                $accounts_voucher_id = ($value['accounts_expense_id'] != '' ? $value['accounts_expense_id']: 0);
                ...
                if(@$resp['voucher_'.$vid]){
                    ...
                    $colspan = 3;
                    /*if($this->voucher_type == 'expense' || $this->voucher_type == 'expense')*/ 
                    $colspan = 5;
                ...
                //    $tr .= "<td><input data-id='{$vid}' type='text' value='' class='disable_in form-control' name='invoice_narration'></td>";
                    ...
                    $tr .= "<td>{$Cr_html}</td>";
                    /*if($this->access['m_update']){*/
                        $reference_id = $expense_id = $this->encryption_url->encode($value['reference_id']);
                        if($value['delete_status'] == '0'){
                            /*$tr .= "<td><a href='". base_url('expense/edit/') .$reference_id."' class='edit_led' data-id='".$vid."'><i class='glyphicon glyphicon-edit'></i></a> ";*///| <a href='javascript:void(0);' data-id='".$vid."' class='sub_led'><i class='glyphicon glyphicon-check'></i></a></td>
                        }else{
                            /*$tr .= "<td></td>";*/
                        }
                    /*}else{
