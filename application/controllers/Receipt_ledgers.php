...
class Receipt_ledgers extends MY_Controller
{
    ...
    function index(){
        ...
        $this->load->view('receipt_voucher/ledgers', $data);
    }
    ...
    function GetAllVoucher(){
        ...
        $sql = "SELECT receipt_id FROM `receipt_voucher` ";
        if($voucher_id != 0 && $voucher_id != ''){
            $sql .= " WHERE receipt_id='{$voucher_id}' ";
        }else{
        ...
        $sub_ary = array();
        $extraWhere = '';
        /*if($voucher_id != '0') $sql .= " AND receipt_id = '{$voucher_id}' ";*/
        ...
        if(!empty($sub_ary)){
            ...
            $extraWhere = " AND receipt_id IN (SELECT receipt_voucher_id FROM accounts_receipt_voucher WHERE {$where})";
        }
        $sql .= $extraWhere;
        ...
        $sql .= " ORDER BY receipt_id DESC LIMIT $end,$limit"; 
        $qry = $this->db->query($sql);
        $vids_ary = array();
        if($qry->num_rows() > 0){
            $vids = $qry->result_array();
            foreach ($vids as $key => $value) {
                array_push($vids_ary, $value['receipt_id']);
            }
        }
        ...
        if(!empty($vids_ary)){
            $this->db->select('ts.voucher_number,ts.reference_id,ts.receipt_id,ts.voucher_date,ts.receipt_amount,ts.reference_number,ts.delete_status,ts.description,tsa.ledger_id,tsa.voucher_amount,tsa.dr_amount,tsa.cr_amount,tsa.accounts_receipt_id');
            $this->db->where_in('ts.receipt_id',$vids_ary);
            //$this->db->where_in('ts.receipt_id',array('28'));
            $this->db->where('ledger_id != ','');
            $this->db->where('tsa.delete_status','0');
            $this->db->order_by('ts.receipt_id', 'DESC');
            $this->db->join('accounts_receipt_voucher tsa','ts.receipt_id=tsa.receipt_voucher_id','left');
            $getAll = $this->db->get('receipt_voucher ts');
        ...
        if(!empty($response)){
            ...
            foreach ($response as $key => $value) {
                ...
                $vid = $value['receipt_id'];
                $accounts_receipt_id = ($value['accounts_receipt_id'] != '' ? $value['accounts_receipt_id']: 0);
                /* Ledger dropdown with selected option */
                $ledger_html = "<input type='hidden' value='{$accounts_receipt_id}' data-id='{$vid}' name='accounts_receipt_id'><select data-id='{$vid}' class='form-control js-example-basic-single disable_in' name='ledger_id'><option value=''>Select Ledger</option>";
                ...
                if(@$resp['voucher_'.$vid]){
                    ...
                    if($resp['voucher_'.$vid] >= 2){
                        ...
                        if($value['delete_status'] == '0'){ 
                            $action .=" | <a href='javascript:void(0);' aid='".$accounts_receipt_id."' class='remove_led' data-id='".$vid."'><i class='icon-minus'></i></a>";
                        }
                    }else{
                        $tr .=  '<tr data-id="'.$vid.'">';
                    }
                    ...
                    $tr .= "<td><a href='".base_url('receipt_voucher/view_details/').$this->encryption_url->encode($value['receipt_id'])."'>{$value['voucher_number']}</a></td>";
                    ...
                    /*if($this->voucher_type == 'purchase' || $this->voucher_type == 'sales')*/
                    $reference_numbers = $value['reference_number'];
                    if($value['reference_number'] != ''){
                        $reference_number = explode(',', $value['reference_number']);
                        $reference_numbers = implode("<br>", $reference_number); 
                    }
                    $tr .= "<td><input data-id='{$vid}' type='hidden' value='{$value['reference_number']}' class='disable_in form-control' name='invoice_number'>{$reference_numbers}</td>";
                    ...
                    /*if($this->access['m_update']){*/
                        ...
                        if($value['delete_status'] == '0'){
                            $tr .= "<td><a href='". base_url('receipt_voucher/edit/') .$this->encryption_url->encode($value['receipt_id'])."' class='edit_led' data-id='".$vid."'><i class='glyphicon glyphicon-edit'></i></a> ";//| <a href='javascript:void(0);' data-id='".$vid."' class='sub_led'><i class='glyphicon glyphicon-check'></i></a></td>
                        }else{
                            $tr .= "<td></td>";
                        }*/
                    /*}else{
