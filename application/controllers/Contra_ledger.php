...
class Contra_ledger extends MY_Controller{
   ...
   function __construct(){
    ...
    function index(){
        $contra_voucher_module         = $this->config->item('contra_voucher_module');
        $data['module_id']               = $contra_voucher_module;
        $modules                         = $this->modules;
        $privilege                       = "view_privilege";
        $data['privilege']               = $privilege;
        $section_modules                 = $this->get_section_modules($contra_voucher_module, $modules, $privilege);
        ...
        $this->load->view('contra_voucher/voucher_list', $data);
    }
    ...
    function GetAllVoucher(){
        ...
        $sql = "SELECT contra_voucher_id FROM `contra_voucher` ";
        if($voucher_id != 0 && $voucher_id != ''){
            $sql .= " WHERE contra_voucher_id='{$voucher_id}' ";
        }else{
        ...
        $sub_ary = array();
        $extraWhere = '';
        /*if($voucher_id != '0') $sql .= " AND advance_voucher_id = '{$voucher_id}' ";*/
        ...
        if(!empty($sub_ary)){
            ...
            $extraWhere = " AND contra_voucher_id IN (SELECT contra_voucher_id FROM accounts_contra_voucher WHERE {$where})";
        }
        $sql .= $extraWhere;
        ...
        $sql .= " ORDER BY contra_voucher_id DESC LIMIT $end,$limit"; 
        ...
        if($qry->num_rows() > 0){
            ...
            foreach ($vids as $key => $value) {
                array_push($vids_ary, $value['contra_voucher_id']);               
            }
        }
        ...
        if(!empty($vids_ary)){
            $this->db->select('ts.voucher_number,ts.reference_id,ts.contra_voucher_id,ts.voucher_date,ts.receipt_amount,ts.reference_number,ts.delete_status,ts.description,tsa.ledger_id,tsa.voucher_amount,tsa.dr_amount,tsa.cr_amount,tsa.accounts_contra_id');
            $this->db->where_in('ts.contra_voucher_id',$vids_ary);
            $this->db->where('ledger_id != ','');
            $this->db->where('tsa.delete_status',0);
            $this->db->order_by('ts.contra_voucher_id', 'DESC');
            $this->db->join('accounts_contra_voucher tsa',
                'ts.contra_voucher_id=tsa.contra_voucher_id','left');
            $getAll = $this->db->get('contra_voucher ts');
        ...
        if(!empty($response)){
            ...
            foreach ($response as $key => $value) {
                ...
                $vid = $value['contra_voucher_id'];
                $accounts_voucher_id = ($value['accounts_contra_id'] != '' ? $value['accounts_contra_id']: 0);
