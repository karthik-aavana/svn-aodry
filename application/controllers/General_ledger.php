...
class General_ledger extends MY_Controller
{
   ...
   function __construct()
    ...
    function index(){
        $general_voucher_module         = $this->config->item('general_voucher_module');
        $data['module_id']               = $general_voucher_module;
        $modules                         = $this->modules;
        $privilege                       = "view_privilege";
        $data['privilege']               = $privilege;
        $section_modules                 = $this->get_section_modules($general_voucher_module, $modules, $privilege);
        ...
        $this->load->view('general_voucher/voucher_list', $data);
    }
    ...
    function GetAllVoucher(){
        ...
        $sql = "SELECT general_voucher_id FROM `general_voucher` ";
        if($voucher_id != 0 && $voucher_id != ''){
            $sql .= " WHERE general_voucher_id='{$voucher_id}' ";
        }else{
        ...
        if(!empty($sub_ary)){
            ...
            $extraWhere = " AND general_voucher_id IN (SELECT general_voucher_id FROM accounts_general_voucher WHERE {$where})";
        }
        $sql .= $extraWhere;
        ...
        $sql .= " ORDER BY general_voucher_id DESC LIMIT $end,$limit"; 
        ...
        if($qry->num_rows() > 0){
            ...
            foreach ($vids as $key => $value) {
                array_push($vids_ary, $value['general_voucher_id']);               
            }
        }
        ...
        if(!empty($vids_ary)){
            $this->db->select('ts.voucher_number,ts.reference_id,ts.general_voucher_id,ts.voucher_date,ts.receipt_amount,ts.reference_number,ts.delete_status,ts.description,tsa.ledger_id,tsa.voucher_amount,tsa.dr_amount,tsa.cr_amount,tsa.accounts_general_id');
            $this->db->where_in('ts.general_voucher_id',$vids_ary);
            $this->db->where('ledger_id != ','');
            $this->db->where('tsa.delete_status',0);
            $this->db->order_by('ts.general_voucher_id', 'DESC');
            $this->db->join('accounts_general_voucher tsa',
                'ts.general_voucher_id=tsa.general_voucher_id','left');
            $getAll = $this->db->get('general_voucher ts');
