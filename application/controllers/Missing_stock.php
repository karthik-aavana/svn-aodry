...
class Missing_stock extends MY_Controller {
    ...
    public function index() {
        ...
        $data['products'] = $this->general_model->get_product_stock('missing');
        ...
        if (!empty($this->input->post())) {
            ...
            $list_data = $this->common->get_product_stage_stock('missing');
            ...
            if (!empty($posts)) {
                ...
                foreach ($posts as $post) {
                    ...
                    $remaining = $missing - ($damaged + $fixed);
                    ...
                    $nestedData['history'] = '<a href="#" data-backdrop="static" data-keyboard="false" data-toggle="modal" data-target="#view_product" data-id="' . $product_id . '" class="view_missing"><i class="fa fa-eye"></i> View</a>';
                    $cols = '';

                    $cols .= '<a href="#" class="edit_fixed_stock" data-backdrop="static" data-keyboard="false" data-toggle="modal" data-target="#edit_fixed_stock"  data-id="' . $product_id . '"><i class="fa fa-eye"></i> View</a>';
            ...
            $this->load->view('missing_stock/list', $data);
        }
    }

    public function add_fixed_product() {
        ...
        if (isset($_FILES["attachment"]["name"]) && $_FILES["attachment"]["name"] != "") {
            ...
            if (!is_dir('assets/missing_attachment/' . $this->session->userdata('SESS_BRANCH_ID'))) {
                mkdir('./assets/missing_attachment/' . $this->session->userdata('SESS_BRANCH_ID'), 0777, TRUE);
            } $url = "assets/missing_attachment/" . $this->session->userdata('SESS_BRANCH_ID') . "/" . $image_path;
        ...
        if($damage_id = $this->general_model->insertData('product_damaged', $damage_data)){
            $successMsg = 'Found/Damage Stock Reported Successfully';
            $this->session->set_flashdata('missing_stock_success',$successMsg);
            $log_data = array(
            'user_id' => $this->session->userdata('SESS_USER_ID'),
            'table_id' => $damage_id,
            'table_name' => 'product_damaged',
        ...
        if ($damage_id) {
            ...
            if ($cmb_move_product == 'damaged') {
                ...
                if ($this->general_model->updateData('products', ['product_missing_quantity' => $final_missing, 'product_damaged_quantity' => $final_damage], $where)) {
                    $res['flag'] = true;
                    $res['msg'] = 'Found/Damage Stock Reporting Successful Added.'; 
                    $this->session->set_flashdata('missing_stock_success',$successMsg);
                } else {
                    $errorMsg = 'Found/Damage Stock Reported Unsuccessful';
                    $res['flag'] = false;              
                    $res['msg'] = 'Found/Damage Stock Reported Unsuccessful'; 
                    $this->session->set_flashdata('missing_stock_error',$errorMsg); 
                }
            } else {
                $final_stock = $quantity + $txt_quantity;
                if ($this->general_model->updateData('products', ['product_quantity' => $final_stock, 'product_missing_quantity' => $final_missing], $where)) {
                    $res['flag'] = true;
                    $res['msg'] = 'Found/Damage Stock Reporting Successful Added.'; 
                    $this->session->set_flashdata('missing_stock_success',$successMsg);
                } else {
    ...
    public function product_details() {
        ...
        $product_data = $this->general_model->get_product_missing_products($id);
        $send = array();
            ...
            foreach ($product_data as $com) {
                ...
                $nestedData['movement'] = '<select class="form-control disable_in" id="movement_' . $product_damage_id . '" name="movement" tabindex="-1" aria-hidden="true" data-id="' . $product_damage_id . '">
                                        ...
                                        <option value="damaged" ' . $selected_damaged . '>Damaged</option>
                                         <option value="found" ' . $selected_missing . '>Found</option>
                                    </select>';
    ...
    public function update_missing_stock() {
        ...
        if($damage_id = $this->general_model->updateData('product_damaged', $damage_data, $where_damage)){
            $successMsg = 'Missing Report Updated successfully';
            $this->session->set_flashdata('missing_stock_success',$successMsg);
            $log_data = array(
            'user_id' => $this->session->userdata('SESS_USER_ID'),
            'table_id' => $damage_id,
        ...
        if ($damage_id) {
            ...
            if ($result) {
                $res['flag'] = true;
                $res['msg'] = 'Found/Damage Stock Update Successful.'; 
                $successMsg  = 'Found/Damage Stock Update Successful.'; 
                $this->session->set_flashdata('missing_stock_success',$successMsg);
            } else {                
                $errorMsg = 'Found/Damage Stock Update Unsuccessful';
                $res['flag'] = false;              
                $res['msg'] = 'Found/Damage Stock Update Unsuccessful'; 
                $this->session->set_flashdata('missing_stock_error',$errorMsg);
            }
        } else {
            $errorMsg = 'Found/Damage Stock Update Unsuccessful';
    ...
    public function missing_history($id) {
        $missing_stock_module_id        = $this->config->item('missing_stock_module');
        $data['missing_stock_module_id'] = $missing_stock_module_id;
        $modules                = $this->modules;
        $privilege              = "view_privilege";
        $data['privilege']      = $privilege;
        $section_modules        = $this->get_section_modules($missing_stock_module_id , $modules , $privilege);
        $list_data = $this->common->get_product_stock_missing_history($id);
        $list_data['search'] = 'all';
