...
class Sales_credit_note extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $data['sales_credit_note_voucher']   = $this->config->item('sales_credit_note_voucher');
        ...
        {
            $columns = array(
                0 => 'cr.sales_credit_note_id',
                1 => 'cr.sales_credit_note_date',
                2 => 'cr.sales_credit_note_invoice_number',
                3 => 'c.customer_name',
                5 => 'cr.sales_credit_note_grand_total');
            $limit               = $this->input->post('length');
            $start               = $this->input->post('start');
            $order               = $columns[$this->input->post('order')[0]['column']];
            $dir                 = $this->input->post('order')[0]['dir'];
            $list_data           = $this->common->sales_credit_note_list_field($order, $dir);
            ...
            if (!empty($posts))
            {
                ...
                foreach ($posts as $post)
                {
                    ...
                    $nestedData['grand_total'] = $currency_symbol . ' ' . $this->precise_amount($post->sales_credit_note_grand_total, $access_common_settings[0]->amount_precision) . ' (INV)';
                    ...
                    if (in_array($data['sales_credit_note_module_id'] , $data['active_view']))
                    {
                        $cols .= '<span><a class="btn btn-app" data-toggle="tooltip" data-placement="bottom" title="View Sales Credit Note" href="' . base_url('sales_credit_note/view/') . $sales_credit_note_id.'"><i class="fa fa-eye"></i></a></span>';
                    }
                    if (in_array($data['sales_credit_note_module_id'] , $data['active_edit']))
                    {
                    $cols .= '<span><a class="btn btn-app" data-toggle="tooltip" data-placement="bottom" title="Edit Sales Credit Note" href="' . base_url('sales_credit_note/edit/') . $sales_credit_note_id.'"><i class="fa fa-pencil"></i></a></span>';
                    }
                    ...
                    if (in_array($data['sales_credit_note_module_id'] , $data['active_view']))
                    {
                        $cols .= '<span data-backdrop="static" data-keyboard="false" data-toggle="modal" data-target="#pdf_type_modal"><a class="btn btn-app pdf_button" data-toggle="tooltip" data-placement="bottom" title="Download PDF"  b_curr="'.$this->session->userdata('SESS_DEFAULT_CURRENCY').'"  b_code="'.$data['currency_code'].'" c_code="'.$customer_curr_code.'" c_curr="'.$post->currency_id.'" data-id="' . $sales_credit_note_id . '" data-name="regular" href1="' . base_url('sales_credit_note/pdf/') . $sales_credit_note_id . '" href="javascript:void(0);"><i class="fa fa-file-pdf-o"></i></a></span>';
                    }
                    /*if (in_array($sales_credit_note_module_id, $data['active_view']))
                    {
                        if (in_array($data['email_sub_module_id'], $data['access_sub_modules']))
                        {
                            $cols .= '<span><a class="btn btn-app" data-toggle="tooltip" data-placement="bottom" title="Email sales credit note" href="' . base_url('sales_credit_note/email/') . $sales_credit_note_id . '"><i class="fa fa-envelope-o"></i></a></span>';
                        }
                    }*/
                    if ($post->currency_id != $this->session->userdata('SESS_DEFAULT_CURRENCY'))
                    {
                        /*$cols .= '<span data-backdrop="static" data-keyboard="false" class="convert_currency" data-toggle="modal" data-target="#convert_currency_modal" data-id="' . $sales_credit_note_id . '" data-path="sales_credit_note/convert_currency" data-currency_code="' . $post->currency_code . '" data-grand_total="' . $post->sales_credit_note_grand_total . '" ><a class="btn btn-app" data-toggle="tooltip" data-placement="bottom" href="#" title="Convert Currency"><i class="fa fa-money"></i></a></span>';*/
                        ...
                        $cols .= '<span data-backdrop="static" data-keyboard="false" data-toggle="modal" data-target="#convert_currency_modal"><a href="javascript:void(0);" class="btn btn-app convert_currency" data-id="' . $sales_credit_note_id . '" data-path="sales_credit_note/convert_currency" data-conversion_date="'.$conversion_date.'" data-currency_code="' . $post->currency_code . '" data-grand_total="' . $this->precise_amount($post->sales_credit_note_grand_total, $access_common_settings[0]->amount_precision) . '" data-rate="' . $this->precise_amount($post->currency_converted_rate, $access_common_settings[0]->amount_precision) . '" data-toggle="tooltip" data-placement="bottom" title="Convert Currency"><i class="fa fa-exchange"></i></a></span>';
                    }

                    $this->db->select('sales_voucher_id');
                    $this->db->from('sales_voucher');
                    $this->db->where('reference_id',$post->sales_credit_note_id);
                    $this->db->where('delete_status',0);
                    $this->db->where('reference_type','sales_credit_note');
                    $get_sv_qry = $this->db->get();
                    $ref_id = $get_sv_qry->result();
                    $sales_cn_voucher_id = '';
                   if(!empty($ref_id)){
                        $sales_cn_voucher_id = $ref_id[0]->sales_voucher_id;
                    }

                    if($sales_cn_voucher_id != ''){
                        $sales_cn_voucher_id = $this->encryption_url->encode($sales_cn_voucher_id);
                        /* $nestedData['sales_cn_voucher_view'] = ' <a href="' .base_url('sales_voucher/view_details/') . $sales_cn_voucher_id.'" target="_blank">' . '<i class="fa fa-file" aria-hidden="true" title="Voucher View"></i>' . '</a>'. '  ' .' <form  action="' .base_url('sales_ledger').'" method="POST" target="_blank"><input type="hidden" name="reference_id" value="'.$sales_cn_voucher_id.'"><button type="submit">' . '<i class="fa fa-file" aria-hidden="true" title="Ledger View"></i></button></form>';*/
                        if(in_array($data['sales_credit_note_voucher'], $data['active_view'])){
                            $cols .= '<span><a href="' .base_url('sales_voucher/view_details/') . $sales_cn_voucher_id.'" target="_blank" class="btn btn-app" data-toggle="tooltip" data-placement="bottom" title="View Voucher"><i class="fa fa-eye"></i></a></span>';

                            $cols .= '<span><form action="' .base_url('sales_credit_ledgers').'" method="POST" target="_blank"><input type="hidden" name="reference_id" value="'.$sales_cn_voucher_id.'"><a href="javascript:void(0)" data-toggle="tooltip" data-placement="bottom" class="btn btn-app" title="View Ledger"><button type="submit" class="sales_action">' . '<i class="fa fa-eye" aria-hidden="true"></i></button></a></form></span>';
                        }
                    }
                    if (in_array($sales_credit_note_module_id, $data['active_delete']))
                    {
                    ...
                    $e_way_bill_date = '';
                        if($post->sales_credit_note_e_way_bill_date != '' && $post->sales_credit_note_e_way_bill_date != '0000-00-00'){
        ...
        else
        {
            ...
            $this->load->view('sales_credit_note/list', $data);
        }
    ...
    public function add()
    {
        ...
        $table_name             = $this->config->item('sales_credit_note_table');
        ...
        $this->load->view('sales_credit_note/add', $data);
    }
    ...
    public function get_sales_credit_note_suggestions($term, $inventory_advanced, $item_access)
    {
        $sales_credit_note_module_id = $this->config->item('sales_credit_note_module');
        $modules                     = $this->modules;
        $privilege                   = "add_privilege";
    ...
    public function add_sales_credit_note()
    {
        $data                        = $this->get_default_country_state();
        $sales_credit_note_module_id = $this->config->item('sales_credit_note_module');
        $module_id                   = $sales_credit_note_module_id;
        $modules                     = $this->modules;
        $privilege                   = "add_privilege";
        $section_modules             = $this->get_section_modules($sales_credit_note_module_id, $modules, $privilege);
        /* presents all the needed */
        ...
        $sales_credit_note_data = array(
            "sales_credit_note_date"                    => date('Y-m-d',strtotime($this->input->post('invoice_date'))),
            "sales_id"                                  => $this->input->post('sales_invoice_number'),
            "sales_credit_note_invoice_number"          => $invoice_number,
            "sales_credit_note_sub_total"               => (float) $this->input->post('total_sub_total') ? (float) $this->input->post('total_sub_total') : 0,
            "sales_credit_note_grand_total"             => $this->input->post('total_grand_total') ? (float) $this->input->post('total_grand_total') : 0,
            "sales_credit_note_discount_amount"         => $this->input->post('total_discount_amount') ? (float) $this->input->post('total_discount_amount') : 0,
            "sales_credit_note_tax_amount"              => $this->input->post('total_tax_amount') ? (float) $this->input->post('total_tax_amount') : 0,
            "sales_credit_note_taxable_value"           => $this->input->post('total_taxable_amount') ? (float) $this->input->post('total_taxable_amount') : 0,
            "sales_credit_note_gst_payable"              => $this->input->post('gst_payable') ,
            "sales_credit_note_tds_amount"              => $this->input->post('total_tds_amount') ? (float) $this->input->post('total_tds_amount') : 0,
            "sales_credit_note_tcs_amount"              => $this->input->post('total_tcs_amount') ? (float) $this->input->post('total_tcs_amount') : 0,
            "sales_credit_note_igst_amount"             => 0,
            "sales_credit_note_cgst_amount"             => 0,
            "sales_credit_note_sgst_amount"             => 0,
            "from_account"                              => 'sales',
            "to_account"                                => 'customer',
            "financial_year_id"                         => $this->session->userdata('SESS_FINANCIAL_YEAR_ID'),
            "sales_credit_note_party_id"                => $this->input->post('customer'),
            "sales_credit_note_ship_to_customer_id"     => $this->input->post('ship_to'),
            "sales_credit_note_party_type"              => "customer",
            "sales_credit_note_nature_of_supply"        => $this->input->post('nature_of_supply'),
            "sales_credit_note_customer_invoice_number" => $this->input->post('customer_ref'),
            "sales_credit_note_customer_date"           => date('Y-m-d',strtotime($this->input->post('customer_date'))),
            "sales_credit_note_delivery_challan_number" => $this->input->post('delivery_challan_number'),
            "sales_credit_note_delivery_date"           => date('Y-m-d',strtotime($this->input->post('delivery_date'))),
            "sales_credit_note_e_way_bill_number"       => $this->input->post('e_way_bill'),
            "sales_credit_note_type_of_supply"          => $this->input->post('type_of_supply'),
            "sales_credit_note_billing_country_id"      => $this->input->post('billing_country'),
            "sales_credit_note_billing_state_id"        => $this->input->post('billing_state'),
        ...
        $sales_credit_note_data['freight_charge_tax_id']         = $this->input->post('freight_charge_tax_id') ? (float) $this->input->post('freight_charge_tax_id') : 0;
        $sales_credit_note_data['insurance_charge_tax_id']       = $this->input->post('insurance_charge_tax_id') ? (float) $this->input->post('insurance_charge_tax_id') : 0;
        $sales_credit_note_data['packing_charge_tax_id']         = $this->input->post('packing_charge_tax_id') ? (float) $this->input->post('packing_charge_tax_id') : 0;
        $sales_credit_note_data['incidental_charge_tax_id']      = $this->input->post('incidental_charge_tax_id') ? (float) $this->input->post('incidental_charge_tax_id') : 0;
        $sales_credit_note_data['inclusion_other_charge_tax_id'] = $this->input->post('inclusion_other_charge_tax_id') ? (float) $this->input->post('inclusion_other_charge_tax_id') : 0;
        $sales_credit_note_data['exclusion_other_charge_tax_id'] = $this->input->post('exclusion_other_charge_tax_id') ? (float) $this->input->post('exclusion_other_charge_tax_id') : 0;

        $round_off_value = $sales_credit_note_data['sales_credit_note_grand_total'];
        ...
        $sales_credit_note_data['round_off_amount'] = bcsub($sales_credit_note_data['sales_credit_note_grand_total'], $round_off_value, $section_modules['access_common_settings'][0]->amount_precision);

        $sales_credit_note_data['sales_credit_note_grand_total'] = $round_off_value;

        $sales_credit_note_tax_amount = $sales_credit_note_data['sales_credit_note_tax_amount'] + (float)($this->input->post('total_other_taxable_amount'));
        ...
        if ($section_modules['access_settings'][0]->tax_type == "gst"){
            ...
            if ($sales_credit_note_data['sales_credit_note_billing_state_id'] != 0){

                if ($data['branch'][0]->branch_state_id == $sales_credit_note_data['sales_credit_note_billing_state_id']){
                    $sales_credit_note_data['sales_credit_note_igst_amount'] = 0;
                    $sales_credit_note_data['sales_credit_note_cgst_amount'] = ($sales_credit_note_tax_amount * $cgst_amount_percentage) / 100;
                    $sales_credit_note_data['sales_credit_note_sgst_amount'] = ($sales_credit_note_tax_amount * $sgst_amount_percentage) / 100;
                } else{
                    $sales_credit_note_data['sales_credit_note_igst_amount'] = $sales_credit_note_tax_amount;
                    $sales_credit_note_data['sales_credit_note_cgst_amount'] = 0;
                    $sales_credit_note_data['sales_credit_note_sgst_amount'] = 0;
                }
                $sales_credit_note_data['sales_credit_note_tax_cess_amount'] = $total_cess_amnt;
            } else {
                if ($sales_credit_note_data['sales_credit_note_type_of_supply'] == "export_with_payment"){
                    $sales_credit_note_data['sales_credit_note_igst_amount'] = $sales_credit_note_tax_amount;
                    $sales_credit_note_data['sales_credit_note_cgst_amount'] = 0;
                    $sales_credit_note_data['sales_credit_note_sgst_amount'] = 0;
                    $sales_credit_note_data['sales_credit_note_tax_cess_amount'] = $total_cess_amnt;
        ...
        {
            $sales_credit_note_data['converted_grand_total'] = $sales_credit_note_data['sales_credit_note_grand_total'];
        }
        else
        {
            $sales_credit_note_data['converted_grand_total'] = 0;
        }

        $data_main               = array_map('trim', $sales_credit_note_data);
        $sales_credit_note_table = $this->config->item('sales_credit_note_table');

        if ($sales_credit_note_id = $this->general_model->insertData($sales_credit_note_table, $data_main)){

            if ($this->session->userdata('SESS_DEFAULT_CURRENCY') == $data_main['currency_id']) {

                
            }
            $successMsg = 'Sales Credit Note Added Successfully';
            $this->session->set_flashdata('sales_cn_success',$successMsg);
            $log_data = array(
                'user_id'           => $this->session->userdata('SESS_USER_ID'),
                'table_id'          => $sales_credit_note_id,
                'table_name'        => $sales_credit_note_table,
                'financial_year_id' => $this->session->userdata('SESS_FINANCIAL_YEAR_ID'),
                'branch_id'         => $this->session->userdata('SESS_BRANCH_ID'),
                'message'           => 'sales_credit_note Inserted');
            $data_main['sales_credit_note_id'] = $sales_credit_note_id;
            $log_table                         = $this->config->item('log_table');
            $this->general_model->insertData($log_table, $log_data);
            $sales_credit_note_item_data = $this->input->post('table_data');
            $js_data                     = json_decode($sales_credit_note_item_data);
            $js_data               = array_reverse($js_data);
            $item_table                  = $this->config->item('sales_credit_note_item_table');
            /*echo "<pre>";
            print_r($js_data);exit();*/
            if (!empty($js_data)){
                $js_data1 = array();
                ...
                foreach ($js_data as $key => $value){
                    ...
                    if ($value == null || ($value->item_grand_total <= 0 && $value->item_quantity <= 0)){
                        ...
                        $item_data = array(
                            "item_id"                                => $value->item_id,
                            "item_type"                              => $value->item_type,
                            "sales_credit_note_item_quantity"        => $value->item_quantity ? (float) $value->item_quantity : 0,
                            "sales_credit_note_item_unit_price"      => (@$value->item_price ? (float) $value->item_price : 0),
                            "sales_credit_note_item_sub_total"       => (@$value->item_sub_total ? (float) $value->item_sub_total : 0),
                            "sales_credit_note_item_taxable_value"   => (@$value->item_taxable_value ? (float) $value->item_taxable_value : 0),
                            "sales_credit_note_item_discount_amount" => (@$value->item_discount_amount ? (float) $value->item_discount_amount : 0),
                            "sales_credit_note_item_discount_id"     => (@$value->item_discount_id ? (float) $value->item_discount_id : 0),
                            "sales_credit_note_item_tds_id"          => (@$value->item_tds_id ? (float) $value->item_tds_id : 0),
                            "sales_credit_note_item_tds_percentage"  => (@$value->item_tds_percentage ? (float) $value->item_tds_percentage : 0),
                            "sales_credit_note_item_tds_amount"      => (@$value->item_tds_amount ? (float) $value->item_tds_amount : 0),
                            "sales_credit_note_item_grand_total"     => (@$value->item_grand_total ? (float) $value->item_grand_total : 0),
                            "sales_credit_note_item_tax_id"          => (@$value->item_tax_id ? (float) $value->item_tax_id : 0),
                            "sales_credit_note_item_tax_cess_id"     => (@$value->item_tax_cess_id ? (float)$value->item_tax_cess_id : 0) ,
                            "sales_credit_note_item_igst_percentage" => 0,
                            "sales_credit_note_item_igst_amount"     => 0,
                            "sales_credit_note_item_cgst_percentage" => 0,
                            "sales_credit_note_item_cgst_amount"     => 0,
                            "sales_credit_note_item_sgst_percentage" => 0,
                            "sales_credit_note_item_sgst_amount"     => 0,
                            "sales_credit_note_item_tax_percentage"  => (@$value->item_tax_percentage ? (float) $value->item_tax_percentage : 0),
                            "sales_credit_note_item_tax_amount"      => (@$value->item_tax_amount ? (float) $value->item_tax_amount : 0),
                            "sales_credit_note_item_tax_cess_percentage"  =>  0 ,
                            "sales_credit_note_item_tax_cess_amount"      =>  0 ,
                            "sales_credit_note_item_description"     => (@$value->item_description ? $value->item_description : ""),
                            "debit_note_quantity"                    => 0,
                            "sales_credit_note_id"                   => $sales_credit_note_id);

                        $sales_credit_note_item_tax_amount     = $item_data['sales_credit_note_item_tax_amount'];
                        $sales_credit_note_item_tax_percentage = $item_data['sales_credit_note_item_tax_percentage'];
                        ...
                        if ($section_modules['access_settings'][0]->tax_type == "gst")
                        {
                            ...
                            if ($sales_credit_note_data['sales_credit_note_billing_state_id'] != 0)
                            {
                                ...
                                if ($data['branch'][0]->branch_state_id == $sales_credit_note_data['sales_credit_note_billing_state_id'])
                                {
                                    $item_data['sales_credit_note_item_igst_amount'] = 0;
                                    $item_data['sales_credit_note_item_cgst_amount'] = ($sales_credit_note_item_tax_amount * $cgst_amount_percentage) / 100;
                                    $item_data['sales_credit_note_item_sgst_amount'] = ($sales_credit_note_item_tax_amount * $sgst_amount_percentage) / 100;
                                    $item_data['sales_credit_note_item_tax_cess_amount'] = $item_tax_cess_amount;
                                    $item_data['sales_credit_note_item_tax_cess_percentage'] = $item_tax_cess_percentage;
                                    $item_data['sales_credit_note_item_igst_percentage'] = 0;
                                    $item_data['sales_credit_note_item_cgst_percentage'] = ($sales_credit_note_item_tax_percentage * $cgst_amount_percentage) / 100;
                                    $item_data['sales_credit_note_item_sgst_percentage'] = ($sales_credit_note_item_tax_percentage * $sgst_amount_percentage) / 100;
                                }else{
                                    $item_data['sales_credit_note_item_igst_amount'] = $sales_credit_note_item_tax_amount;
                                    $item_data['sales_credit_note_item_cgst_amount'] = 0;
                                    $item_data['sales_credit_note_item_sgst_amount'] = 0;
                                    $item_data['sales_credit_note_item_tax_cess_amount'] = $item_tax_cess_amount;
                                    $item_data['sales_credit_note_item_tax_cess_percentage'] = $item_tax_cess_percentage;
                                    $item_data['sales_credit_note_item_igst_percentage'] = $sales_credit_note_item_tax_percentage;
                                    $item_data['sales_credit_note_item_cgst_percentage'] = 0;
                                    $item_data['sales_credit_note_item_sgst_percentage'] = 0;
                                }
                        ...
                        if ($data_item['item_type'] == "product" && $value->item_quantity > 0){
                            ...
                            $history = array(
                                "item_id"          => $value->item_id,
                                "item_type"        => 'product',
                                "reference_id"     => $sales_credit_note_id,
                ...
                if (in_array($data['accounts_module_id'], $section_modules['active_add'])){
                    ...
                    {
                        $action = "add";
                        $this->sales_credit_note_voucher_entry($data_main, $js_data1, $action, $data['branch']);
                    }
        ...
        {
            $errorMsg = 'Sales Credit Note Add Unsuccessful';
            $this->session->set_flashdata('sales_cn_error',$errorMsg);
    ...
    public function sales_credit_note_voucher_entry($data_main, $js_data, $action, $branch)
    {
        $sales_voucher_module_id = $this->config->item('sales_credit_note_module');
        ...
        $vouchers = $this->sales_credit_note_vouchers($section_modules, $data_main, $js_data, $branch,$action);
        /*echo "<pre>";
        print_r($vouchers);*/
        $grand_total = $data_main['sales_credit_note_grand_total'];
        /*if ($data_main['sales_credit_note_gst_payable'] != "yes"){
        }else {
            $total_tax_amount = ($data_main['sales_credit_note_tax_amount'] + $data_main['freight_charge_tax_amount'] + $data_main['insurance_charge_tax_amount'] + $data_main['packing_charge_tax_amount'] + $data_main['incidental_charge_tax_amount'] + $data_main['inclusion_other_charge_tax_amount'] - $data_main['exclusion_other_charge_tax_amount']);
            $grand_total      = bcsub($data_main['sales_credit_note_grand_total'], $total_tax_amount, $section_modules['access_common_settings'][0]->amount_precision);
        }*/
        ...
        if ($action == "add"){
            ...
            $current_date    = $data_main['sales_credit_note_date'];
            $voucher_number  = $this->generate_invoice_number($access_settings, $primary_id, $table_name, $date_field_name, $current_date);

            $headers = array(
                "voucher_date"      => $data_main['sales_credit_note_date'],
                "voucher_number"    => $voucher_number,
                "party_id"          => $data_main['sales_credit_note_party_id'],
                "party_type"        => $data_main['sales_credit_note_party_type'],
                "reference_id"      => $data_main['sales_credit_note_id'],
                "reference_type"    => 'sales_credit_note',
                "reference_number"  => $data_main['sales_credit_note_invoice_number'],
        ...
        }else if ($action == "edit")
        {
            ...
            $sales_voucher_data = $this->general_model->getRecords('sales_voucher_id', 'sales_voucher', array(
                'reference_id'  => $data_main['sales_credit_note_id'],'delete_status' => 0,'reference_type' => 'sales_credit_note'));
    ...
    public function sales_credit_note_vouchers($section_modules, $data_main, $js_data, $branch,$action){
        ...
        /* Tax rate slab */
        $present = "";
        $tax_slab =  $tax_slab_minus = $tax_slab_items = $igst_slab_minus = $cgst_slab_minus = $sgst_slab_minus = $cess_slab_minus = $cess_slab_minus = $igst_slab = $cgst_slab = $sgst_slab = $cess_slab = $igst_slab_items =  $cgst_slab_items = $sgst_slab_items =  $cess_slab_items = array();

        if ((($data_main['sales_credit_note_tax_amount'] > 0 && ($data_main['sales_credit_note_igst_amount'] > 0 || $data_main['sales_credit_note_cgst_amount'] > 0 || $data_main['sales_credit_note_sgst_amount'] > 0) && $data_main['sales_credit_note_gst_payable'] != "yes"))){
            $present = "gst";

            if ($data_main['sales_credit_note_billing_state_id'] != 0  && $data_main['sales_credit_note_type_of_supply'] == 'regular'){

                if ($branch[0]->branch_state_id == $data_main['sales_credit_note_billing_state_id']) {
                    $present = "cgst";
                } else {
                    $present = "igst";
                }
            } else {
                if ($data_main['sales_credit_note_type_of_supply'] == "export_with_payment"){
                ...
                /*foreach ($i_o_array as $key_io => $value_io){*/
                    ...
                    foreach ($js_data as $key => $value) {
                        ...
                        if ($present == "cgst"){
                            ...
                            if ($value['sales_credit_note_item_cgst_percentage'] > 0 || $value['sales_credit_note_item_sgst_percentage'] > 0){

                                $cgst_ary = array(
                                            'ledger_name' => 'Output CGST@'.$value['sales_credit_note_item_cgst_percentage'].'%',
                                            'second_grp' => 'CGST',
                                            'primary_grp' => 'Duties and taxes',
                                            'main_grp' => 'Current Liabilities',
                                            'default_ledger_id' => 0,
                                            'default_value' => $value['sales_credit_note_item_cgst_percentage'],
                                            'amount' => 0
                                        );
                                if(!empty($cgst_x)){
                                    $cgst_ledger = $cgst_x->ledger_name;
                                    $cgst_ledger = str_ireplace('{{X}}',$value['sales_credit_note_item_cgst_percentage'] , $cgst_ledger);
                                ...
                                /*$cgst_tax_ledger = $this->ledger_model->addGroupLedger(array(
                                                                        'ledger_name' => 'CGST@'.$value['sales_credit_note_item_cgst_percentage'].'%',
                                                                        'subgrp_1' => 'CGST',
                                ...
                                $gst_lbl = 'SGST';
                                $is_utgst = $this->general_model->checkIsUtgst($data_main['sales_credit_note_billing_state_id']);
                                if($is_utgst == '1') $gst_lbl = 'UTGST';

                                $sgst_ary = array(
                                            'ledger_name' => 'Output '.$gst_lbl.'@'.$value['sales_credit_note_item_sgst_percentage'].'%',
                                            'second_grp' => $gst_lbl,
                                            'primary_grp' => 'Duties and taxes',
                                            'main_grp' => 'Current Liabilities',
                                            'default_ledger_id' => 0,
                                            'default_value' => $value['sales_credit_note_item_sgst_percentage'],
                                            'amount' => 0
                                        );
                                if(!empty($sgst_x)){
                                    if($is_utgst == '1') {
                                        $sgst_ledger = $utgst_x->ledger_name;
                                        $sgst_ledger = str_ireplace('{{X}}',$value['sales_credit_note_item_sgst_percentage'] , $sgst_ledger);
                                ...
                                /*$sgst_tax_ledger = $this->ledger_model->addGroupLedger(array(
                                                                            'ledger_name' => $gst_lbl.'@'.$value['sales_credit_note_item_sgst_percentage'].'%',
                                                                            'subgrp_1' => $gst_lbl,
                                                                            'subgrp_2' => 'Duties and taxes',
                                                                            'main_grp' => 'Current Liabilities',
                                                                            'amount' => 0
                                ...
                                if (in_array($cgst_tax_ledger, $cgst_slab)){
                                    $cgst_slab_items[$cgst_tax_ledger] = bcadd($cgst_slab_items[$cgst_tax_ledger], $value['sales_credit_note_item_cgst_amount'], $section_modules['access_common_settings'][0]->amount_precision);
                                }else{
                                    $cgst_slab[] = $cgst_tax_ledger;

                                    $cgst_slab_items[$cgst_tax_ledger] = $value['sales_credit_note_item_cgst_amount'];
                                }
                               
                                if (in_array($sgst_tax_ledger, $sgst_slab)) {
                                    $sgst_slab_items[$sgst_tax_ledger] = bcadd($sgst_slab_items[$sgst_tax_ledger], $value['sales_credit_note_item_sgst_amount'], $section_modules['access_common_settings'][0]->amount_precision);
                                } else {
                                    $sgst_slab[]                       = $sgst_tax_ledger;
                                    $sgst_slab_items[$sgst_tax_ledger] = $value['sales_credit_note_item_sgst_amount'];
                                }
                            ...
                            if ($value['sales_credit_note_item_tax_cess_percentage'] > 0){

                                $default_cess_id = $sales_ledger['CESS@X'];
                                $cess_ledger_name = $this->ledger_model->getDefaultLedgerId($default_cess_id);
                               
                                $cess_ary = array(
                                                'ledger_name' => 'Output Compensation Cess @'.$value['sales_credit_note_item_tax_cess_percentage'].'%',
                                                'second_grp' => 'Cess',
                                                'primary_grp' => 'Duties and taxes',
                                                'main_grp' => 'Current Liabilities',
                                                'default_ledger_id' => 0,
                                                'default_value' => $value['sales_credit_note_item_tax_cess_percentage'],
                                                'amount' => 0
                                            );
                                if(!empty($cess_ledger_name)){
                                    $cess_ledger = $cess_ledger_name->ledger_name;
                                    $cess_ledger = str_ireplace('{{X}}',$value['sales_credit_note_item_tax_cess_percentage'] , $cess_ledger);
                                ...
                                /*$cess_tax_ledger = $this->ledger_model->addGroupLedger(array(
                                                                                'ledger_name' => 'Compensation Cess @'.$value['sales_credit_note_item_tax_cess_percentage'].'%',
                                                                                'subgrp_1' => 'Cess',
                                                                                'subgrp_2' => 'Duties and taxes',
                                                                                'main_grp' => 'Current Liabilities',
                                                                                'amount' => 0
                                ...
                                if (in_array($cess_tax_ledger , $cess_slab)){
                                    $cess_slab_items[$cess_tax_ledger] = bcadd($cess_slab_items[$cess_tax_ledger] , $value['sales_credit_note_item_tax_cess_amount'],$section_modules['access_common_settings'][0]->amount_precision);
                                } else {
                                    $cess_slab[]                       = $cess_tax_ledger;
                                    $cess_slab_items[$cess_tax_ledger] = $value['sales_credit_note_item_tax_cess_amount'];
                                }
                            }
                        } elseif($present == "igst"){

                            if ($value['sales_credit_note_item_igst_percentage'] > 0){

                                $igst_ary = array(
                                            'ledger_name' => 'Output IGST@'.$value['sales_credit_note_item_igst_percentage'].'%',
                                            'second_grp' => 'IGST',
                                            'primary_grp' => 'Duties and taxes',
                                            'main_grp' => 'Current Liabilities',
                                            'default_ledger_id' => 0,
                                            'default_value' => $value['sales_credit_note_item_igst_percentage'],
                                            'amount' => 0
                                        );
                                if(!empty($igst_x)){
                                    $igst_ledger = $igst_x->ledger_name;
                                    $igst_ledger = str_ireplace('{{X}}',$value['sales_credit_note_item_igst_percentage'] , $igst_ledger);
                                ...
                                /*$igst_tax_ledger = $this->ledger_model->addGroupLedger(array(
                                                                        'ledger_name' => 'IGST@'.$value['sales_credit_note_item_igst_percentage'].'%',
                                                                        'subgrp_1' => 'IGST',
                                                                        'subgrp_2' => 'Duties and taxes',
                                                                        'main_grp' => 'Current Liabilities',
                                                                        'amount' => 0
                                ...
                                if (in_array($igst_tax_ledger, $igst_slab)){
                                    $igst_slab_items[$igst_tax_ledger] = bcadd($igst_slab_items[$igst_tax_ledger], $value['sales_credit_note_item_igst_amount'], $section_modules['access_common_settings'][0]->amount_precision);
                                }else{
                                    $igst_slab[]                       = $igst_tax_ledger;
                                    $igst_slab_items[$igst_tax_ledger] = $value['sales_credit_note_item_igst_amount'];
                                }
        ...
        }else if (($data_main['sales_credit_note_tax_amount'] > 0 && ($data_main['sales_credit_note_igst_amount'] == 0 && $data_main['sales_credit_note_cgst_amount'] == 0 && $data_main['sales_credit_note_sgst_amount'] == 0) && $data_main['sales_credit_note_gst_payable'] != "yes")){
           ...
           /* foreach ($i_o_array as $key_io => $value_io) {*/
                /***foreach ($js_data as $key => $value) {
                    if ($value['sales_credit_note_item_tax_percentage'] > 0){                    
                        $tax_ledger = $this->ledger_model->addGroupLedger(array(
                                                                        'ledger_name' => 'TAX@'.$value['sales_credit_note_item_tax_percentage'].'%',
                                                                        'subgrp_1' => 'TAX',
                                                                        'subgrp_2' => 'Duties and taxes',
                                                                        'main_grp' => 'Current Liabilities',
                                                                        'amount' => 0
                        ...
                        if (in_array($tax_ledger, $tax_slab)){
                            $tax_slab_items[$tax_ledger] = bcadd($tax_slab_items[$tax_ledger], $value['sales_credit_note_item_tax_amount'], $section_modules['access_common_settings'][0]->amount_precision);
                        } else {
                            $tax_slab[]                  = $tax_ledger;
                            $tax_slab_items[$tax_ledger] = $value['sales_credit_note_item_tax_amount'];
                        }
                    }
                }****/
            /*}*/
        ...
        /* Charges modules tax */
        if($data_main['sales_credit_note_type_of_supply'] != 'export_without_payment' && $data_main['sales_credit_note_gst_payable'] != "yes"){
            $present = "gst";
        ...
        /* TDS SLAB */
        if ($data_main['sales_credit_note_tds_amount'] > 0 || $data_main['sales_credit_note_tcs_amount'] > 0){
            $tds_slab                      = array();
            ...
            foreach ($js_data as $key => $value){
                if ($value['sales_credit_note_item_tds_percentage'] > 0){
                    ...
                    $where = array(
                        'td.delete_status' => 0,
                        'td.tax_id'        => $value['sales_credit_note_item_tds_id']);
                    ...
                    if (strtoupper($module_type) == "TCS"){
                        ...
                        $tds_ary = array(
                                        'ledger_name' => $tds_subgroup.' '.$section_name.'@'.$value['sales_credit_note_item_tds_percentage'].'%',
                                        'second_grp' => '',
                                        'primary_grp' => 'Duties and taxes',
                                        'main_grp' => 'Current Liabilities',
                                        'default_ledger_id' => 0,
                                        'default_value' => $value['sales_credit_note_item_tds_percentage'],
                                        'amount' => 0
                                    );
                        if(!empty($tds_ledger_name)){
                            $tds_ledger = $tds_ledger_name->ledger_name;
                            $tds_ledger = str_ireplace('{{SECTION}}',$section_name , $tds_ledger);
                            $tds_ledger = str_ireplace('{{X}}',$value['sales_credit_note_item_tds_percentage'] , $tds_ledger);
                    ...
                    {
                        $tds_slab_items[$tds_ledger] = bcadd($tds_slab_items[$tds_ledger], $value['sales_credit_note_item_tds_amount'], $section_modules['access_common_settings'][0]->amount_precision);
                    }
                    else
                    {
                        $tds_slab[]                  = $tds_ledger;
                        $tds_slab_items[$tds_ledger] = $value['sales_credit_note_item_tds_amount'];
                    }
        ...
        $table              = 'customer';
        $where              = array('customer_id' => $data_main['sales_credit_note_party_id']);
        ...
        {
            $total_tax_amount = ($data_main['sales_credit_note_tax_amount'] + $data_main['freight_charge_tax_amount'] + $data_main['insurance_charge_tax_amount'] + $data_main['packing_charge_tax_amount'] + $data_main['incidental_charge_tax_amount'] + $data_main['inclusion_other_charge_tax_amount'] - $data_main['exclusion_other_charge_tax_amount'] + $data_main['sales_credit_note_tax_cess_amount']);
            $grand_total      = bcsub($data_main['sales_credit_note_grand_total'], $total_tax_amount, 2);
        }
        ...
        if (isset($data_main['sales_credit_note_tds_amount']) && $data_main['sales_credit_note_tds_amount'] > 0){
            $grand_total = bcsub($grand_total, $data_main['sales_credit_note_tds_amount'], 2);
        }


        $sub_total = $data_main['sales_credit_note_sub_total'];
        ...
        if($action == 'edit'){
            $old_sales_credit_note_data = $this->general_model->getRecords('*', 'sales_credit_note', array(
            'sales_credit_note_id' => $data_main['sales_credit_note_id']));
            $sales_id   = $old_sales_credit_note_data[0]->sales_id;
            $old_amount = $this->general_model->getRecords("credit_note_amount, converted_credit_note_amount,customer_payable_amount", "sales", array(
                'sales_id' => $sales_id));
            $new_amount           = bcsub($old_amount[0]->credit_note_amount, $old_sales_credit_note_data[0]->sales_credit_note_grand_total, $section_modules['access_common_settings'][0]->amount_precision);
            $cust_new_amount      = bcadd($old_amount[0]->customer_payable_amount, $old_sales_credit_note_data[0]->customer_payable_amount, $section_modules['access_common_settings'][0]->amount_precision);
            $new_converted_amount = bcsub($old_amount[0]->converted_credit_note_amount, $old_sales_credit_note_data[0]->converted_grand_total, $section_modules['access_common_settings'][0]->amount_precision);
        ...
        $old_amount = $this->general_model->getRecords("credit_note_amount,converted_credit_note_amount,customer_payable_amount", "sales", array('sales_id' => $sales_id));

        $new_amount           = bcadd($old_amount[0]->credit_note_amount, $data_main["sales_credit_note_grand_total"], $section_modules['access_common_settings'][0]->amount_precision);
        $cust_new_amount      = bcsub($old_amount[0]->customer_payable_amount, $grand_total, $section_modules['access_common_settings'][0]->amount_precision);
        ...
        if ($data_main['sales_credit_note_tds_amount'] > 0 || $data_main['sales_credit_note_tcs_amount'] > 0){
            ...
            foreach ($tds_slab_items as $key => $value){
                ...
                /*if ($data_main['sales_credit_note_tds_amount'] > 0) {
                    $dr_amount = '';
                    $cr_amount = $value;
                    $ledger_to = $ledgers['ledger_from'];
                } else {
       ...
       /* print_r($cgst_slab_minus);*/
        ...
        if ($data_main['sales_credit_note_discount_amount'] > 0){
           /* $discount_ledger_id            = $this->ledger_model->getDefaultLedger('Discount Given');*/
            ...
            foreach ($js_data as $key => $value){
                $discount_sum = bcadd($discount_sum, $value['sales_credit_note_item_discount_amount'], $section_modules['access_common_settings'][0]->amount_precision);
            }
    ...
    function edit($id){
        ...
        $data['data'] = $this->general_model->getRecords('*' , 'sales_credit_note' , array(
        ...
        $data['shipping_address'] = $this->general_model->getRecords('*' , 'shipping_address' , array(
            'shipping_party_id'   => $data['data'][0]->sales_credit_note_party_id ,
            'shipping_party_type' => $data['data'][0]->sales_credit_note_party_type ,
            'delete_status' => 0
        ));
        ...
        if ($data['data'][0]->sales_credit_note_tax_amount > 0 || $data['access_settings'][0]->tax_type != "no_tax"){
            $data['tax'] = $this->tax_call();
        }

        if ($data['data'][0]->sales_credit_note_nature_of_supply == "service" || $data['data'][0]->sales_credit_note_nature_of_supply == "both"){
            $data['sac']              = $this->sac_call();
            $data['service_category'] = $this->service_category_call();
        }
        ...
        if ($data['data'][0]->sales_credit_note_discount_amount > 0 || $data['access_settings'][0]->discount_visible == "yes")
        {
            /* Discount */
            $discountExist    = 1;
            $data['discount'] = $this->discount_call();
        }
        if ($data['data'][0]->sales_credit_note_tds_amount > 0 || $data['data'][0]->sales_credit_note_tcs_amount > 0 || $data['access_settings'][0]->tds_visible == "yes")
        {
            /* Discount */
            $tdsExist = 1;
        }
       ...
       /* echo "<pre>";
        print_r($data);
        exit();*/
        $this->load->view('sales_credit_note/edit' , $data);

    }

    public function edit_sales_credit_note()
    {

        $data            = $this->get_default_country_state();
        $sales_credit_note_id        = $this->input->post('sales_credit_note_id');
        $sales_credit_note_module_id = $this->config->item('sales_credit_note_module');
        $module_id       = $sales_credit_note_module_id;
        $modules         = $this->modules;
        $privilege       = "edit_privilege";
        ...
        $sales_credit_note_data = array(
            ...
            "sales_credit_note_discount_amount"                 => $this->input->post('total_discount_amount') ? (float) $this->input->post('total_discount_amount') : 0 ,
            "sales_credit_note_tax_amount"                      => $this->input->post('total_tax_amount') ? (float) $this->input->post('total_tax_amount') : 0 ,
            "sales_credit_note_tax_cess_amount"                 => 0 ,
            "sales_credit_note_taxable_value"                   => $this->input->post('total_taxable_amount') ? (float) $this->input->post('total_taxable_amount') : 0 ,
            "sales_credit_note_tds_amount"                      => $this->input->post('total_tds_amount') ? (float) $this->input->post('total_tds_amount') : 0 ,
            "sales_credit_note_tcs_amount" 						=> $this->input->post('total_tcs_amount') ? (float) $this->input->post('total_tcs_amount') : 0 ,
            "sales_credit_note_igst_amount"                     => 0 ,
        ...
        $sales_credit_note_tax_amount = $sales_credit_note_data['sales_credit_note_tax_amount'];
        $sales_credit_note_tax_amount = $sales_credit_note_data['sales_credit_note_tax_amount'] + (float)($this->input->post('total_other_taxable_amount'));
        ...
        $old_amount = $this->general_model->getRecords("credit_note_amount, converted_credit_note_amount,customer_payable_amount", "sales", array(
            'sales_id' => $sales_id));
        $new_amount           = bcsub($old_amount[0]->credit_note_amount, $old_sales_credit_note_data[0]->sales_credit_note_grand_total, $section_modules['access_common_settings'][0]->amount_precision);
        $cust_new_amount           = bcadd($old_amount[0]->customer_payable_amount, ($old_sales_credit_note_data[0]->sales_credit_note_grand_total - $old_sales_credit_note_data[0]->sales_credit_note_tds_amount), $section_modules['access_common_settings'][0]->amount_precision);
        $new_converted_amount = bcsub($old_amount[0]->converted_credit_note_amount, $old_sales_credit_note_data[0]->converted_grand_total, $section_modules['access_common_settings'][0]->amount_precision);
        $this->general_model->updateData("sales", array(
            'credit_note_amount'           => $new_amount,
            'converted_credit_note_amount' => $new_converted_amount,
        ...
        $data_main   = array_map('trim' , $sales_credit_note_data);
        $sales_credit_note_table = $this->config->item('sales_credit_note_table');
        $where       = array('sales_credit_note_id' => $sales_credit_note_id );


        if ($this->general_model->updateData($sales_credit_note_table , $data_main , $where)){

            /*$old_amount1 = $this->general_model->getRecords("credit_note_amount,customer_payable_amount", "sales", array(
                'sales_id' => $sales_id));
            $new_amount1 = bcadd($old_amount1[0]->credit_note_amount, $sales_credit_note_data['sales_credit_note_grand_total'], $section_modules['access_common_settings'][0]->amount_precision);
            $cust_new_amount1 = bcsub($old_amount1[0]->customer_payable_amount, ($sales_credit_note_data['sales_credit_note_grand_total'] - $sales_credit_note_data['sales_credit_note_tds_amount']), $section_modules['access_common_settings'][0]->amount_precision);
            $this->general_model->updateData("sales", array(
                'credit_note_amount' => $new_amount1,'customer_payable_amount' => $cust_new_amount1), array(
                'sales_id' => $sales_id));*/
            $successMsg = 'Sales Credit Note Updated Successfully';
            $this->session->set_flashdata('sales_cn_success',$successMsg);
            $log_data              = array(
                'user_id'           => $this->session->userdata('SESS_USER_ID') ,
                'table_id'          => $sales_credit_note_id ,
            ...
            if (!empty($js_data)) {
                ...
                $string          = 'sales_credit_note_item_id,sales_credit_note_item_quantity,item_type,item_id';
                $table           = 'sales_credit_note_item';
                $where           = array(
                    'sales_credit_note_id'      => $sales_credit_note_id ,
                    'delete_status' => 0 );
                $old_sales_credit_note_items = $this->general_model->getRecords($string , $table , $where , $order           = "");
                $old_item_ids = $this->getValues($old_sales_credit_note_items,'item_id');
                $not_deleted_ids= array();

                foreach ($old_sales_credit_note_items as $key => $value){
                    ...
                    $product_qty    = bcadd($product[0]->product_quantity , $value->sales_credit_note_item_quantity,$section_modules['access_common_settings'][0]->amount_precision);
                    $product_data   = array(
                    ...
                    if (!empty($history))
                    {
                        $history_quantity        = bcadd($history[0]->quantity , $value->sales_credit_note_item_quantity,$section_modules['access_common_settings'][0]->amount_precision);
                ...
                $this->db->delete('sales_credit_note_item');
                ...
                foreach ($js_data as $key => $value) {
                    if ($value != null) {
                        ...
                        /*if (($item_key = array_search($value->item_id, $old_item_ids)) !== false) {
                            ...
                            $sales_credit_note_item_id = $old_sales_credit_note_items[$item_key]->sales_credit_note_item_id;
                            array_push($not_deleted_ids,$sales_credit_note_item_id );
                            $where = array('sales_credit_note_item_id' => $sales_credit_note_item_id );
                ...
                /*if(!empty($old_sales_credit_note_items)){
                    foreach ($old_sales_credit_note_items as $key => $items) {
                        if(!in_array( $items->sales_credit_note_item_id,$not_deleted_ids)){
                            $table      = 'sales_credit_note_item';
                            $where      = array(
                                'sales_credit_note_item_id' => $items->sales_credit_note_item_id );
                            $sales_credit_note_data = array(
                                'delete_status' => 1 );
                           
                            $this->general_model->updateData($table , $sales_credit_note_data , $where);
                        }
                    }
                }*/
        ...
        {
            $errorMsg = 'Sales Credit Note Update Unsuccessful';
            $this->session->set_flashdata('sales_cn_error',$errorMsg);
            redirect('sales_credit_note' , 'refresh');
        }
    ...
    public function edit_sales_credit_note_1()
    {
        $sales_credit_note_id              = $this->input->post('sales_credit_note_id');
        $sales_credit_note_module_id       = $this->config->item('sales_credit_note_module');
        $module_id                         = $sales_credit_note_module_id;
        ...
        $sales_credit_note_data = array(
            "sales_id"                             => $this->input->post('sales_invoice_number'),
            "sales_credit_note_date"               => date('Y-m-d',strtotime($this->input->post('invoice_date'))),
            "sales_customer_credit_note_date"      => date('Y-m-d',strtotime($this->input->post('customer_credit_note_date'))),
            "sales_customer_credit_note_no"        => $this->input->post('customer_credit_note_no'),
            "sales_credit_note_invoice_number"     => $invoice_number,
            "sales_credit_note_sub_total"          => $this->input->post('total_sub_total'),
            "sales_credit_note_grand_total"        => $this->input->post('total_grand_total'),
            "sales_credit_note_discount_value"     => $this->input->post('total_discount_amount'),
            "sales_credit_note_tax_amount"         => $this->input->post('total_tax_amount'),
            "sales_credit_note_taxable_value"      => $this->input->post('total_taxable_amount'),
            "sales_credit_note_igst_amount"        => $this->input->post('total_igst_amount'),
            "sales_credit_note_cgst_amount"        => $this->input->post('total_cgst_amount'),
            "sales_credit_note_sgst_amount"        => $this->input->post('total_sgst_amount'),
            "financial_year_id"                    => $this->session->userdata('SESS_FINANCIAL_YEAR_ID'),
            "sales_credit_note_party_id"           => $this->input->post('customer'),
            "sales_credit_note_party_type"         => "customer",
            "sales_credit_note_nature_of_supply"   => $this->input->post('nature_of_supply'),
            "sales_credit_note_type_of_supply"     => $this->input->post('type_of_supply'),
            "sales_credit_note_gst_payable"        => $this->input->post('gstPayable'),
            "sales_credit_note_billing_country_id" => $this->input->post('billing_country'),
            "sales_credit_note_billing_state_id"   => $this->input->post('billing_state'),
            "branch_id"                            => $this->session->userdata('SESS_BRANCH_ID'),
            "currency_id"                          => $this->input->post('currency_id'),
        ...
        if ($this->session->userdata('SESS_DEFAULT_CURRENCY') == $this->input->post('currency_id'))
        {
            $sales_credit_note_data['currency_converted_amount'] = $this->input->post('total_grand_total');
        }
        else
        {
            $sales_credit_note_data['currency_converted_amount'] = "0.00";
        }

        $old_sales_credit_note_data = $this->general_model->getRecords('*', 'sales_credit_note', array(
            'sales_credit_note_id' => $sales_credit_note_id));
        $sales_id   = $old_sales_credit_note_data[0]->sales_id;
        $old_amount = $this->general_model->getRecords("credit_note_amount, converted_credit_note_amount", "sales", array(
            'sales_id' => $sales_id));
        $new_amount           = bcsub($old_amount[0]->credit_note_amount, $old_sales_credit_note_data[0]->sales_credit_note_grand_total, $section_modules['access_common_settings'][0]->amount_precision);
        $new_converted_amount = bcsub($old_amount[0]->converted_credit_note_amount, $old_sales_credit_note_data[0]->currency_converted_amount, $section_modules['access_common_settings'][0]->amount_precision);
        $this->general_model->updateData("sales", array(
            'credit_note_amount'           => $new_amount,
        ...
        if ($this->general_model->updateData($sales_credit_note_table, $data_main, $where))
        {
            $old_amount1 = $this->general_model->getRecords("credit_note_amount", "sales", array(
                'sales_id' => $sales_id));
            $new_amount1 = bcadd($old_amount1[0]->credit_note_amount, $sales_credit_note_data['sales_credit_note_grand_total'], $section_modules['access_common_settings'][0]->amount_precision);
            $this->general_model->updateData("sales", array(
                'credit_note_amount' => $new_amount1), array(
                'sales_id' => $sales_id));
            $log_data = array(
                'user_id'           => $this->session->userdata('SESS_USER_ID'),
                'table_id'          => $sales_credit_note_id,
            ...
            foreach ($js_data as $key => $value)
            {
                ...
                else
                {
                    ...
                    $item_data[] = array(
                        "item_id"                                => $value->item_id,
                        "item_type"                              => $value->item_type,
                        "sales_credit_note_item_description"     => $value->item_description,
                        "sales_credit_note_item_quantity"        => $value->item_quantity,
                        "sales_credit_note_item_unit_price"      => $value->item_price,
                        "sales_credit_note_item_sub_total"       => $value->item_sub_total,
                        "sales_credit_note_item_taxable_value"   => $value->item_taxable_value,
                        "sales_credit_note_item_discount_amount" => $value->item_discount_amount,
                        "sales_credit_note_item_discount_id"     => $value->item_discount,
                        "sales_credit_note_item_grand_total"     => $value->item_grand_total,
                        "sales_credit_note_item_igst_percentage" => $value->item_igst,
                        "sales_credit_note_item_igst_amount"     => $value->item_igst_amount,
                        "sales_credit_note_item_cgst_percentage" => $value->item_cgst,
                        "sales_credit_note_item_cgst_amount"     => $value->item_cgst_amount,
                        "sales_credit_note_item_sgst_percentage" => $value->item_sgst,
                        "sales_credit_note_item_sgst_amount"     => $value->item_sgst_amount,
                        "sales_credit_note_item_tax_percentage"  => $value->item_tax_percentage,
                        "sales_credit_note_item_tax_amount"      => $value->item_tax_amount,
                        "sales_credit_note_id"                   => $sales_credit_note_id);
                }
            ...
            foreach ($old_sales_credit_note_items as $key => $value)
            {
                ...
                elseif ($value->item_type == "product_inventory")
                {
                    ...
                    $product      = $this->general_model->getRecords($product_string, $product_table, $product_where, $order = "");
                    $product_qty  = bcadd($product[0]->quantity, $value->sales_credit_note_item_quantity, $section_modules['access_common_settings'][0]->amount_precision);
                    $product_data = array(
            ...
            if (count($old_sales_credit_note_items) == count($item_data))
            {
                ...
                foreach ($old_sales_credit_note_items as $key => $value)
                {
                    $table = 'sales_credit_note_item';
                    $where = array(
                        'sales_credit_note_item_id' => $value->sales_credit_note_item_id);
                    $this->general_model->updateData($table, $item_data[$key], $where);
                }

            }
            elseif (count($old_sales_credit_note_items) < count($item_data))
            {
            ...
            else
            {
                ...
                for ($j = $i + 1; $j < count($old_sales_credit_note_items); $j++)
                {
                    $table = 'sales_credit_note_item';
                    $where = array(
                        'sales_credit_note_item_id' => $old_sales_credit_note_items[$j]->sales_credit_note_item_id);
                    $sales_credit_note_data = array(
                        'delete_status' => 1);
                    $this->general_model->updateData($table, $sales_credit_note_data, $where);
                }
            ...
            $string = 'sales_credit_note_item_id,item_id,sales_credit_note_item_quantity,item_type,item_id';
            $table  = 'sales_credit_note_item';
            $where  = array(
                'sales_credit_note_id' => $sales_credit_note_id,
                'delete_status'        => 0);
            $new_sales_credit_note_items = $this->general_model->getRecords($string, $table, $where, $order = "");

            foreach ($new_sales_credit_note_items as $key => $value)
            {
                ...
                if ($value->item_type == "product" || $value->item_type == "product_inventory")
                {
                    ...
                    $product_where  = array(
                        'product_id' => $value->item_id);
                    $product      = $this->general_model->getRecords($product_string, $product_table, $product_where, $order = "");
                    $product_qty  = bcsub($product[0]->product_quantity, $value->sales_credit_note_item_quantity, $section_modules['access_common_settings'][0]->amount_precision);
                    $product_data = array(
                        'product_quantity' => $product_qty);
                    $this->general_model->updateData($product_table, $product_data, $product_where);
                }
                /*elseif ()
                {
                    ...
                    $product_where  = array(
                        'product_inventory_varients_id' => $value->item_id);
                    $product      = $this->general_model->getRecords($product_string, $product_table, $product_where, $order = "");
                    $product_qty  = bcsub($product[0]->quantity, $value->sales_credit_note_item_quantity, $section_modules['access_common_settings'][0]->amount_precision);
                    $product_data = array(
                        'quantity'        => $product_qty,
                        'updated_date'    => date('Y-m-d'),
                        'updated_user_id' => $this->session->userdata('SESS_USER_ID'));
                    ...
                    $this->db->where($where);
                    $history                 = $this->db->get('quantity_history')->result();
                    $history_quantity        = bcsub($history[0]->quantity, $value->sales_credit_note_item_quantity, $section_modules['access_common_settings'][0]->amount_precision);
                    $update_history_quantity = array(
                        'quantity' => $history_quantity);
                    $this->db->where($where);
                    $this->db->update('quantity_history', $update_history_quantity);
    ...
    public function delete(){
        ...
        $sales_credit_note_id        = $this->encryption_url->decode($id);
        $sales_credit_note_module_id = $this->config->item('sales_credit_note_module');
        $data['module_id']           = $sales_credit_note_module_id;
        $modules                     = $this->modules;
        ...
        $data['data'] = $this->general_model->getRecords('*', 'sales_credit_note', array(
            'sales_credit_note_id' => $sales_credit_note_id));
        $data['items'] = $this->general_model->getRecords('*', 'sales_credit_note_item', array(
            'sales_credit_note_id' => $sales_credit_note_id,
            'delete_status'        => 0));

        $sales_voucher_id = $this->general_model->getRecords('sales_voucher_id', 'sales_voucher', array(
            'reference_id'   => $sales_credit_note_id, 'reference_type' => 'sales_credit_note'));
        ...
        /*$this->general_model->updateData('sales_voucher', array('delete_status' => 1), array('reference_id'   => $sales_credit_note_id,'reference_type' => 'sales_credit_note'));*/

        if ($this->general_model->updateData('sales_credit_note', array(
            'delete_status' => 1), array(
            'sales_credit_note_id' => $sales_credit_note_id))){
            $sales_id   = $data['data'][0]->sales_id;
            $old_amount = $this->general_model->getRecords("credit_note_amount, converted_credit_note_amount", "sales", array('sales_id' => $sales_id));

            $new_amount           = bcsub($old_amount[0]->credit_note_amount, $data['data'][0]->sales_credit_note_grand_total, $section_modules['access_common_settings'][0]->amount_precision);
            ...
            foreach ($data['items'] as $key => $value){
                ...
                if ($value->item_type == "product" || $value->item_type == "product_inventory"){
                    ...
                    $product_result   = $this->general_model->getJoinRecords($product_data['string'], $product_data['table'], $product_data['where'],$product_data['join']);
                    $product_quantity = ($product_result[0]->product_quantity + $value->sales_credit_note_item_quantity);
                    $data1            = array(
            ...
            $successMsg = 'Sales Credit Note Deleted Successfully';
            $this->session->set_flashdata('sales_cn_success',$successMsg);
            ...
            $log_data = array(
                ...
                'financial_year_id' => $this->session->userdata('SESS_FINANCIAL_YEAR_ID'),
                "branch_id"         => $this->session->userdata('SESS_BRANCH_ID'),
                'message'           => 'Sales credit Note Deleted');
            $this->general_model->insertData('log', $log_data);

            $redirect = 'sales_credit_note';
        ...
        {
            $errorMsg = 'Sales Credit Note Delete Unsuccessful';
            $this->session->set_flashdata('sales_cn_error',$errorMsg);
            redirect("sales_credit_note", 'refresh');
        }
    ...
    public function view($id)
    {
        $id                          = $this->encryption_url->decode($id);
        $data                        = array();
        $data = $this->sales_credit_details($this->encryption_url->encode($id));
        ...
        if($this->input->post('is_only_view')){
            ...
            $data['data'][0]->sales_credit_note_date = date('Y-m-d',strtotime($this->input->post('invoice_date')));
        }
    ...
    public function pdf($id)
    {
        ...
        $html = $this->load->view('sales_credit_note/pdf', $data, true);
    ...
    public function email($id){
        ...
        if ($email_sub_module == 1)
        {
          ...
          //  $mpdf                           = new mPDF();
            ...
            //and now im creating new instance dompdf
            $file_path                      = "././pdf_form/";
            $file_name = str_replace($data['access_settings'][0]->invoice_seperation, "_", $data['data'][0]->sales_credit_note_invoice_number);
            ...
            $data['email_template'] = $this->general_model->getRecords('*', 'email_template', array(
                ...
                'branch_id'     => $this->session->userdata('SESS_BRANCH_ID'),
                'delete_status' => 0));
            $this->load->view('sales_credit_note/email', $data);
        }
        else
        {
            $this->load->view('sales_credit_note', $data);
        }
    ...
    public function convert_currency()
    {
        ...
        $credit_note_data     = $this->general_model->getRecords('sales_id,converted_grand_total', 'sales_credit_note', array(
            'sales_credit_note_id' => $id));
        $old_credit_note_converted_amount = $credit_note_data[0]->converted_grand_total;

        $sales_data = $this->general_model->getRecords('converted_credit_note_amount', 'sales', array(
            'sales_id' => $credit_note_data[0]->sales_id));
        $old_sales_con_credit_amt = $sales_data[0]->converted_credit_note_amount;

        $converted_credit_amount       = bcsub($old_sales_con_credit_amt, $old_credit_note_converted_amount);
        $total_converted_credit_amount = bcadd($converted_credit_amount, $new_converted_amount);
        ...
        $this->general_model->updateData('sales_credit_note', $data, array(
            'sales_credit_note_id' => $id));
