...
$(document).ready(function () {
    ...
    $('#input_purchase_debit_note_code').on('change focus', function () {
        $('#input_purchase_debit_note_code').trigger('click');
    });
    ...
    $('#purchase_debit_note_table_body').on('change blur','input[name=item_quantity],input[name=item_taxable_value],input[name=item_grand_total]',function(){
        setDefault($(this));
    })
    ...
    $("#purchase_invoice_number").change(function (event) {
        ...
        if (purchase_id != "") {
            $.ajax({
                url: base_url + 'purchase_debit_note/get_purchase_item',
                type: 'POST',
                data: { purchase_id: purchase_id},
                success: function (result) {

                    table_index = 0;
                    purchase_debit_note_data = new Array();
                    $("#purchase_debit_note_table_body").html('');
                    ...
                    /*purchase_debit_note_data = new Array();*/
                    var table_data = JSON.stringify(purchase_debit_note_data);
    ...
    function suggestion_box() {
        ...
        if ($('#purchase_invoice_number').val() != "") {
            $('#input_purchase_debit_note_code').prop('disabled', false);
            $('.search_purchase_debit_note_code').show();
            $('#err_purchase_debit_note_code').text('');
        } else {
            $('.search_purchase_debit_note_code').hide();
            $('#input_purchase_debit_note_code').prop('disabled', true);
            $('#err_purchase_debit_note_code').text('Please select the supplier to do purchase credit note.');
            $('#err_product').text('');
    ...
    $('#supplier').change(function () {
        if ($('#supplier').val() != "") {
            ...
            $.ajax({
                url: base_url + 'purchase_debit_note/get_purchase_invoice_number',
                type: 'POST',
                dataType: "json",
                data: {
    ...
    $('#input_purchase_debit_note_code').autoComplete({
        minChars: 1,
        cache: false,
        source: function (term, suggest) {
            ...
            $.ajax({
                url: base_url + "purchase_debit_note/get_purchase_debit_note_suggestions/" + term + "/" + inventory_advanced + "/" + item_access,
                type: "GET",
                dataType: "json",
                success: function (data) {
                    var suggestions = [];
                    ...
                    if (i == 1 && term.length > 7 ) {
                        ...
                        $.ajax({
                            url: base_url + 'purchase_debit_note/get_table_items/' + mapping[data[0].item_code],
                            type: "GET",
                            dataType: "JSON",
                            success: function (data1) {
                                add_row(data1);
                                call_css();
                                $('#input_purchase_debit_note_code').val('')
                            }
                        });
                        suggest('');
                    } else {
        ...
        onSelect: function (event, ui) {
            ...
            $.ajax({
                url: base_url + 'purchase_debit_note/get_table_items/' + mapping[code[0]],
                type: "GET",
                dataType: "JSON",
                success: function (data) {
    ...
    //delete rows 
    $("#purchase_debit_note_table_body").on("click", "a.deleteRow", function (event) {
    ...
    // changes in rows
    $("#purchase_debit_note_table_body").on("change blur", 'input[name^="item_price"],input[name^="item_description"],input[name="item_quantity"],select[name="item_discount"],select[name="item_tax"]', function (event) {
        var newRow = $(this).closest("tr");
        calculateTable(newRow);
    });

    $("#purchase_debit_note_table_body").on("change",'select[name="item_tax_cess"]',function (event) {
            var newRow = $(this).closest("tr");
            calculateTable(newRow);
        }
    );

    $("#purchase_debit_note_table_body").on("change",'select[name="item_tds_percentage"]',function (event) {
            /**/
            var newRow = $(this).closest("tr");
    ...
    $("#purchase_debit_note_table_body").on("change", 'input[name="item_grand_total"]', function (event) {
...
function add_row(data, loop = "") {
    ...
    var item_price = purchase_item.purchase_item_unit_price;
    var item_description = purchase_item.purchase_item_description;
    var item_quantity = purchase_item.purchase_item_quantity;
    ...
    newRow.append(cols);
    $("#purchase_debit_note_table_body").prepend(newRow);
    table_index++;
    calculateTable(newRow);
}
//calculate row
...
function generateJson(row) {
    ...
    var flag = 0;
    var i_val = "";
    for (var i = 0; i < purchase_debit_note_data.length; i++) {
        if (purchase_debit_note_data[i] !== "undefined" && purchase_debit_note_data[i].item_key_value == table_index) {
            flag = 1;
            i_val = i;
            break;
        }
    ...
    if (flag == 1) {
        purchase_debit_note_data[i_val] = data_item;
    } else {
        purchase_debit_note_data.push(data_item);
    }
...
function calculateGrandTotal() {
    ...
    var tdsflag = 0;
    var tcsflag = 0;
    $("#purchase_debit_note_table_body").find('input[name="item_sub_total"]').each(function () {
        sub_total += +$(this).val()
    });
    $("#purchase_debit_note_table_body").find('input[name="item_taxable_value"]').each(function () {
        taxable += +$(this).val()
    });
    $("#purchase_debit_note_table_body").find('input[name="item_discount_amount"]').each(function () {
        dflag = 1;
        discount += +$(this).val()
    });
    $("#purchase_debit_note_table_body").find('input[name="item_tax_amount"]').each(function () {
        tflag = 1;
        tax += +$(this).val()
    });

    $("#purchase_debit_note_table_body")
        .find('input[name="item_tax_cess_amount"]')
        .each(function () {
            tflag = 1;
            tax_cess += +$(this).val();
    ...
    $("#purchase_debit_note_table_body").find('input[name^="item_tds_amount"]').each(function () {
        row = $(this).closest("tr");
    ...
    $("#purchase_debit_note_table_body").find('input[name="item_grand_total"]').each(function () {
...
function deleteRow(row) {
    ...
    var j = 0;
    var purchase_debit_note_data_temp = new Array();

    for (var i = 0; i < purchase_debit_note_data.length; i++) {

        if (purchase_debit_note_data[i] !== "undefined" && purchase_debit_note_data[i].item_key_value != table_index_key) {
            // purchase_debit_note_data.splice(i, 1);
            var data_item = {
                "item_key_value": +purchase_debit_note_data[i].item_key_value,
                "item_id": +purchase_debit_note_data[i].item_id,
                "item_type": purchase_debit_note_data[i].item_type,
                "item_code": purchase_debit_note_data[i].item_code,
                "item_quantity": +purchase_debit_note_data[i].item_quantity,
                "item_price": +purchase_debit_note_data[i].item_price,
                "item_description": purchase_debit_note_data[i].item_description,
                "item_sub_total": +purchase_debit_note_data[i].item_sub_total,
                "item_discount_amount": +purchase_debit_note_data[i].item_discount_amount,
                "item_discount_id": +purchase_debit_note_data[i].item_discount_id,
                "item_discount_percentage": +purchase_debit_note_data[i].item_discount_percentage,
                "item_tax_amount": +purchase_debit_note_data[i].item_tax_amount,
                "item_tax_id": +purchase_debit_note_data[i].item_tax_id,
                "item_tax_percentage": +purchase_debit_note_data[i].item_tax_percentage,
                "item_tds_amount": +purchase_debit_note_data[i].item_tds_amount,
                "item_tds_id": +purchase_debit_note_data[i].item_tds_id,
                "item_tds_percentage": +purchase_debit_note_data[i].item_tds_percentage,
                "item_tds_type": +purchase_debit_note_data[i].item_tds_type,
                "item_taxable_value": +purchase_debit_note_data[i].item_taxable_value,
                "item_grand_total": +purchase_debit_note_data[i].item_grand_total
            };
            purchase_debit_note_data_temp.push(data_item);
        }
    }

    purchase_debit_note_data = new Array();
    purchase_debit_note_data.length = 0;
    purchase_debit_note_data = purchase_debit_note_data_temp;
