...
$(document).ready(function () {
    ...
    $('#input_sales_debit_note_code').on('change focus', function () {
        $('#input_sales_debit_note_code').trigger('click');
    });
    ...
    $('#sales_debit_note_table_body').on('change blur','input[name=item_quantity],input[name=item_taxable_value],input[name=item_grand_total]',function(){
        setDefault($(this));
    })
    ...
    $("#sales_invoice_number").change(function (event) {
        ...
        if (sales_id != "") {
            $.ajax({
                url: base_url + 'sales_debit_note/get_sales_item',
                type: 'POST',
                data: { sales_id: sales_id},
                success: function (result) {

                    table_index = 0;
                    sales_debit_note_data = new Array();
                    $("#sales_debit_note_table_body").html('');
                    ...
                    /*sales_debit_note_data = new Array();*/
                    var table_data = JSON.stringify(sales_debit_note_data);
    ...
    function suggestion_box() {
        ...
        if ($('#sales_invoice_number').val() != "") {
            $('#input_sales_debit_note_code').prop('disabled', false);
            $('.search_sales_debit_note_code').show();
            $('#err_sales_debit_note_code').text('');
        } else {
            $('.search_sales_debit_note_code').hide();
            $('#input_sales_debit_note_code').prop('disabled', true);
            $('#err_sales_debit_note_code').text('Please select the customer to do sales debit note.');
            $('#err_product').text('');
    ...
    $('#customer').change(function () {
        if ($('#customer').val() != "") {
            ...
            $.ajax({
                url: base_url + 'sales_debit_note/get_sales_invoice_number',
                type: 'POST',
                dataType: "json",
                data: {
    ...
    $('#input_sales_debit_note_code').autoComplete({
        minChars: 1,
        cache: false,
        source: function (term, suggest) {
            ...
            $.ajax({
                url: base_url + "sales_debit_note/get_sales_debit_note_suggestions/" + term + "/" + inventory_advanced + "/" + item_access,
                type: "GET",
                dataType: "json",
                success: function (data) {
                    var suggestions = [];
                    ...
                    if (i == 1 && term.length > 7) {
                        ...
                        $.ajax({
                            url: base_url + 'sales_debit_note/get_table_items/' + mapping[data[0].item_code],
                            type: "GET",
                            dataType: "JSON",
                            success: function (data1) {
                                add_row(data1);
                                call_css();
                                $('#input_sales_debit_note_code').val('')
                            }
                        });
                        suggest('');
                    } else {
        ...
        onSelect: function (event, ui) {
            ...
            $.ajax({
                url: base_url + 'sales_debit_note/get_table_items/' + mapping[code[0]],
                type: "GET",
                dataType: "JSON",
                success: function (data) {
    ...
    //delete rows 
    $("#sales_debit_note_table_body").on("click", "a.deleteRow", function (event) {
    ...
    // changes in rows
    $("#sales_debit_note_table_body").on("change blur", 'input[name^="item_price"],input[name^="item_description"],select[name="item_discount"],select[name="item_tax"]', function (event) {
        var newRow = $(this).closest("tr");
        calculateTable(newRow);
    });

    $("#sales_debit_note_table_body").on("change",'select[name="item_tax_cess"]',function (event) {
            var newRow = $(this).closest("tr");
            calculateTable(newRow);
        }
    );

    $("#sales_debit_note_table_body").on("change",'select[name="item_tds_percentage"]',function (event) {
            /**/
            var newRow = $(this).closest("tr");
    ...
    $("#sales_debit_note_table_body").on("change", 'input[name="item_grand_total"]', function (event) {
...
function add_row(data, loop = "") {
    ...
    if (item_type == 'product' || item_type == 'product_inventory')
        cols += "<td>" + item_name + "<br>(P) (HSN/SAC:" + item_hsn_sac_code + ")<br>"+product_batch;
    else
        cols += "<td>" + item_name + "<br>(S) (HSN/SAC:" + item_hsn_sac_code +")" ;
    cols += "<input type='hidden' name='loop_exist' value='" + loop + "' > <input type='hidden' name='item_key_value'  value=" + table_index + "><input type='hidden' name='item_id' value=" + item_id + "><input type='hidden' name='item_type' value=" + item_type + "><input type='hidden' name='item_code' value='" + item_code + "'></td>";
    ...
    /*if (tax_exist == 1) {
        if (discount_exist == 1) {*/
            ...
            <input type='hidden' class='form-control form-fixer text-right' style='' value='" + item_price + "' name='item_sub_total'><input type='hidden' class='form-control form-fixer text-right float_number' name='item_price' value='" + item_price + "' limit='"+item_price+"'>";
            if (discount_exist == 1) {
    ...
    newRow.append(cols);
    $("#sales_debit_note_table_body").prepend(newRow);
    table_index++;
    calculateTable(newRow);
    $(".select2").select2();
}
...
function calculateTds(row) {
    ...
    var item_tds_amount = precise_amount(item_taxable_value) * item_tds / 100;
...
function generateJson(row) {
    ...
    var flag = 0;
    var i_val = "";
    for (var i = 0; i < sales_debit_note_data.length; i++) {
        if (sales_debit_note_data[i] !== "undefined" && sales_debit_note_data[i].item_key_value == table_index) {
            flag = 1;
            i_val = i;
            break;
        }
    ...
    if (flag == 1) {
        sales_debit_note_data[i_val] = data_item;
    } else {
        sales_debit_note_data.push(data_item);
    }
...
function calculateGrandTotal() {
    ...
    var tdsflag = 0;
    var tcsflag = 0;
    $("#sales_debit_note_table_body").find('input[name="item_sub_total"]').each(function () {
        sub_total += +$(this).val()
    });
    $("#sales_debit_note_table_body").find('input[name="item_taxable_value"]').each(function () {
        taxable += +$(this).val()
    });
    $("#sales_debit_note_table_body").find('input[name="item_discount_amount"]').each(function () {
        dflag = 1;
        discount += +$(this).val()
    });
    $("#sales_debit_note_table_body").find('input[name="item_tax_amount"]').each(function () {
        tflag = 1;
        tax += +$(this).val()
    });

    $("#sales_debit_note_table_body")
        .find('input[name="item_tax_cess_amount"]')
        .each(function () {
            tflag = 1;
            tax_cess += +$(this).val();
    ...
    $("#sales_debit_note_table_body").find('input[name^="item_tds_amount"]').each(function () {
        row = $(this).closest("tr");
    ...
    $("#sales_debit_note_table_body").find('input[name="item_grand_total"]').each(function () {
...
function deleteRow(row) {
    ...
    var j = 0;
    var sales_debit_note_data_temp = new Array();

    for (var i = 0; i < sales_debit_note_data.length; i++) {

        if (sales_debit_note_data[i] !== "undefined" && sales_debit_note_data[i].item_key_value != table_index_key) {
            // sales_debit_note_data.splice(i, 1);
            var data_item = {
                "item_key_value": +sales_debit_note_data[i].item_key_value,
                "item_id": +sales_debit_note_data[i].item_id,
                "item_type": sales_debit_note_data[i].item_type,
                "item_code": sales_debit_note_data[i].item_code,
                "item_quantity": +sales_debit_note_data[i].item_quantity,
                "item_price": +sales_debit_note_data[i].item_price,
                "item_description": sales_debit_note_data[i].item_description,
                "item_sub_total": +sales_debit_note_data[i].item_sub_total,
                "item_discount_amount": +sales_debit_note_data[i].item_discount_amount,
                "item_discount_id": +sales_debit_note_data[i].item_discount_id,
                "item_discount_percentage": +sales_debit_note_data[i].item_discount_percentage,
                "item_tax_amount": +sales_debit_note_data[i].item_tax_amount,
                "item_tax_id": +sales_debit_note_data[i].item_tax_id,
                "item_tax_percentage": +sales_debit_note_data[i].item_tax_percentage,
                "item_tds_amount": +sales_debit_note_data[i].item_tds_amount,
                "item_tds_id": +sales_debit_note_data[i].item_tds_id,
                "item_tds_percentage": +sales_debit_note_data[i].item_tds_percentage,
                "item_tds_type": +sales_debit_note_data[i].item_tds_type,
                "item_taxable_value": +sales_debit_note_data[i].item_taxable_value,
                "item_grand_total": +sales_debit_note_data[i].item_grand_total
            };
            sales_debit_note_data_temp.push(data_item);
        }
    }

    sales_debit_note_data = new Array();
    sales_debit_note_data.length = 0;
    sales_debit_note_data = sales_debit_note_data_temp;
