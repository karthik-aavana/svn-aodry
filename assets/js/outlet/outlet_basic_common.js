...
$(document).ready(function () {
    ...
    $("#input_outlet_code").on('change focus', function () {
        $("#input_outlet_code").trigger("click");
    });
    ...
    function isInRange(currentElement,term) {
        var n = currentElement.product_name.toLowerCase();
        var c = currentElement.product_code.toLowerCase();
    ...
    $("#input_outlet_code").autoComplete({
        minChars: 1,
        cache: false,
        source: function (term, suggest) {
            ...
            var suggestions = [];
            /*let bigCities = Object.values(allPros).filter(city => city.product_code.includes(term));*/
            let filterPros = Object.values(allPros).filter(isInRange,term);
            
            if(filterPros.length == 1){
                product_id = filterPros.product_id;
                ...
                /*call_css();*/
                $("#input_outlet_code").val("");
            }else{
                $.each(filterPros,function(k,v){
                    suggestions.push(v.product_code + " " + v.product_name+ ' ' + v.product_batch);
                    var pro_name = v.product_name;
                    pro_name = pro_name.replace(/\s/g, "");
                    pro_name = pro_name.replace(/\//g, "");
                    pro_name = pro_name.replace(/-/g, "");
                    
                    var product_code = v.product_code+pro_name+'-'+v.product_batch;
                    var code = product_code.toString().split(' ');
                    mapping[code[0]] =
                        v.product_id +
                        "-" +
                        v.product_type +
                ...
                /*for (var i = 0; i < allPros.length; ++i) {
                    suggestions.push(allPros[i].product_code + " " + allPros[i].product_name+ ' ' + allPros[i].product_batch);
                    var product_code = allPros[i].product_code+'-'+allPros[i].product_batch;
                    var code = product_code.toString().split(' ');
                    mapping[code[0]] =
                        allPros[i].product_id +
                        "-" +
                        allPros[i].product_type +
                        "-" +
                        settings_discount_visible +
                        "-" +
                        settings_tax_type;
    ...
    //delete rows
    $("#outlet_table_body").on("click", "a.deleteRow", function (event) {
...
function getAllProducts(){
    $.ajax({
        url: base_url +"outlet/get_all_items",
        type: "POST",
        dataType: "JSON",
        success: function (data) {
            allPros = data;
...
function add_row(data) {
    ...
    var branch_country_id = branch_country_id;
    var branch_state_id = branch_state_id;
        ...
        for (a = 0; a < tax_ary.length; a++) {
            ...
            if(tax_ary[a].tax_name == 'GST'){
                select_tax +=
                    ...
                    '" ' +
                    selected +
                    " per='"+parseFloat(tax_ary[a].tax_value)+"'>" +
                    parseFloat(tax_ary[a].tax_value) +
                    "%</option>";
            }
        }
    ...
    cols +=
        "<input type='" +
        input_type +
        "' class='float_number form-control form-fixer text-right' name='item_grand_total'><input type='hidden' name='table_data[]' class='table_json'></td>";
    cols += "</tr>";

    newRow.append(cols);
    $("#outlet_table_body").prepend(newRow);
    table_index++;
...
function generateJson(row) {
    ...
    var flag = 0;
    var i_val = "";
    row.find('.table_json').val(JSON.stringify(data_item));
