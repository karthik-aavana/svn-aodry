$(document).ready(function () {
    $("#input_purchase_credit_note_code").keypress(function (event) {
        if (event.which == 13) {
            return false;
        }
    });
    $(document).on("click", '#purchase_credit_note_submit,#purchase_credit_note_pay_now', function (event) {
        ...
        if ($('#invoice_number').val() == "") {
            ...
            if (section_area == "edit_purchase_credit_note" || section_area == "edit_purchase_credit_note" || section_area == "edit_delivery_challan" || section_area == "convert_quotation") {
        ...
        if ($('#purchase_invoice_number').val() == "") {
            $('#err_purchase_invoice_number').text("Please Select the Purchase reference.");
        ...
        var tablerowCount = $('#purchase_credit_note_table_body tr').length;
        if (tablerowCount < 1) {
            $("#err_product").text("Please Select Items");
            $('#input_purchase_credit_note_code').focus();
            return !1
        }
    ...
    $("#invoice_number").on("blur", function (event) {
        ...
        if (invoice_number == null || invoice_number == "") {
            ...
            $("#err_invoice_number").text("");
            if (section_area == "edit_purchase_credit_note" || section_area == "edit_purchase" || section_area == "edit_delivery_challan" || section_area == "convert_quotation") {
                var invoice_number_old = $("#invoice_number_old").val();
                if (invoice_number_old != invoice_number) {
                    invoice_number_count();
                }
