...
$(document).ready(function () {
    ...
    $('#input_purchase_credit_note_code').on('change focus', function () {
        $('#input_purchase_credit_note_code').trigger('click');
    });
    ...
    $('#purchase_credit_note_table_body').on('change blur','input[name=item_quantity],input[name=item_taxable_value],input[name=item_grand_total]',function(){
        setDefault($(this));
    })

    $("#purchase_invoice_number").change(function (event) {
        var purchase_id = $('#purchase_invoice_number').val();
        k = 'purchase_'+purchase_id;
        var taxes = purchase_invoice_ary[k];
        var purchase_date = taxes.purchase_date;
        var purchase_billing_state_id = taxes.purchase_billing_state_id;
        var purchase_billing_country_id = taxes.purchase_billing_country_id;
        var nature_of_supply = taxes.purchase_nature_of_supply;
        var type_of_supply = taxes.purchase_type_of_supply;
        var currency_id = taxes.currency_id;
        var gstPayable = taxes.purchase_gst_payable;
        var shipping_address_id = taxes.shipping_address_id;
        var billing_address_id = taxes.billing_address_id;
        
        var freight_val = '';
        $('#shipping_address_id').val(shipping_address_id);
        /*$('#invoice_date').val(purchase_date);*///.attr('disabled',true)
        /*$('#invoice_date_old').val(purchase_date);*/
        $('[name=billing_country]').val(purchase_billing_country_id).change();
        if(purchase_billing_state_id == 0){
            $('[name=billing_state]').val(0).change();
        }else{
            $('[name=billing_state]').val(purchase_billing_state_id).change();
        }        
        ...
        if (purchase_id != "") {
            $.ajax({
                ...
                success: function (result) {
                    ...
                    table_index = 0;
                    purchase_credit_note_data = new Array();
                    $("#purchase_credit_note_table_body").html('');
                    ...
                    /*purchase_credit_note_data = new Array();*/
                    var table_data = JSON.stringify(purchase_credit_note_data);
    ...
    function suggestion_box() {
        ...
        if ($('#purchase_invoice_number').val() != "") {
            $('#input_purchase_credit_note_code').prop('disabled', false);
            $('.search_purchase_credit_note_code').show();
            $('#err_purchase_credit_note_code').text('');
        } else {
            $('.search_purchase_credit_note_code').hide();
            $('#input_purchase_credit_note_code').prop('disabled', true);
            $('#err_purchase_credit_note_code').text('Please select the supplier to do purchase credit note.');
            $('#err_product').text('');
    ...
    $('#supplier').change(function () {
        if ($('#supplier').val() != "") {
            ...
            $.ajax({
                url: base_url + 'purchase_credit_note/get_purchase_invoice_number',
                type: 'POST',
                dataType: "json",
                data: {
                ...
                success: function (data) {
                    purchase_invoice_ary = {};
                    $.each(data,function(k,v){
                        purchase_invoice_ary['purchase_'+v.purchase_id] = v;
                    })
                    
                    $('#purchase_invoice_number').html('');
                    $('#purchase_invoice_number').append('<option value="">Select</option>');
                    for (var i = 0; i < data.length; i++) {
                        $('#purchase_invoice_number').append('<option value="' + data[i].purchase_id + '">' + data[i].purchase_invoice_number + '</option>');
                    }
    ...
    $('#input_purchase_credit_note_code').autoComplete({
        minChars: 1,
        cache: false,
        source: function (term, suggest) {
            ...
            $.ajax({
                url: base_url + "purchase_credit_note/get_purchase_credit_note_suggestions/" + term + "/" + inventory_advanced + "/" + item_access,
                type: "GET",
                dataType: "json",
                success: function (data) {
                    var suggestions = [];
                    ...
                    if (i == 1 && term.length > 7 ) {
                        ...
                        $.ajax({
                            url: base_url + 'purchase_credit_note/get_table_items/' + mapping[data[0].item_code],
                            type: "GET",
                            dataType: "JSON",
                            success: function (data1) {
                                add_row(data1);
                                call_css();
                                $('#input_purchase_credit_note_code').val('')
                            }
                        });
                        suggest('');
                    } else {
        ...
        onSelect: function (event, ui) {
            ...
            $.ajax({
                url: base_url + 'purchase_credit_note/get_table_items/' + mapping[code[0]],
                type: "GET",
                dataType: "JSON",
                success: function (data) {
    ...
    //delete rows 
    $("#purchase_credit_note_table_body").on("click", "a.deleteRow", function (event) {
    ...
    // changes in rows
    $("#purchase_credit_note_table_body").on("change blur", 'input[name^="item_price"],input[name^="item_description"],input[name="item_quantity"],select[name="item_discount"],select[name="item_tax"]', function (event) {
        var newRow = $(this).closest("tr");
        calculateTable(newRow);
    });

    $("#purchase_credit_note_table_body").on("change",'select[name="item_tax_cess"]',function (event) {
            var newRow = $(this).closest("tr");
            calculateTable(newRow);
        }
    );

    $("#purchase_credit_note_table_body").on("change",'select[name="item_tds_percentage"]',function (event) {
            /**/
            var newRow = $(this).closest("tr");
    ...
    $("#purchase_credit_note_table_body").on("change", 'input[name="item_grand_total"]', function (event) {
        ...
        var item_taxable_value = (+item_grand_total * 100) / (100 + (+item_tds_percentage + +item_tax_percentage + +item_tax_cess_percentage));
...
function add_row(data, loop = "") {
    ...
    var purchase_item_discount_id = purchase_item.purchase_item_discount_id;
    var item_discount_percentage = purchase_item.purchase_item_discount_percentage;
    var item_tds_id = purchase_item.purchase_item_tds_id;
    var item_tds_amount = purchase_item.purchase_item_tds_amount;
    var item_tds_percentage = purchase_item.purchase_item_tds_percentage;
    var item_price = typeof purchase_item.purchase_item_unit_price != 'undefined' ? purchase_item.purchase_item_unit_price : 0;
    var item_description = typeof purchase_item.purchase_item_description != 'undefined' ? purchase_item.purchase_item_description : '';
    var item_quantity = typeof purchase_item.purchase_item_quantity != 'undefined' ? purchase_item.purchase_item_quantity : '';
    var item_tax_id = purchase_item.purchase_item_tax_id;
    var item_tax_percentage = purchase_item.purchase_item_tax_percentage;
    var item_tax_amount = purchase_item.purchase_item_tax_amount;
    var item_tax_cess_id = purchase_item.purchase_item_tax_cess_id;
    var item_tax_cess_percentage = purchase_item.purchase_item_tax_cess_percentage;
    var item_tax_cess_amount = purchase_item.purchase_item_tax_cess_amount;
    var grand_total = purchase_item.purchase_item_grand_total;
    ...
    if (settings_discount_visible != "no") {
        ...
        if(typeof item_discount_percentage == 'undefined')item_discount_percentage = 0;
        for (a = 0; a < data.discount.length; a++) {
            selected = '';
            if(purchase_item_discount_id == data.discount[a].discount_id) selected= 'selected';
            if(parseFloat(data.discount[a].discount_value) <= parseFloat(item_discount_percentage)){
                select_discount += '<option value="' + data.discount[a].discount_id + '-' + parseFloat(data.discount[a].discount_value) + '" '+selected+' >' + parseFloat(data.discount[a].discount_value) + '%</option>'
            }
        }
    ...
    /*if (tax_exist == 1) {
        if (discount_exist == 1) {
            cols += "<td style='text-align:center'><input type='hidden' name='item_taxable_value' value='" + item_price + "' limit='"+item_price+"'><span id='item_taxable_value_lbl_" + table_index + "'>" + item_price + "</span></td>";
        }
    }*/
    ...
    newRow.append(cols);
    $("#purchase_credit_note_table_body").prepend(newRow);
    table_index++;
    calculateTable(newRow);
}
//calculate row
...
function calculateTax(row) {
    ...
    if (settings_tax_type != "no_tax") {
        ...
        if(tax_data != '' && typeof tax_data != 'undefined' ){
            ...
            if (item_tax == "" || isNaN(item_tax) || $('#type_of_supply').val() == 'import') {
                item_tax = 0;
            }
        }
        ...
        if(cess_tax != '' && typeof cess_tax != 'undefined' ){
            ...
            if (item_tax_cess == "" || isNaN(item_tax_cess)  || $('#type_of_supply').val() == 'import') {
                item_tax_cess = 0;
            }
        }
        var type_of_supply = $("#type_of_supply").val();
    ...
    row.find('input[name^="item_tax_amount_sgst"]').val(purchase_sgst_amount);
    row.find('input[name^="item_tax_amount_cgst"]').val(purchase_cgst_amount);
...
function generateJson(row) {
    ...
    var flag = 0;
    var i_val = "";
    for (var i = 0; i < purchase_credit_note_data.length; i++) {
        if (purchase_credit_note_data[i] !== "undefined" && purchase_credit_note_data[i].item_key_value == table_index) {
            flag = 1;
            i_val = i;
            break;
        }
    ...
    if (flag == 1) {
        purchase_credit_note_data[i_val] = data_item;
    } else {
        purchase_credit_note_data.push(data_item);
    }
...
function calculateGrandTotal() {
    ...
    var tdsflag = 0;
    var tcsflag = 0;
    $("#purchase_credit_note_table_body").find('input[name="item_sub_total"]').each(function () {
        sub_total += +$(this).val()
    });
    $("#purchase_credit_note_table_body").find('input[name="item_taxable_value"]').each(function () {
        taxable += +$(this).val()
    });
    $("#purchase_credit_note_table_body").find('input[name="item_discount_amount"]').each(function () {
        dflag = 1;
        discount += +$(this).val()
    });
    $("#purchase_credit_note_table_body").find('input[name="item_tax_amount"]').each(function () {
        tflag = 1;
        tax += +$(this).val()
    });

    $("#purchase_credit_note_table_body")
        .find('input[name="item_tax_cess_amount"]')
        .each(function () {
            tflag = 1;
            tax_cess += +$(this).val();
    ...
    $("#purchase_credit_note_table_body").find('input[name^="item_tds_amount"]').each(function () {
        row = $(this).closest("tr");
    ...
    $("#purchase_credit_note_table_body").find('input[name="item_grand_total"]').each(function () {
...
function deleteRow(row) {
    ...
    var j = 0;
    var purchase_credit_note_data_temp = new Array();

    for (var i = 0; i < purchase_credit_note_data.length; i++) {

        if (purchase_credit_note_data[i] !== "undefined" && purchase_credit_note_data[i].item_key_value != table_index_key) {
            // purchase_credit_note_data.splice(i, 1);
            var data_item = {
                "item_key_value": +purchase_credit_note_data[i].item_key_value,
                "item_id": +purchase_credit_note_data[i].item_id,
                "item_type": purchase_credit_note_data[i].item_type,
                "item_code": purchase_credit_note_data[i].item_code,
                "item_quantity": +purchase_credit_note_data[i].item_quantity,
                "item_price": +purchase_credit_note_data[i].item_price,
                "item_description": purchase_credit_note_data[i].item_description,
                "item_sub_total": +purchase_credit_note_data[i].item_sub_total,
                "item_discount_amount": +purchase_credit_note_data[i].item_discount_amount,
                "item_discount_id": +purchase_credit_note_data[i].item_discount_id,
                "item_discount_percentage": +purchase_credit_note_data[i].item_discount_percentage,
                "item_tax_amount": +purchase_credit_note_data[i].item_tax_amount,
                "item_tax_id": +purchase_credit_note_data[i].item_tax_id,
                "item_tax_percentage": +purchase_credit_note_data[i].item_tax_percentage,
                "item_tds_amount": +purchase_credit_note_data[i].item_tds_amount,
                "item_tds_id": +purchase_credit_note_data[i].item_tds_id,
                "item_tds_percentage": +purchase_credit_note_data[i].item_tds_percentage,
                "item_tds_type": +purchase_credit_note_data[i].item_tds_type,
                "item_taxable_value": +purchase_credit_note_data[i].item_taxable_value,
                "item_grand_total": +purchase_credit_note_data[i].item_grand_total
            };
            purchase_credit_note_data_temp.push(data_item);
        }
    }

    purchase_credit_note_data = new Array();
    purchase_credit_note_data.length = 0;
    purchase_credit_note_data = purchase_credit_note_data_temp;
