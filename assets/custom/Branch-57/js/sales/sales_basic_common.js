...
$(document).ready(function () {
    ...
    $("#sales_table_body").on(
        "change blur",
        'input[name="item_price"],input[name="item_description"],input[name="item_quantity"],select[name="item_mrkd_discount"],select[name="item_mrgn_discount"],select[name="item_out_tax"],select[name="item_tax"],select[name="item_tax_cess"]',
        function (event) {
            var newRow = $(this).closest("tr");
    ...
    $("#sales_table_body").on(
        ...
        function (event) {
            ...
            var item_mrkd_discount_amount = 0;
            var item_tax_amount = 0;
            var item_tax_cess_amount = 0;
            var item_tds_amount = 0;
            var item_mrkd_discount_percentage = 0;
            var item_tax_percentage = 0;
            var item_tax_cess_percentage = 0;
            var item_tds_percentage = 0;
            if (settings_discount_visible != "no") {
                item_mrkd_discount_amount = +parseFloat(
                    row.find('input[name^="item_mrkd_discount_amount"]').val()
                );
                item_mrkd_discount_percentage = +parseFloat(
                    row.find('input[name^="item_mrkd_discount_percentage"]').val()
                );
            }
            ...
            var item_sub_total =
                (+item_taxable_value * 100) / (100 - +item_mrkd_discount_percentage);
...
function add_row(data) {
    ...
    if(item_mrkd_discount == ""){
       var item_mrkd_discount = 0; 
    }
    ...
    if (settings_discount_visible != "no") {
        ...
        select_discount +=
            '<select class="form-control open_discount form-fixer select2" name="item_mrkd_discount" style="width: 100%;">';
        select_discount += '<option value="">Select</option>';
        for (a = 0; a < discount_ary.length; a++) {
            var selected = "";
                if (item_mrkd_discount == discount_ary[a].discount_id) {
        ...
        discount_exist = 1;
        var select_mrgn_discount = "";
        if (input_type == "hidden") {
            select_mrgn_discount +=
                '<div class="form-group" style="margin-bottom:0px !important;display:none">';
        } else {
            select_mrgn_discount +=
                '<div class="form-group" style="margin-bottom:0px !important;">';
        ...
        select_mrgn_discount +=
            '<select class="form-control open_discount form-fixer select2" name="item_mrgn_discount" style="width: 100%;">';
        select_mrgn_discount += '<option value="">Select</option>';
        for (a = 0; a < discount_ary.length; a++) {
            var selected = "";
            if (item_mrg_discount == discount_ary[a].discount_id) {
               var selected = "selected";
            }
            select_mrgn_discount +='<option value="' +discount_ary[a].discount_id +
                "-" +parseFloat(discount_ary[a].discount_value) +'" '+selected +' >' +
                parseFloat(discount_ary[a].discount_value) + "%</option>";
        }
        select_mrgn_discount += "</select></div>";
    }
...
//        select_tds += "</select></div>";
        ...
        for (a = 0; a < tax_data.length; a++) {
            ...
            if(tax_data[a].tax_name == 'TDS' || tax_data[a].tax_name == 'TCS'){
                ...
                if(item_tds_type.toLowerCase() == tax_name.toLowerCase()){
                    ...
                    tds_body += '<tr>\
                            <td>'+tax_data[a].tax_name+'(Sec '+tax_data[a].section_name+') @ '+parseFloat(tax_data[a].tax_value) +'%</td>\
                            <td><div class="radio">\
                                    <label><input type="radio" name="tds_tax" value="'+precise_amount(tax_data[a].tax_value) +
                       '"'+selected +" tds_id='"+tax_data[a].tax_id+"' typ='"+tax_data[a].tax_name+"'></label>\
                                </div></td></tr>";
                }
            }
        }
        ...
        select_tax_out +=
            '<select class="form-control open_tax form-fixer select2" name="item_out_tax" style="width: 100%;" '+gst_disable+'>';
        select_tax_out += '<option value="">Select</option>';

        for (a = 0; a < tax_data.length; a++) {
            var selected = "";
            if (item_out_tax_id == tax_data[a].tax_id) {
                var selected = "selected";
            }

            if(tax_data[a].tax_name == 'GST'){
                select_tax_out +=
                    '<option value="' +
                    tax_data[a].tax_id +
                    "-" +
        ...
        for (a = 0; a < tax_data.length; a++) {
            default_tax = 5;
            if(item_basic_price >= 1000){
                default_tax = 18;
            }else{
                if($('#type_of_supply').val() == 'regular' && branch_state_id != $('#billing_state').val())
                default_tax = 9;
            }

            var selected = "";
            /*if (item_tax_id == tax_data[a].tax_id) {
                var selected = "selected";
            }*/
            if(default_tax == parseFloat(tax_data[a].tax_value)){
                var selected = "selected";
            }
            if(tax_data[a].tax_name == 'GST'){
                select_tax +=
    ...
    /* customize */
    cols += "<td style='text-align:center'>"+uom+"</td>";
    ...
    if (discount_exist == 1) {
        if (input_type == "hidden") {
            cols +=
                "<td style='text-align:center'>" +
                "<span id='item_mrkd_discount_hide_lbl_" +
                table_index +
        ...
        cols +=
            "<input type='hidden' name='item_mrkd_discount_id' value='0'>" +
            "<input type='hidden' name='item_mrkd_discount_percentage' value='0'>" +
            "<input type='hidden' name='item_mrkd_discount_amount' value='0'>" +
            select_discount +
            "<span id='item_mrkd_discount_lbl_" +
            table_index +
            "' class='pull-right' " +
            color_style +
            " >0.00</span>" +
    ...
    cols +=
        "<input type='hidden' name='item_mrgn_discount_id' value='0'>" +
        "<input type='hidden' name='item_mrgn_discount_percentage' value='0'>" +
        "<input type='hidden' name='item_mrgn_discount_amount' value='0'>" +
        select_mrgn_discount +
        "<span id='item_mrgn_discount_lbl_" +
        table_index +
        "' class='pull-right' " +
        color_style +
        " >0.00</span>" +
    ...
    cols += "<td style='text-align:center'>"+
            select_tax_out +
            "<span id='item_out_tax_lbl_" +
            table_index +
            "' class='pull-right' style='color:red;'>0.00</span><input type='hidden' value='' name='item_out_tax_percentage'><input type='hidden' value='' name='item_out_tax_id'><input type='hidden' name='item_out_tax_amount' value=''></td>";
    //taxable area
    if (tax_exist == 1) {
        if (discount_exist == 1) {
            ...
            cols +=
                ...
                item_basic_price +
                "' ><input type='hidden' name='item_sub_total' value='" +
                item_basic_price +
...
function calculateRow(row) {
    ...
    var item_quantity = +row.find('input[name^="item_quantity"]').val();
    var item_basic_total = parseFloat(precise_amount(item_price) * item_quantity);
    row.find('input[name^="item_basic_total"]').val(precise_amount(item_basic_total));
    row.find("#item_basic_total_lbl_" + table_index).text(item_basic_total);

    if (settings_item_editable == "no") {
        row.find("#item_price_hide_lbl_" + table_index).text(item_basic_total);
    }
}
// calculate row
//calculate Discount
...
function calculateDiscount(row) {
    ...
    var discount_data = row.find('select[name^="item_mrkd_discount"]').val();
    if (discount_data == "" || typeof discount_data == 'undefined') {
        discount_data = 0;
    }
    var discount_split = discount_data.toString().split("-");
    var item_mrkd_discount_id = +discount_split[0];
    var item_mrkd_discount = +parseFloat(discount_split[1]);

    if (item_mrkd_discount == "" || isNaN(item_mrkd_discount)) {
        item_mrkd_discount = 0;
    }
    var item_basic_total = +parseFloat(
        row.find('input[name^="item_basic_total"]').val()
    );
    var item_mrkd_discount_amount = (precise_amount(item_basic_total) * item_mrkd_discount) / 100;

    row.find('input[name^="item_mrkd_discount_amount"]').val(precise_amount(item_mrkd_discount_amount));
    row.find('input[name^="item_mrkd_discount_id"]').val(item_mrkd_discount_id);
    row.find('input[name^="item_mrkd_discount_percentage"]').val(item_mrkd_discount);
    row.find("#item_mrkd_discount_lbl_" + table_index).text(item_mrkd_discount_amount);

    if (settings_item_editable == "no") {
        row
            .find("#item_mrkd_discount_hide_lbl_" + table_index)
            .text(item_mrkd_discount_amount);
    }
    var selling_price = item_basic_total - item_mrkd_discount_amount;
    row.find('input[name^=item_selling_price]').val(precise_amount(selling_price));
    row.find('#item_selling_price_lbl_' + table_index).text(precise_amount(selling_price));
}

function calculateMrgnDiscount(row){
    var table_index = +row.find('input[name^="item_key_value"]').val();

    var discount_data = row.find('select[name^="item_mrgn_discount"]').val();
    if (discount_data == "" || typeof discount_data == 'undefined') {
        discount_data = 0;
    }
    var discount_split = discount_data.toString().split("-");
    var item_mrgn_discount_id = +discount_split[0];
    var item_mrgn_discount = +parseFloat(discount_split[1]);

    if (item_mrgn_discount == "" || isNaN(item_mrgn_discount)) {
        item_mrgn_discount = 0;
    }
    var item_selling_price = +parseFloat(
        row.find('input[name^="item_selling_price"]').val()
    );
    var item_mrgn_discount_amount = (precise_amount(item_selling_price) * item_mrgn_discount) / 100;

    row.find('input[name^="item_mrgn_discount_amount"]').val(precise_amount(item_mrgn_discount_amount));
    row.find('input[name^="item_mrgn_discount_id"]').val(item_mrgn_discount_id);
    row.find('input[name^="item_mrgn_discount_percentage"]').val(item_mrgn_discount);
    row.find("#item_mrgn_discount_lbl_" + table_index).text(precise_amount(item_mrgn_discount_amount));

    if (settings_item_editable == "no") {
        row
            .find("#item_mrgn_discount_hide_lbl_" + table_index)
            .text(precise_amount(item_mrgn_discount_amount));
    }
}

function calculateOutTax(row){
    var table_index = +row.find('input[name^="item_key_value"]').val();
    var tax_data = row.find('select[name="item_out_tax"]').val();
    ...
    var item_out_tax_amount = (item_selling_price * item_tax) / (100 + item_tax);
    row.find("#item_out_tax_lbl_" + table_index).text(precise_amount(item_out_tax_amount));
    row.find('input[name^="item_out_tax_amount"]').val(precise_amount(item_out_tax_amount));
    row.find('input[name^="item_out_tax_id"]').val(item_tax_id);
    row.find('input[name^="item_out_tax_percentage"]').val(precise_amount(item_tax));
}
...
function calculateTax(row) {
    ...
    var item_mrkd_discount_amount = +parseFloat(
        row.find('input[name^="item_mrkd_discount_amount"]').val()
    );
    var item_taxable_value = precise_amount(item_sub_total);
    /*if (typeof item_mrkd_discount_amount != "undefined" && !isNaN(item_mrkd_discount_amount)) {
        item_taxable_value = precise_amount(item_sub_total) - precise_amount(item_mrkd_discount_amount);
...
function calculateTds(row) {
    ...
    var item_taxable_value = precise_amount(item_sub_total);
    /*if (
        typeof item_mrkd_discount_amount != "undefined" &&
        !isNaN(item_mrkd_discount_amount)
    ) {
        item_taxable_value = precise_amount(item_sub_total) - precise_amount(item_mrkd_discount_amount);
    }*/
...
function preciseRowAmount(row) {
    ...
    if (
        ...
        row
            .find('input[name^="item_mrkd_discount_amount"]')
            .val(precise_amount(item_mrkd_discount_amount));
        row
            .find("#item_mrkd_discount_lbl_" + table_index)
            .text(precise_amount(item_mrkd_discount_amount));
...
function generateJson(row) {
    var table_index = +row.find('input[name^="item_key_value"]').val();
    var item_price = +parseFloat(row.find('input[name^="item_price"]').val());
    var item_selling_price  = +parseFloat(row.find('input[name^="item_selling_price"]').val());
    ...
    var item_basic_total = +parseFloat(row.find('input[name^="item_basic_total"]').val());

    var item_mrkd_discount_amount = +parseFloat(
        row.find('input[name^="item_mrkd_discount_amount"]').val()
    );
    var item_mrgn_discount_amount = +parseFloat(
        row.find('input[name^="item_mrgn_discount_amount"]').val()
    );
    var item_tax_amount = +parseFloat(
        row.find('input[name="item_tax_amount"]').val()
    );
    var item_out_tax_amount = +parseFloat(
        row.find('input[name="item_out_tax_amount"]').val()
    );
    var item_tax_cess_amount = +parseFloat(
        row.find('input[name="item_tax_cess_amount"]').val()
    );
    var item_mrkd_discount_id = +row.find('input[name^="item_mrkd_discount_id"]').val();
    var item_mrgn_discount_id = +row.find('input[name^="item_mrgn_discount_id"]').val();
    var item_tax_id = +row.find('input[name^="item_tax_id"]').val();
    var item_out_tax_id = +row.find('input[name^="item_out_tax_id"]').val();
    ...
    var item_mrkd_discount_percentage = +parseFloat(
        row.find('input[name^="item_mrkd_discount_percentage"]').val()
    );
    var item_mrgn_discount_percentage = +parseFloat(
        row.find('input[name^="item_mrgn_discount_percentage"]').val()
    );
    var item_tax_percentage = +parseFloat(
        row.find('input[name^="item_tax_percentage"]').val()
    );
    var item_out_tax_percentage = +parseFloat(
        row.find('input[name^="item_out_tax_percentage"]').val()
    );
    ...
    if (typeof item_mrkd_discount_amount === "undefined" || isNaN(item_mrkd_discount_amount)) {
        item_mrkd_discount_amount = 0;
        item_mrkd_discount_id = 0;
        item_mrkd_discount_percentage = 0;
    }
    ...
    var data_item = {
        ...
        item_basic_total:item_basic_total,
        item_selling_price : item_selling_price,
        item_sub_total: +item_sub_total,
        item_mrkd_discount_amount: +item_mrkd_discount_amount,
        item_mrkd_discount_id: +item_mrkd_discount_id,
        item_mrkd_discount_percentage: +item_mrkd_discount_percentage,
        item_mrgn_discount_amount: +item_mrgn_discount_amount,
        item_mrgn_discount_id: +item_mrgn_discount_id,
        item_mrgn_discount_percentage: +item_mrgn_discount_percentage,
        item_tax_amount: +item_tax_amount,
        item_tax_id: +item_tax_id,
        item_tax_percentage: +item_tax_percentage,
        item_out_tax_percentage: +item_out_tax_percentage,
        item_out_tax_amount: +item_out_tax_amount,
        item_out_tax_id: +item_out_tax_id,
    ...
    for (var i = 0; i < sales_data.length; i++) {
        if (sales_data[i] !== "undefined" && sales_data[i].item_key_value != table_index_key) {
            ...
            var data_item = {
                ...
                item_mrkd_discount_amount: +sales_data[i].item_mrkd_discount_amount,
                item_mrkd_discount_id: +sales_data[i].item_mrkd_discount_id,
                item_mrkd_discount_percentage: +sales_data[i].item_mrkd_discount_percentage,
...
$("#cmb_department").change(function () {
    if ($("#cmb_department").val() != "") {        
        var department_id = $("#cmb_department").val();
        $.ajax({
            url: base_url + "general/get_subdepartment",
            dataType: "JSON",
            method: "POST",
            data: {
            ...
            success: function (result) {
                var data = result["sub_department"];
                $("#cmb_subdepartment").html("");
                $("#cmb_subdepartment").append('<option value="">Select</option>');
                for (i = 0; i < data.length; i++) {
                   
                    $("#cmb_subdepartment").append(
                        '<option value="' +
                        data[i].sub_department_id +
                        '">' +
                        data[i].sub_department_name +
                        "</option>"
                    );
                }
