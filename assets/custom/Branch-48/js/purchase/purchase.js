$(document).ready(function () {
    ...
    $(document).on("click", '#purchase_submit,#purchase_pay_now', function (event) {
        ...
        var flag = true;
        $(document).find('#purchase_table_body tr').each(function () {
            var rows = $(this);
            rows.find('[name=input_sales_code_err]').text("");
            if(rows.find('[name=input_purchase_code]').length > 0){
                if(rows.find('[name=input_purchase_code]').val() == ''){
                    rows.find('[name=input_sales_code_err]').text("Please enter product name.");
                    rows.find('[name="input_purchase_code"]').focus();
                    flag = false;
                }
            }
