                ...
                success: function (data) {
                   ...
                   // console.log(isnum);
                    ...
                    if (i == 1 && term.length > 7 ) {
                        ...
                        var item_keys = mapping[k].split("-");
                        var key = item_keys[1]+item_keys[0];/* key from item type and item ID.
                        
                        if(typeof items_jsn[key] != 'undefined'){
                            var row_index = items_jsn[key];
                            ...
                            $.ajax({
                                ...
                                success: function (data1) {
                                    $('#table-total').show();
                                    var key = data1.item_type+data1.item_id;/* key from item type and item ID./
        ...
        onSelect: function (event, ui) {
            ...
            if(typeof items_jsn[key] != 'undefined'){
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        $('#table-total').show();
                        var key = data.item_type+data.item_id;/* key from item type and item ID/
    ...
    $("#input_sales_code").autoComplete({
        ...
        source: function (term, suggest) {
            ...
            if(filterpros.length == 1){
                var key = filterpros[0].item_type+''+''+filterpros[0].item_id;/* key from item type and item ID.*/
                ...
                $.each(filterpros, function(k,v){
                    ...
                    var item_code = v.item_code+'-'+v.product_batch;
                    var code = item_code.toString().split(' ');
                    mapping[code[0]] =
                        v.item_id +
                        "-" +
...
function generateJson(row) {
    ...
    items_jsn[item_type+item_id] = table_index;
