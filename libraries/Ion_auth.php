...
class Ion_auth
{
    ...
    {
        if ($this->ion_auth_model->forgotten_password($identity, $login_code))
        {
            // Get user information
            $identifier = $this->ion_auth_model->identity_column; // use model identity column, so it can be overridden in a controller
            $user       = $this->where($identifier, $identity)->where('active', 1)->where('branch_code', $login_code)->users()->row();
